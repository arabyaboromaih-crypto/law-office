...
try {
  ...
  console.error("[Server Setup]: Could not read firebase-applet-config.json:", err);
}
...
المطلوب: الإجابة على استفسار المحامي بكل دقة ومهنية قانونية بالاعتماد التام على تفاصيل القضية والقواعد القانونية المصرية المتبعة. إذا كان السؤال عن مستندات أو مبالغ أو جلسات، استخرج الإجابة بدقة من بيانات القضية. رتب الإجابة ونسقها بـ Markdown.`;
  ...
  app.get("/api/proxy", async (req, res) => {
    ...
    try {
      ...
      if (decodedUrl.includes("%2F") || decodedUrl.includes("%3A")) {
        try {
      ...
      const getMimeFromPath = (pathOrUrl: string): string | null => {
      ...
      if (decodedUrl.startsWith("https://firebasestorage.googleapis.com")) {
        ...
        const detectedType = getMimeFromPath(decodedUrl) || response.headers.get("content-type") || "application/octet-stream";
      ...
      if (decodedUrl.includes("cloudflarestorage.com") || decodedUrl.includes(R2_BUCKET_NAME)) {
        ...
        try {
          ...
          const detectedType = getMimeFromPath(key) || r2Response.ContentType || "application/octet-stream";
          ...
          const queryFilename = (req.query.filename as string) || path.basename(key) || "document";
          ...
          console.error("[Server Proxy R2 Error, trying direct fetch]:", r2Err);
          try {
            const fallbackResp = await fetch(decodedUrl);
            if (fallbackResp.ok) {
              const detectedType = getMimeFromPath(decodedUrl) || fallbackResp.headers.get("content-type") || "application/octet-stream";
              res.setHeader("Content-Type", detectedType);
              res.setHeader("Content-Disposition", "inline");
              res.setHeader("Access-Control-Allow-Origin", "*");
              res.setHeader("Cache-Control", "public, max-age=86400");
      ...
      // Case C: Any HTTP/HTTPS storage URL (Stream directly using standard fetch)
      if (decodedUrl.startsWith("http")) {
        console.log(`[Server Proxy]: Fetching and streaming URL: ${decodedUrl}`);
        const response = await fetch(decodedUrl);
        if (!response.ok) {
          return res.status(response.status).send("Failed to fetch file from storage");
        }
  ...
  // Serve static public assets directly (pdf-viewer, sample.pdf, pdf.js, etc.)
