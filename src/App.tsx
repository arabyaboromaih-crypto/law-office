...
export default function App() {
  ...
  const syncAutoSessionsToFirestore = async (casesToSync: Case[], usersToSync: User[]) => {
    ...
    try {
      ...
      // B. Clean up any redundant duplicate session records in Firestore (same caseId + normalized date + sessionCategory)
      const seenSessions = new Map<string, HearingSession>();
      for (const s of currentSessions) {
        const normDate = normalizeHearingDate(s.date);
        if (!normDate) continue;
        const pCase = casesToSync.find(c => c.id === s.caseId);
        const cat = getSessionCategory(s, pCase);
      ...
      for (const s of currentSessions) {
        if (isAutoGeneratedSessionId(s.id) && !expectedAutoSessionsMap.has(s.id)) {
          // NEVER delete any session that has a decision, completed status, or recorded details (preserve as historical record)
          if (s.decision || s.status === 'completed' || s.whatHappened || s.requirements) {
            continue;
          }
  ...
  const handleUpdateCase = async (updated: Case) => {
    ...
    try {
      ...
      for (const expected of newCaseAutoSessions) {
        const existing = findMatchingSession(currentCaseSessions, expected, caseToSave, cases);
        if (existing) {
      ...
      setSessions(prev => {
        const remaining = prev.filter(s => !(s.caseId === caseToSave.id && isAutoGeneratedSessionId(s.id)));
      ...
      for (const oldSess of currentCaseSessions) {
        if (isAutoGeneratedSessionId(oldSess.id) && !activeAutoSessionIds.has(oldSess.id)) {
          if (oldSess.decision || oldSess.status === 'completed' || oldSess.whatHappened || oldSess.requirements) {
            continue;
          }
      ...
      if (caseToSave.opponent) {
        ...
        const existingOpp = opponents.find(o => o.name === caseToSave.opponent.name);
        if (existingOpp) {
  ...
  const handleUpdateSession = async (updated: HearingSession) => {
    ...
    try {
      ...
      const cleanUpdated: HearingSession = {
        ...updated,
        date: normUpdatedDate || updated.date,
        nextHearingDate: normNextDate || undefined,
        status: (updated.decision && updated.decision.trim() !== '')
          ? (updated.status === 'postponed' ? 'postponed' : 'completed')
      ...
      if (parentCase) {
        ...
        const finalNextHearingDate = isReferralToCourt 
          ...
          : (upcoming.hasUpcoming ? upcoming.date : (normNextDate || undefined));
        const finalNextHearingTime = isReferralToCourt
          ? (normNextDate ? (cleanUpdated.time || '09:00') : undefined)
          : (upcoming.hasUpcoming ? upcoming.time : (cleanUpdated.time || parentCase.nextHearingTime || '09:00'));
        ...
        if ((isDetention || isExpert) && !isReferralToCourt) {
          ...
          for (const expected of newCaseAutoSessions) {
            const existing = findMatchingSession(currentCaseSessions, expected, updatedCase, cases);
            if (existing) {
              const merged = mergeSessionData(existing, expected);
        ...
        nextSessions = deduplicateSessions(nextSessions, cases);
        setSessions(nextSessions);
