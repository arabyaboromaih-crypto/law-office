...
export default function App() {
  ...
  useEffect(() => {
    if (isSyncingAutoSessionsRef.current || !cases || cases.length === 0) return;

    const syncAutoSessionsToFirestore = async () => {
      isSyncingAutoSessionsRef.current = true;
      try {
        const expectedAutoSessionsMap = new Map<string, HearingSession>();
        ...
        cases.forEach(c => {
          ...
          if (c.files) {
            c.files.forEach(file => {
              ...
              expectedAutoSessionsMap.set(sessionId, {
                id: sessionId,
                caseId: c.id,
          ...
          if ((c.isReferredToExperts || c.expertReferral?.isReferred) && c.expertReferral?.sessions) {
            c.expertReferral.sessions.forEach((expSess, idx) => {
              ...
              expectedAutoSessionsMap.set(sessionId, {
                ...
                subject: `جلسة خبرة (${expSess.sessionType || 'حضور الخبير'}) - ${expSess.location || 'مكتب الخبراء'}`,
                status: (expSess.status as any) || (expSess.decisionOrAction ? 'completed' : 'pending'),
                decision: expSess.decisionOrAction || undefined,
                assignedLawyerId: c.assignedLawyerId || undefined,
                assignedLawyerName: users.find(u => u.id === c.assignedLawyerId)?.fullName || undefined,
                notes: expSess.notes || 'جلسة خبراء محددة سلفاً في ملف القضية.',
          ...
          if (c.detentionRenewals && c.detentionRenewals.length > 0) {
            c.detentionRenewals.forEach((ren, idx) => {
              ...
              // Next renewal date (Upcoming detention renewal session)
              if (ren.nextRenewalDate) {
                const nextSessionId = `session-detention-next-${c.id}-${renKey}`;
                expectedAutoSessionsMap.set(nextSessionId, {
                  id: nextSessionId,
                  caseId: c.id,
                  ...
                  court: ren.nextAuthority || ren.authority || c.investigationAuthority || c.court || 'النيابة العامة / المحكمة الجزئية',
                  circuit: c.circuit || 'غرفة المشورة / التجديدات',
                  type: c.type || 'جنح',
              ...
              if (renDate) {
                ...
                expectedAutoSessionsMap.set(renSessionId, {
                  id: renSessionId,
                  caseId: c.id,
                  caseNumber: c.investigationNumber ? `${c.investigationNumber}/${c.investigationYear || c.caseYearFirstInstance}` : c.caseNumberFirstInstance,
                  caseYear: c.investigationYear || c.caseYearFirstInstance,
                  clientName: c.clientName,
                  opponentName: c.investigationAuthority || (c.opponent?.name || 'النيابة العامة'),
                  court: ren.authority || c.investigationAuthority || c.court || 'النيابة العامة',
                  circuit: c.circuit || 'غرفة المشورة / التجديدات',
                  type: c.type || 'جنح',
                  date: renDate,
                  time: '09:00',
                  subject: `جلسة تجديد حبس احتياطي - ${ren.authority || 'النيابة العامة'}`,
                  status: ren.decision ? 'completed' : 'pending',
                  decision: ren.decision ? `${ren.decision} (${ren.duration || (ren.durationDays ? `${ren.durationDays} يوم` : 'تجديد حبس')})` : undefined,
                  assignedLawyerId: c.assignedLawyerId || undefined,
                  assignedLawyerName: users.find(u => u.id === c.assignedLawyerId)?.fullName || undefined,
          ...
          // 4. Investigation Procedures scan (سجل إجراءات التحقيق بالنيابة)
          if (c.investigationProcedures && c.investigationProcedures.length > 0) {
            c.investigationProcedures.forEach((proc, idx) => {
              if (proc.sessionDate) {
                const isDetention = (proc.actionType || '').includes('تجديد') || (proc.actionType || '').includes('حبس') || (proc.actionType || '').includes('استئناف أمر الحبس');
                if (isDetention) {
                  const procKey = proc.id || `proc-${idx}`;
                  const procSessionId = `session-proc-${c.id}-${procKey}`;
                  expectedAutoSessionsMap.set(procSessionId, {
                    id: procSessionId,
                    caseId: c.id,
                    caseNumber: c.investigationNumber ? `${c.investigationNumber}/${c.investigationYear || c.caseYearFirstInstance}` : c.caseNumberFirstInstance,
                    caseYear: c.investigationYear || c.caseYearFirstInstance,
                    ...
                    detentionDurationDays: proc.renewalDays,
                  });
                }
              }
            });
        ...
        for (const s of currentSessions) {
          const isAutoGenerated = 
            s.id.startsWith('session-expert-') ||
            s.id.startsWith('session-detention-') ||
            s.id.startsWith('session-proc-') ||
            s.id.startsWith('session-file-') ||
            s.id.startsWith('session-sync-expert-') ||
            s.id.startsWith('session-detention-next-') ||
            s.id.startsWith('session-detention-prev-');

          if (isAutoGenerated && !expectedAutoSessionsMap.has(s.id)) {
    ...
    try {
      ...
      if (updated.status === 'completed' || updated.status === 'postponed' || updated.decision) {
        ...
        if (parentCase) {
          ...
          // Only create a next pending session if nextHearingDate is a DIFFERENT date than current session date
