...
export default function App() {
  ...
  const handleUpdateCase = async (updated: Case) => {
    ...
    try {
      ...
      const currentCaseSessions = sessions.filter(s => s.caseId === updated.id);
      ...
      setSessions(prev => {
        ...
        return deduplicateSessions([...remaining, ...mergedAutoSessions], cases.map(c => c.id === updated.id ? updated : c));
      });
        ...
        if (!hasSessionOnDate) {
          ...
          if (pendingNonDecided) {
            const updatedPending: HearingSession = {
              ...
              opponentName: updated.opponent?.name || pendingNonDecided.opponentName,
              court: updated.court || pendingNonDecided.court,
              circuit: updated.circuit || pendingNonDecided.circuit,
              type: updated.type || pendingNonDecided.type,
              date: normCaseDate,
              time: updated.nextHearingTime || pendingNonDecided.time || '09:00',
              subject: updated.status || pendingNonDecided.subject || 'جلسة متداولة ومتابعة',
              assignedLawyerId: updated.assignedLawyerId || pendingNonDecided.assignedLawyerId,
              assignedLawyerName: users.find(u => u.id === (updated.assignedLawyerId || pendingNonDecided.assignedLawyerId))?.fullName
            };
            ...
            const newSession: HearingSession = {
              ...
              caseYear: updated.caseYearFirstInstance,
              clientName: updated.clientName,
              opponentName: updated.opponent?.name || 'غير محدد',
              court: updated.court,
  ...
  const handleUpdateSession = async (updated: HearingSession) => {
    ...
    try {
      ...
      if (parentCase) {
        ...
        if (isDetention) {
          ...
          if (matchedIdx !== -1) {
            updatedRenewals[matchedIdx] = {
              ...
              decision: cleanUpdated.decision || updatedRenewals[matchedIdx].decision || 'تجديد حبس',
              durationDays: currentDurationDays,
              duration: currentDuration,
              nextRenewalDate: normNextDate || updatedRenewals[matchedIdx].nextRenewalDate || '',
              authority: cleanUpdated.court || cleanUpdated.detentionAuthority || updatedRenewals[matchedIdx].authority || 'النيابة العامة',
              nextAuthority: cleanUpdated.detentionNextAuthority || cleanUpdated.detentionAuthority || updatedRenewals[matchedIdx].nextAuthority || 'غرفة المشورة / النيابة',
            ...
            updatedRenewals.push({
              ...
              decision: cleanUpdated.decision || 'تجديد حبس',
              durationDays: currentDurationDays,
              duration: currentDuration,
              nextRenewalDate: normNextDate || '',
              authority: cleanUpdated.court || cleanUpdated.detentionAuthority || 'النيابة العامة',
              nextAuthority: cleanUpdated.detentionNextAuthority || cleanUpdated.detentionAuthority || 'غرفة المشورة / النيابة',
        ...
        const upcoming = getClosestUpcomingSession(parentCase, nextSessions);
        const finalNextHearingDate = upcoming.hasUpcoming ? upcoming.date : (normNextDate || undefined);
        const finalNextHearingTime = upcoming.hasUpcoming ? upcoming.time : (cleanUpdated.time || parentCase.nextHearingTime || '09:00');
        ...
        // 5. For Detention & Expert, safely merge auto-generated sessions without creating duplicates
