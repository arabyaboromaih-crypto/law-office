...
import { getDueCollectionStatus } from './RealEstateData';
...
export default function AddCollectionReceiptModal({
  ...
  // Calculate Required Rent Sum
  const totalRequiredRent = useMemo(() => {
    return selectedDues.reduce((sum, d) => sum + (d.rentAmount || 0), 0);
  }, [selectedDues]);
  ...
  const handleToggleDue = (due: ReRentDue) => {
    if (selectionMode === 'single') {
      ...
      setCollectedAmount(due.rentAmount || 0);
    } else {
      let nextIds: string[];
      ...
      const nextTotal = availableTenantDues
        .filter(d => nextIds.includes(d.id))
  ...
  const handleSwitchMode = (mode: 'single' | 'multi') => {
    ...
    if (mode === 'single' && selectedDues.length > 1) {
      ...
      if (firstId) {
        ...
        if (d) setCollectedAmount(d.rentAmount || 0);
      }
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedDues.length === 0) {
      setErrorMessage('يرجى تحديد شهر واحد على الأقل لتحصيل الإيجار.');
      return;
    }
    if (numericCollected <= 0) {
      setErrorMessage('يرجى إدخال مبلغ محصل صحيح أكبر من الصفر.');
      return;
    }
          ...
          <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
            <div className="p-4 sm:p-6 overflow-y-auto space-y-5 flex-1 custom-scrollbar">
              ...
              <div className="bg-[#08111F]/80 border border-[#D4A84F]/20 rounded-2xl p-4 sm:p-5 space-y-3.5 shadow-sm">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2 max-h-36 overflow-y-auto p-1.5 custom-scrollbar bg-[#0F1D30]/60 rounded-xl border border-white/5">
                    {availableTenantDues.map((d) => {
                      ...
                      const isAlreadyCollected = cStatus === 'collected' || cStatus === 'prepaid';
            ...
            <div className="flex items-center justify-between p-3.5 sm:p-5 border-t border-[#D4A84F]/20 bg-[#0A1424] shrink-0 gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-2.5 mr-auto">
                ...
                <button
                  type="submit"
                  disabled={isSaving || selectedDues.length === 0 || Number(collectedAmount) <= 0}
                  className="px-6 sm:px-8 py-2.5 rounded-xl bg-gradient-to-r from-[#D4A84F] to-[#C3973E] hover:brightness-110 disabled:opacity-50 text-slate-950 font-black text-xs sm:text-sm shadow-lg shadow-[#D4A84F]/20 transition-all flex items-center gap-2 cursor-pointer"
