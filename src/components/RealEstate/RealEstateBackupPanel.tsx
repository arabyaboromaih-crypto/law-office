...
import { restoreRealEstateBackupBatch, fetchRealEstateLiveSnapshot, RealEstateBackupPayload } from '../../services/dbSync';
...
export default function RealEstateBackupPanel({
  ...
  // Helper: Merge primary live Firestore records with in-memory state (reconciling by ID without losing anything)
  const reconcileLists = <T extends { id: string }>(dbList: T[] = [], stateList: T[] = []): T[] => {
    const map = new Map<string, T>();
    // First populate from state
    (stateList || []).forEach(item => {
      if (item && item.id) map.set(item.id, item);
    });
    // Overlay/merge with fresh primary Firestore docs
    (dbList || []).forEach(item => {
      if (item && item.id) {
  ...
  // Action 1: Create Real Live Backup from primary database, trigger file download, save to list, and notify only on verified success
  const handleCreateBackup = async () => {
    try {
      setIsCreatingBackup(true);
      setStatusMsg(null);

      // Step 1: Query primary live Firestore database for fresh real estate snapshot
      let liveSnapshot: RealEstateBackupPayload['data'] | null = null;
      try {
        liveSnapshot = await fetchRealEstateLiveSnapshot();
      } catch (e) {
        console.warn('Could not fetch directly from Firestore, falling back to live state:', e);
      }

      // Step 2: Merge and reconcile all collections
      const finalOwners = reconcileLists(liveSnapshot?.owners, owners);
      const finalProperties = reconcileLists(liveSnapshot?.properties, properties);
      const finalUnits = reconcileLists(liveSnapshot?.units, units);
      const finalTenants = reconcileLists(liveSnapshot?.tenants, tenants);
      const finalCollections = reconcileLists(liveSnapshot?.collections, collections);
      const finalPayouts = reconcileLists(liveSnapshot?.payouts, payouts);
      const finalExpenses = reconcileLists(liveSnapshot?.expenses, expenses);
      const finalAdvances = reconcileLists(liveSnapshot?.advances, advances);
      const finalDues = reconcileLists(liveSnapshot?.dues, dues);
      const finalCommissionStatuses = reconcileLists(liveSnapshot?.commissionStatuses, commissionStatuses);
      const finalLogs = reconcileLists(liveSnapshot?.logs, logs);

      // Deleted dues
      const finalDeletedDues = (liveSnapshot?.deletedDues && liveSnapshot.deletedDues.length > 0)
        ? liveSnapshot.deletedDues
        : (deletedDueIds || []).map(id => ({ id }));

      const dtStr = formatDateTimeForFilename();
      const filename = `real_estate_backup_${dtStr}_v1.0.json`;
      ...
      const payload: RealEstateBackupPayload = {
        ...
        exportedBy: currentUser?.fullName || 'مدير النظام',
        filename,
        data: {
          owners: finalOwners,
          ...
          dues: finalDues,
          commissionStatuses: finalCommissionStatuses,
          deletedDues: finalDeletedDues,
          logs: finalLogs
        }
      };

      // Step 3: Validate payload serialization
      const jsonStr = JSON.stringify(payload, null, 2);
      if (!jsonStr || jsonStr.length < 50) {
        throw new Error('تعذر تجهيز بيانات النسخة الاحتياطية (محتوى الملف غير صالح أو فارغ).');
      }

      // Step 4: Execute real browser file download
      ...
      // Step 5: Save entry to local history list
      const backupItem: SavedBackupItem = {
        ...
        type: 'manual',
        note: `نسخة احتياطية يدوية مكتملة تم تجميعها من قاعدة البيانات الأساسية وتنزيلها (${new Date().toLocaleTimeString('ar-EG')})`,
        payload,
        stats: {
          ownersCount: finalOwners.length,
          propertiesCount: finalProperties.length,
          unitsCount: finalUnits.length,
          tenantsCount: finalTenants.length,
          collectionsCount: finalCollections.length,
          payoutsCount: finalPayouts.length,
          expensesCount: finalExpenses.length,
          advancesCount: finalAdvances.length,
          duesCount: finalDues.length,
          commissionStatusesCount: finalCommissionStatuses.length
        }
      };

      setSavedBackups(prev => [backupItem, ...prev]);

      // Step 6: Log operational action
      await logAction(
        'add',
        'النسخ الاحتياطي للإدارة العقارية',
        `تم إنشاء وتنزيل نسخة احتياطية حقيقية وشاملة للإدارة العقارية (${filename}) بنجاح.`
      ).catch(() => {});

      // Step 7: Display success notification ONLY after successful generation & download
      setStatusMsg({
        type: 'success',
        text: `✅ تم إنشاء النسخة الاحتياطية بنجاح وتم تنزيل الملف (${filename}) إلى جهازك.`
      });
    } catch (err: any) {
      console.error('Failed to create backup:', err);
      // Display real error message if execution fails
      setStatusMsg({
        type: 'error',
        text: `❌ فشل إنشاء النسخة الاحتياطية: ${err?.message || 'تعذر الوصول لقاعدة البيانات الأساسية أو تنزيل الملف'}`
      });
    } finally {
  ...
  const handleDownloadBackup = (backupItem: SavedBackupItem) => {
    try {
      ...
      setStatusMsg({
        type: 'error',
        text: `❌ فشل تنزيل الملف: ${err?.message || 'تعذر استخراج محتوى الملف'}`
      });
    }
  };
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10">
          ...
          <div className="flex flex-wrap items-center gap-2.5 w-full md:w-auto">
            ...
            <button
              ...
              className="px-4 py-3 bg-gradient-to-r from-[#D4A84F] to-[#B38734] hover:from-[#E5B95F] hover:to-[#C49845] text-slate-950 text-xs font-black rounded-2xl flex items-center gap-2 shadow-xl shadow-[#D4A84F]/10 active:scale-95 transition-all cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {isCreatingBackup ? (
                <>
                  <RefreshCw className="w-4 h-4 animate-spin stroke-[2.5]" />
                  <span>جاري تجميع البيانات والإنشاء...</span>
                </>
              ) : (
                <>
