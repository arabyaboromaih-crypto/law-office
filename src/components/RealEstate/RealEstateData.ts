...
export function getDueCollectionStatus(
  ...
  // Check if there is a matching active collection receipt in re_collections strictly for THIS tenant
  if (collections) {
    ...
    if (matchingReceipt) {
      const paidAmt = matchingReceipt.amountPaid || 0;
      const rentAmt = due.rentAmount || 0;
      if (paidAmt >= rentAmt || paidAmt > 0) {
    ...
    // If collections array is passed and NO active receipt exists for THIS tenant on this month/due,
    // it is strictly NOT collected!
...
export function calculatePropertyStatementsData(params: {
  ...
  propertyGroupsMap.forEach(group => {
    ...
    monthMap.forEach((mEntry, mKey) => {
      ...
      mEntry.advancesSum = monthAdv.reduce((s, a) => s + (a.amount || 0), 0);
...
export function calculateOwnerStatementsData(params: {
  ...
  ownerPropertyGroupsMap.forEach(group => {
    ...
    group.sumAdvancesDeducted = group.advancesDeducted.reduce((sum, a) => sum + (a.amount || 0), 0);
    ...
    propMonthsMap.forEach((mEntry, mKey) => {
      ...
      mEntry.advancesDeducted = monthAdv;
      mEntry.expensesDeducted = monthExp;
      mEntry.sumAdvancesDeducted = monthAdv.reduce((s, a) => s + (a.amount || 0), 0);
