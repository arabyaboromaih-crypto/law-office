...
import { 
  ReRentDue, ReOwner, ReProperty, ReUnit, ReTenant, User, ReCollectionReceipt, ReCommissionStatus 
} from '../../types';
import { getDueCollectionStatus, getDuePayoutStatus } from './RealEstateData';
...
export function generateRealEstateReportHTML(params: {
  ...
  const rawFilteredDues = dues.filter(d => {
    ...
    if (reportType === 'tenant_statement') {
      // Rule: Tenant statement is up to end of current month (forMonthYear <= currentMonthISO)
      // EXCEPT: Paid reserve/future months (forMonthYear > currentMonthISO && isCollected) ARE INCLUDED.
      ...
      if (!tenantToMonth && !(isCurrentOrPast || isPaidReserveFuture)) {
        return false;
      }
  ...
  const totalUnitsCount = selectedPropertyUnits.length || 1;
  const leasedUnitsCount = new Set(filteredDues.map(d => d.unitId)).size || selectedPropertyUnits.filter(u => u.status === 'rented').length;
  const vacantUnitsCount = Math.max(0, totalUnitsCount - leasedUnitsCount);
  const totalArrears = Math.max(0, sumRent - sumCollected);
  const collectionPercentage = sumRent > 0 ? Math.round((sumCollected / sumRent) * 100) : 0;
  ...
  const tenantOverdueDues = filteredDues.filter(d => getDueCollectionStatus(d) === 'overdue' || (getDueCollectionStatus(d) !== 'collected' && d.dueDate < todayISO));
  ...
  const tenantMonthlyRentAmount = selectedTenantObj?.rentAmount || tenantUnit?.rentValue || (filteredDues[0]?.rentAmount || 0);
    ...
    if (commStatements) {
      ...
      properties.forEach(prop => {
        ...
        if (accountType === 'total') {
          ...
          let earnedComm = filteredDues.reduce((s, d) => s + (d.commissionAmount || 0), 0);
          if (earnedComm === 0 && totalDueRent > 0 && owner?.commissionType === 'percentage') {
            earnedComm = (totalDueRent * (owner.commissionValue || 0)) / 100;
          }
          ...
          monthYearsList.forEach(my => {
            ...
            let earnedComm = myDues.reduce((s, d) => s + (d.commissionAmount || 0), 0);
            if (earnedComm === 0 && totalDueRent > 0 && owner?.commissionType === 'percentage') {
              earnedComm = (totalDueRent * (owner.commissionValue || 0)) / 100;
            }
  ...
  <div class="page-container">
    ...
    ` : reportType === 'tenant_statement' ? `
      <div class="kpi-row">
        ...
        <div class="kpi-card">
          <span class="kpi-lbl">الشهور المتأخرة</span>
          <span class="kpi-val">${tenantOverdueMonthsCount} شهر</span>
        </div>
        <div class="kpi-card">
          <span class="kpi-lbl">حالة المستأجر المالية</span>
          <span class="kpi-val"><span class="badge ${tenantFinancialBadgeClass}">${tenantFinancialStatus}</span></span>
        </div>
      </div>
      ...
      ${selectedTenantObj ? `
        <div style="background-color: #f8fafc; border: 1px solid #cbd5e1; border-radius: 8px; padding: 10px 14px; margin-bottom: 15px; font-size: 8.5pt;">
          ...
          <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; margin-bottom: 6px;">
            <div><strong>مدة المحاسبة:</strong> من ${selectedTenantObj.createdAt ? selectedTenantObj.createdAt.slice(0, 7) : (selectedTenantObj.contractStartDate ? selectedTenantObj.contractStartDate.slice(0, 7) : '—')} إلى ${currentMonthISO}</div>
          ...
          <div style="border-top: 1px dashed #cbd5e1; padding-top: 6px; margin-top: 4px; color: ${tenantOverdueMonthsCount > 0 ? '#dc2626' : '#059669'}; font-weight: bold;">
            <strong>بيان الأشهر المتأخرة عن السداد:</strong> ${tenantOverdueStatement}
          </div>
        </div>
      ` : ''}
    ` : `
      ...
      <table class="report-table">
        ${reportType === 'office_commissions' ? `
          <thead>
            <tr>
              <th style="width: 4%;">#</th>
              <th style="width: 22%;">اسم العقار والمالك والمستأجرين</th>
            ...
            ` : commItems.map((item, idx) => `
              <tr>
                ...
                <td style="font-weight: 700; text-align: right;">
                  ...
                  ${item.tenantNamesList ? `<div style="font-size: 7pt; color: #475569; font-weight: 600;">المستأجرون: ${item.tenantNamesList}</div>` : ''}
                </td>
        ...
        ` : reportType === 'tenant_statement' && selectedTenantId === 'all' ? `
          <thead>
            <tr>
              ...
              <th style="width: 9%;">الإيجار الحالي</th>
          ...
          <tbody>
            ${(() => {
              ...
              return displayTenants.map((t, idx) => {
                ...
                <tr class="total-row">
                  ...
                  <td class="num-font gold-text">${displayTenants.reduce((s, t) => s + (t.rentAmount || 0), 0).toLocaleString('ar-EG')} ج.م</td>
                  ...
                  <td class="num-font gold-text">${totalArrears.toLocaleString('ar-EG')} ج.م</td>
        ...
        ` : reportType === 'tenant_statement' ? `
          ...
          <tbody>
            ...
            ` : filteredDues.map((d, idx) => {
              ...
              let badgeText = 'بانتظار السداد';
              let badgeClass = 'badge-pending';
              let badgeCustomStyle = '';

              if (isCollected) {
                if (isPrepaidReserve) {
                  badgeText = '✨ شهر احتياطي مدفوع (سداد مسبق)';
                  badgeClass = 'badge-collected';
                  badgeCustomStyle = 'background-color: #d97706; color: #ffffff; font-weight: bold; padding: 2px 6px; border-radius: 4px;';
                } else {
                  badgeText = 'محصل بالكامل';
                  badgeClass = 'badge-collected';
                }
              } else if (revertedColl || d.lastRevertDate || (d.collectionNotes && d.collectionNotes.includes('رجوع'))) {
                badgeText = '⚠️ تم الرجوع عن التحصيل';
                badgeClass = 'badge-overdue';
                badgeCustomStyle = 'background-color: #fef2f2; color: #dc2626; border: 1px solid #fca5a5; font-weight: bold; padding: 2px 6px; border-radius: 4px;';
              } else if (cStat === 'overdue' || (d.forMonthYear && d.forMonthYear < currentMonthISO)) {
                badgeText = 'متأخر';
                badgeClass = 'badge-overdue';
              } else if (d.forMonthYear === currentMonthISO) {
                badgeText = 'مستحق الشهر الحالي';
                badgeClass = 'badge-pending';
              }

              const rowBgStyle = isPrepaidReserve ? 'style="background-color: #FEF3C7; font-weight: bold;"' : (revertedColl ? 'style="background-color: #FFF1F2;"' : '');
              ...
              return `
                <tr ${rowBgStyle}>
                  ...
                  <td class="num-font" style="font-weight: 700;">${d.monthNameAr || d.forMonthYear}${isPrepaidReserve ? ' <span style="color: #b45309; font-size: 7.5pt; font-weight: 800;">(احتياطي)</span>' : ''}</td>
                  <td class="num-font">${d.dueDate || '—'}</td>
        ...
        ` : reportType === 'property_monthly' ? `
          <thead>
            <tr>
              <th style="width: 4%;">#</th>
              <th style="width: 15%;">اسم العقار</th>
              <th style="width: 15%;">اسم المالك</th>
              <th style="width: 8%;">الوحدة</th>
              <th style="width: 16%;">اسم المستأجر</th>
              <th style="width: 8%;">الشهر المالي</th>
              <th style="width: 12%;">الإيجار المستحق (الإيراد الإجمالي)</th>
              <th style="width: 11%;">المبلغ المحصل</th>
              <th style="width: 11%;">المتأخرات</th>
              <th style="width: 10%;">حالة التحصيل</th>
            </tr>
          </thead>
          <tbody>
            ${filteredDues.length === 0 ? `
              <tr>
                <td colspan="10" style="padding: 20px; color: #64748b; font-weight: bold;">
                  لا توجد بيانات إيجارية سجلت لهذا العقار في الشهر المحدد.
                </td>
              </tr>
            ` : filteredDues.map((d, idx) => {
              const cStat = getDueCollectionStatus(d);
              const rentDue = d.rentAmount || 0;
              const collected = cStat === 'collected' ? (d.collectedAmount || d.rentAmount) : 0;
              const arrears = Math.max(0, rentDue - collected);
              ...
              return `
                <tr>
                  <td class="num-font">${idx + 1}</td>
                  <td style="font-weight: 700;">${d.propertyName}</td>
                  <td>${d.ownerName}</td>
            ...
            <tr class="total-row">
              <td colspan="6" style="text-align: right; padding-right: 15px;">
                إجمالي الإيراد الشهري للعقار (بدون خصم عمولات) — [ ${filteredDues.length} وحدة مؤجرة / ${vacantUnitsCount} شاغرة ]
              </td>
              <td class="num-font gold-text">${sumRent.toLocaleString('ar-EG')} ج.م</td>
              <td class="num-font gold-text">${sumCollected.toLocaleString('ar-EG')} ج.م</td>
              <td class="num-font gold-text">${(sumRent - sumCollected).toLocaleString('ar-EG')} ج.م</td>
              <td style="font-size: 8pt; text-align: center;">نسبة التحصيل: ${collectionPercentage}%</td>
            </tr>
          </tbody>
        ` : reportType === 'owner_statement' ? `
          <thead>
            <tr>
              <th style="width: 4%;">#</th>
              <th style="width: 13%;">اسم المالك</th>
              <th style="width: 14%;">اسم العقار (الوحدة)</th>
              <th style="width: 8%;">الشهر المالي</th>
              <th style="width: 10%;">إجمالي المستحق</th>
              <th style="width: 10%;">الإيجار المحصل</th>
              <th style="width: 9%;">عمولة المكتب</th>
              <th style="width: 10%;">صافي مستحق المالك</th>
              <th style="width: 8%;">نوع الصرف</th>
              <th style="width: 8%;">حالة التسوية</th>
              <th style="width: 6%;">تاريخ الصرف</th>
            </tr>
          </thead>
          <tbody>
            ${filteredDues.length === 0 ? `
              <tr>
                <td colspan="11" style="padding: 20px; color: #64748b; font-weight: bold;">
                  لا توجد بيانات مطابقة للفلاتر المحددة لكشف حساب المالك.
                </td>
              </tr>
            ` : filteredDues.map((d, idx) => {
              const pStat = getDuePayoutStatus(d);
              ...
              const isImmediate = ownerPayoutType === 'immediate' || (ownerPayoutType === 'all' && (d as any).payoutType !== 'deferred');
              const rentDue = d.rentAmount || 0;
              const rentCollected = cStat === 'collected' ? (d.collectedAmount || d.rentAmount) : 0;
              const commission = isImmediate ? (d.commissionAmount || 0) : (rentCollected > 0 ? Math.round(rentCollected * ((d.commissionAmount || 0) / (d.rentAmount || 1))) : 0);
              const netOwner = isImmediate ? (d.netOwnerAmount || (rentDue - commission)) : Math.max(0, rentCollected - commission);
              ...
              if (pStat === 'paid_out') {
                ...
                badgeClass = 'badge-collected';
              } else if (pStat === 'pending_payout' && (d as any).disbursedAmount > 0) {
                statusBadge = 'صُرف جزئيًا';
                badgeClass = 'badge-pending';
              }
              ...
              return `
                <tr>
                  ...
                  <td>${d.propertyName} <span style="color: #64748b; font-size: 8pt;">(وحدة ${d.unitNumber})</span></td>
                  <td class="num-font">${d.monthNameAr || d.forMonthYear}</td>
                  <td class="num-font">${rentDue.toLocaleString('ar-EG')} ج.م</td>
                  <td class="num-font" style="color: #059669;">${rentCollected.toLocaleString('ar-EG')} ج.م</td>
                  <td class="num-font" style="color: #d97706;">${commission.toLocaleString('ar-EG')} ج.م</td>
                  <td class="num-font" style="font-weight: 800; color: #b45309;">${netOwner.toLocaleString('ar-EG')} ج.م</td>
                  <td>
                    <span class="badge ${isImmediate ? 'badge-payout' : 'badge-pending'}">
                      ${isImmediate ? 'صرف عاجل' : 'صرف آجل'}
                    </span>
                  </td>
                  <td>
                    <span class="badge ${badgeClass}">${statusBadge}</span>
                  </td>
                  <td class="num-font" style="font-size: 8pt; color: #475569;">${d.payoutDate || d.paidDate || '—'}</td>
                </tr>
              `;
            }).join('')}
            
            <tr class="total-row">
              <td colspan="4" style="text-align: right; padding-right: 15px;">الإجمالي الكلي لكشف الحساب (${filteredDues.length} سجل)</td>
              <td class="num-font gold-text">${sumRent.toLocaleString('ar-EG')} ج.م</td>
              <td class="num-font gold-text">${sumCollected.toLocaleString('ar-EG')} ج.م</td>
              ...
              <td colspan="3" style="font-size: 8pt; text-align: center;">كشف حساب مالك معتمد رسمياً</td>
            </tr>
          </tbody>
        ` : `
          <thead>
            ...
            ` : filteredDues.map(d => {
              ...
              return `
                <tr>
                  ...
                  <td class="num-font" style="color: #d97706;">${d.commissionAmount.toLocaleString('ar-EG')} ج.م</td>
...
export default function RealEstateReportModal({
  ...
  const htmlContent = React.useMemo(() => {
    ...
    isOpen, reportType, dues, collections, owners, properties, units, tenants,
    commissionStatuses, selectedPropertyId, selectedOwnerId, selectedTenantId, selectedMonthYear,
    tenantFromMonth, tenantToMonth, accountType, commissionsFilter, commSearchTerm, currentUser, reportMetadata, ownerPayoutType
  ]);
