...
import { 
  ReRentDue, ReOwner, ReProperty, ReUnit, ReTenant, User, ReCollectionReceipt, ReCommissionStatus, ReOwnerAdvance, RePropertyExpense, ReRentAdjustment
} from '../../types';
import { getDueCollectionStatus, getDuePayoutStatus, calculateCommissionFromSettings, getDueCommissionAmount, getPropertyCommissionSettings, getSectionCommissionForPropertyMonth, calculatePropertyStatementsData, calculateOwnerStatementsData, formatMonthYearAr } from './RealEstateData';
              ...
              return displayTenants.map((t, idx) => {
                ...
                <tr class="total-row">
                  ...
                  <td class="num-font gold-text">${displayTenants.reduce((s, t) => {
                    const tDues = dues.filter(d => d.tenantId === t.id);
          ...
          const renderDeductionsCell = (advancesList: ReOwnerAdvance[] = [], expensesList: RePropertyExpense[] = []) => {
              ...
              items.push(`
                <div style="margin-bottom: 3px; padding: 3px 5px; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 4px; font-size: 7pt; text-align: right; line-height: 1.25;">
                  <div style="display: flex; justify-content: space-between; align-items: center; gap: 4px;">
                    <span style="font-weight: 800; color: #b45309;">سلفة: ${(a.amount || 0).toLocaleString('ar-EG')} ج.م</span>
                  ...
                  <div style="color: #64748b; font-size: 6.5pt; margin-top: 1px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="${a.notes || ''}">${noteText}</div>
                </div>
              `);
            });
            ...
            expensesList.forEach(e => {
              ...
              items.push(`
                <div style="margin-bottom: 3px; padding: 3px 5px; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 4px; font-size: 7pt; text-align: right; line-height: 1.25;">
                  <div style="display: flex; justify-content: space-between; align-items: center; gap: 4px;">
                    <span style="font-weight: 800; color: #0284c7;">مصروف: ${(e.amount || 0).toLocaleString('ar-EG')} ج.م</span>
                    <span style="font-size: 6pt; font-weight: bold; color: ${statusColor}; background: ${statusBg}; padding: 1px 3px; border-radius: 2px; white-space: nowrap;">${statusText}</span>
                  </div>
                    ...
                    ${displayAdvances.map((a, idx) => {
                      ...
                      return `
                        <tr style="font-size: 8.5pt; ${isDeducted ? 'background-color: #fffbfa;' : isReverted ? 'background-color: #fff1f2;' : ''}">
                          ...
                          <td class="num-font" style="font-weight: 800; color: #0f172a;">${(a.amount || 0).toLocaleString('ar-EG')} ج.م</td>
                          <td style="text-align: center;">
                            ${isDeducted ? `
                              <span class="badge badge-overdue" style="display: inline-block; margin-bottom: 2px;">تم الخصم (مخصوم من المستحق)</span>
                              <div class="num-font" style="color: #dc2626; font-size: 7.5pt; font-weight: 800;">-${(a.amount || 0).toLocaleString('ar-EG')} ج.م</div>
                            ` : isReverted ? `
                              ...
                              <div class="num-font" style="color: #059669; font-size: 7.5pt; font-weight: 800;">+${(a.amount || 0).toLocaleString('ar-EG')} ج.م</div>
                            ` : `
...
export default function RealEstateReportModal({
  ...
  const htmlContent = React.useMemo(() => {
    ...
    isOpen, reportType, dues, collections, owners, properties, units, tenants, advances, expenses,
