...
import { getDueCollectionStatus, isDueCollected, calculateCommissionFromSettings, getDueCommissionAmount, getPropertyCommissionSettings, getSectionCommissionForPropertyMonth, calculatePropertyStatementsData, calculateOwnerStatementsData } from './RealEstateData';
...
export default function RealEstateFinancials({
  ...
  const validDues = useMemo(() => {
    ...
    const enrichedDues = activeDues.map(d => {
      ...
      if (cStat === 'collected' || cStat === 'prepaid') {
        const matchingReceipt = collections.find(c => 
          c && c.status !== 'reverted' && !c.isCancelled && (c.amountPaid || 0) > 0 &&
          ((c.dueId && c.dueId === d.id) || (c.tenantId === d.tenantId && c.forMonthYear === d.forMonthYear))
        );
        return {
          ...dueWithAdj,
          status: 'collected' as const,
          collectionStatus: cStat,
          collectedAmount: matchingReceipt?.amountPaid || dueWithAdj.rentAmount,
      ...
      return {
        ...
        collectionStatus: 'overdue' as const,
        collectedAmount: 0,
        paidDate: undefined,
  ...
  const [selectedPropertyCommGroup, setSelectedPropertyCommGroup] = useState<typeof propertyCommGroups[0] | null>(null);
  ...
  const handleDeductAdvance = async (advance: ReOwnerAdvance) => {
    if (advance.isDeducted) {
      alert('⚠️ هذه السلفة مخصومة بالفعل من كشف حساب المالك!');
      return;
    }
    ...
    const alreadyDeductedAdvSum = advances
      .filter(a => a.isDeducted && a.ownerId === targetOwnerId && a.id !== advance.id)
    ...
    if (advance.amount > availableNetEntitlement) {
      alert(
        ...
        `• قيمة السلفة المطلوبة خصمها: ${advance.amount.toLocaleString('ar-EG')} ج.م\n\n` +
        `يرجى التأكد من توفر مستحقات إيجارية كافية للمالك في النظام أولاً قبل إجراء الخصم.`
      );
      return;
    }

    const remainingAfterDeduction = availableNetEntitlement - advance.amount;
    if (!confirm(`هل أنت تأكد من خصم السلفة بقيمة ${advance.amount.toLocaleString('ar-EG')} ج.م من مستحقات المالك (${ownerName})؟\n\nالمستحقات المتاحة للمالك حالياً: ${availableNetEntitlement.toLocaleString('ar-EG')} ج.م\nالمستحقات المتبقية بعد الخصم: ${remainingAfterDeduction.toLocaleString('ar-EG')} ج.م`)) return;
    ...
    try {
      await updateFirestoreDoc('re_advances', advance.id, {
        ...
        deductedAmount: advance.amount
      });

      await addFirestoreDoc('re_logs', {
        actionType: 'payout',
        entityName: 'سلفة مالك',
        details: `تم خصم سلفة بقيمة ${advance.amount} ج.م للمالك (${ownerName}) للعقار (${advance.propertyName || 'عقار'}) عند التسوية المالية (المستحقات المتبقية: ${remainingAfterDeduction} ج.م)`,
        username: currentUser.fullName,
        timestamp: nowStr
      });
    } catch (err) {
      console.error('Error deducting advance:', err);
      alert('حدث خطأ أثناء خصم السلفة');
    }
  };
  ...
  return (
    <div className="space-y-6 text-[#F8F9FB]" dir="rtl">
                          ...
                          <tr key={tenant.id} className="hover:bg-white/[0.04] transition-colors">
                            ...
                            <td className="p-3 text-center">
                              {(() => {
                                ...
                                const targetEditMonth = (!isAllMonths && !isAllYears)
                                  ? `${selectedRentYear}-${selectedRentMonth}`
                                  : currentMonthISO;
                                const matchedDueToEdit = isAllMonths 
                                  ? (currentMonthDue || tenantDues.find(d => d.forMonthYear === currentMonthISO) || tenantDues[0])
                                  : (currentMonthDue || tenantDues.find(d => d.forMonthYear === targetEditMonth));

                                const dueToEdit = matchedDueToEdit || {
                                  id: '',
                                  tenantId: tenant.id,
                                  tenantName: tenant.fullName || '',
          ...
          {advancesSubTab === 'owner_advances' && (() => {
            ...
            return (
              <div className="bg-[#132238]/80 backdrop-blur-md rounded-2xl border border-[#D4A84F]/25 overflow-hidden shadow-2xl">
                <div className="p-4 border-b border-[#D4A84F]/20 flex items-center justify-between bg-[#0B1524]">
                  <div className="flex items-center gap-2.5">
                    <Coins className="w-5 h-5 text-[#D4A84F] stroke-[2.2]" />
                    <h3 className="text-sm font-black text-[#F8F9FB]">جدول سلف الملاك المباشرة</h3>
                  </div>
                  <span className="text-xs text-slate-300 font-extrabold font-mono">{filteredAdvances.length} سجل</span>
                </div>
                    ...
                    <table className="w-full text-right text-xs sm:text-sm">
                      <thead>
                        <tr className="bg-[#08111F] text-slate-200 text-xs font-extrabold border-b border-[#D4A84F]/20">
                          ...
                          <th className="p-3.5 text-center">قيمة السلفة</th>
                          ...
                          <th className="p-3.5 text-center">الإجراءات الخصم والتراجع</th>
                        </tr>
                      </thead>
                              ...
                              <td className="p-3.5 text-center">
                                {adv.isDeducted ? (
                                  <div className="inline-flex flex-col items-center">
                                    <span className="px-3 py-1 rounded-full text-xs font-black bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 flex items-center gap-1 shadow-sm">
                                      <CheckCircle className="w-3.5 h-3.5" /> مخصومة من المستحقات
                                    </span>
                                    {adv.deductedAt && (
                                  ...
                                  <span className="px-3 py-1 rounded-full text-xs font-black bg-amber-500/20 text-amber-300 border border-amber-500/40 flex items-center gap-1 shadow-sm">
                                    <Clock className="w-3.5 h-3.5" /> غير مخصومة (جارية)
                                  </span>
                                )}
                              </td>
                              <td className="p-3 text-center">
                  ...
                  <div className="bg-[#132238]/60 backdrop-blur-md rounded-2xl border border-rose-500/20 overflow-hidden shadow-2xl">
                    <div className="p-4 border-b border-rose-500/20 flex items-center justify-between bg-rose-500/5">
                      ...
                      <span className="text-[10px] text-rose-300/80 font-mono font-bold">
                        إجمالي الخصم: {totalOwnerDeductions.toLocaleString('ar-EG')} ج.م
                      </span>
                    </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="p-3 text-center">
                            <button
                              type="button"
                              onClick={() => setSelectedPropertyCommGroup(group)}
                              className="px-3 py-1.5 rounded-xl bg-gradient-to-r from-amber-500/20 to-amber-600/20 hover:from-amber-500/30 hover:to-amber-600/30 text-[#D4A84F] border border-[#D4A84F]/40 text-xs font-black transition-all cursor-pointer inline-flex items-center justify-center gap-1.5 shadow-sm hover:scale-[1.03] active:scale-95"
              ...
              <table className="w-full text-right text-xs">
                ...
                <tbody className="divide-y divide-[#D4A84F]/10 print:divide-slate-300">
                  {dues.filter(d => {
                    ...
                    if (reportType === 'arrears' && getDueCollectionStatus(d, todayISO, currentMonthISO, collections) !== 'overdue') return false;
                    if (reportType === 'office_advances' && (getDuePayoutStatus(d) !== 'paid_out' || getDueCollectionStatus(d, todayISO, currentMonthISO, collections) === 'collected')) return false;
                    return true;
                  }).map(d => {
                    const comm = getDueCommissionAmount(d, owners, properties);
                    const netOwner = Math.max(0, (d.rentAmount || 0) - comm);
                    ...
                    return (
                      <tr key={d.id} className="print:text-black font-bold hover:bg-white/5 transition-all">
                        ...
                        <td className="p-2.5 font-mono text-center text-amber-400 print:text-slate-800">{comm.toLocaleString('ar-EG')} ج.م</td>
                        <td className="p-2.5 font-mono text-center text-[#D4A84F] print:text-black font-black">{netOwner.toLocaleString('ar-EG')} ج.م</td>
                        <td className="p-2.5 text-center">
                          {cStatus === 'collected' ? (
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4" dir="rtl">
            ...
            >
              ...
              <div className="space-y-3 text-xs font-semibold text-[#1E293B]">
                ...
                <div className="p-3 bg-amber-50 rounded-xl border border-amber-200 text-amber-900 text-xs font-bold leading-relaxed flex items-start gap-2">
                  <span className="text-sm shrink-0">ℹ️</span>
                  <span>
                    هذا التعديل مخصص ومحصور بشهر <strong className="font-mono text-amber-950">({editingRentDue.monthNameAr || editingRentDue.forMonthYear})</strong> فقط لهذا المستأجر، وسينعكس فورياً في كشوفات الحسابات والعمولات دون المساس بباقي الشهور أو بيانات العقد العامة.
                  </span>
                </div>
              </div>
        ...
        {viewDetailCommRecord && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-[#132238] border border-[#D4A84F]/30 rounded-2xl sm:rounded-3xl w-full max-w-3xl max-h-[90vh] flex flex-col shadow-[0_15px_40px_rgba(0,0,0,0.6)] relative overflow-hidden text-[#F8F9FB]"
            >
              {/* Header */}
              <div className="flex items-center justify-between border-b border-[#D4A84F]/20 p-4 sm:p-5 shrink-0 bg-[#08111F]/80 backdrop-blur-md">
                <div className="flex items-center gap-3">
                  <div className="p-2.5 rounded-2xl bg-amber-500/15 border border-[#D4A84F]/30 text-amber-400">
                    <Coins className="w-6 h-6" />
                  </div>
                  <div>
                    <h3 className="text-base sm:text-lg font-black text-[#F8F9FB] flex items-center gap-2">
                      <span>تفاصيل عمولة عقار:</span>
                      <span className="text-[#D4A84F]">{viewDetailCommRecord.propertyName}</span>
                    </h3>
                    <p className="text-xs text-[#9EA7B8] font-bold mt-0.5">
                      المالك: <span className="text-white">{viewDetailCommRecord.ownerName}</span> | الشهر المالي: <span className="text-amber-300 font-mono">{viewDetailCommRecord.forMonthYear}</span>
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <span className={`px-3 py-1 rounded-full text-xs font-bold border ${
                    viewDetailCommRecord.status === 'collected'
                      ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/30'
                      : viewDetailCommRecord.status === 'claimed'
                      ? 'bg-sky-500/20 text-sky-300 border-sky-500/30'
                      : viewDetailCommRecord.status === 'overdue'
                      ? 'bg-rose-500/20 text-rose-300 border-rose-500/30'
                      : 'bg-slate-500/20 text-slate-300 border-slate-500/30'
                  }`}>
                    {viewDetailCommRecord.status === 'collected' ? 'تم التحصيل'
                      : viewDetailCommRecord.status === 'claimed' ? 'تمت المطالبة'
                      : viewDetailCommRecord.status === 'overdue' ? 'متأخرة'
                      : 'لم تتم المطالبة'}
                  </span>

                  <button
                    onClick={() => setViewDetailCommRecord(null)}
                    className="p-1.5 rounded-xl text-[#9EA7B8] hover:text-white hover:bg-white/10 transition-colors cursor-pointer"
                  >
                    <X className="w-5 h-5" />
                  </button>
                </div>
              ...
              <div className="p-4 sm:p-6 overflow-y-auto space-y-6">
                ...
                {/* 1. KPI Cards Grid */}
                <div>
                  <h4 className="text-xs font-black text-[#D4A84F] uppercase tracking-wider mb-3 flex items-center gap-1.5">
                    <PieChart className="w-4 h-4" />
                    <span>ملخص الحسابات المالية والعمولة</span>
                  </h4>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    <div className="p-3 bg-[#08111F]/60 rounded-xl border border-white/10 space-y-1">
                      <span className="text-[10px] text-[#9EA7B8] block font-bold">إجمالي الإيجار المستحق</span>
                      <span className="text-sm font-black text-white font-mono">{viewDetailCommRecord.totalDueRent.toLocaleString('ar-EG')} ج.م</span>
                    </div>

                    <div className="p-3 bg-[#08111F]/60 rounded-xl border border-white/10 space-y-1">
                      <span className="text-[10px] text-[#9EA7B8] block font-bold">الإيجار المحصل فعلياً</span>
                      <span className="text-sm font-black text-teal-300 font-mono">{viewDetailCommRecord.totalCollectedRent.toLocaleString('ar-EG')} ج.م</span>
                    </div>

                    <div className="p-3 bg-[#08111F]/60 rounded-xl border border-white/10 space-y-1">
                      <span className="text-[10px] text-[#9EA7B8] block font-bold">آلية الاتفاق والنسبة</span>
                      <span className="text-xs font-bold text-amber-300">{viewDetailCommRecord.commissionRateText}</span>
                    </div>

                    <div className="p-3 bg-[#08111F]/60 rounded-xl border border-amber-500/30 bg-amber-500/5 space-y-1">
                      <span className="text-[10px] text-amber-300 block font-bold">العمولة المستحقة للمكتب</span>
                      <span className="text-sm font-black text-amber-400 font-mono">{viewDetailCommRecord.earnedCommission.toLocaleString('ar-EG')} ج.م</span>
                    </div>

                    <div className="p-3 bg-[#08111F]/60 rounded-xl border border-white/10 space-y-1">
                      <span className="text-[10px] text-[#9EA7B8] block font-bold">عمولة المحصل من الإيجار</span>
                      <span className="text-sm font-black text-[#9EA7B8] font-mono">{viewDetailCommRecord.collectedRentCommission.toLocaleString('ar-EG')} ج.م</span>
                    </div>

                    <div className="p-3 bg-[#08111F]/60 rounded-xl border border-emerald-500/30 bg-emerald-500/5 space-y-1">
                      <span className="text-[10px] text-emerald-300 block font-bold">المحصل من المالك</span>
                      <span className="text-sm font-black text-emerald-400 font-mono">{viewDetailCommRecord.amountCollectedFromOwner.toLocaleString('ar-EG')} ج.م</span>
                    </div>

                    <div className="p-3 bg-[#08111F]/60 rounded-xl border border-rose-500/30 bg-rose-500/5 space-y-1">
                      <span className="text-[10px] text-rose-300 block font-bold">المتبقي من العمولة</span>
                      <span className="text-sm font-black text-rose-400 font-mono">{viewDetailCommRecord.remainingCommission.toLocaleString('ar-EG')} ج.م</span>
                    </div>

                    <div className="p-3 bg-[#08111F]/60 rounded-xl border border-white/10 space-y-1">
                      <span className="text-[10px] text-[#9EA7B8] block font-bold">الوحدات / المستأجرين</span>
                      <span className="text-xs font-bold text-sky-300">{viewDetailCommRecord.tenantCount} مستأجر</span>
                    </div>
                  </div>
                </div>

                {/* 2. Collection Payment Details Box (if collected or has details) */}
                {(viewDetailCommRecord.amountCollectedFromOwner > 0 || viewDetailCommRecord.status === 'collected' || viewDetailCommRecord.notes) && (
                  <div className="p-4 bg-[#08111F]/90 rounded-2xl border border-[#D4A84F]/20 space-y-3">
                    <h4 className="text-xs font-black text-[#D4A84F] flex items-center gap-1.5">
                      <Receipt className="w-4 h-4 text-emerald-400" />
                      <span>سند وبيانات تحصيل العمولة</span>
                    </h4>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                      <div>
                        <span className="text-[10px] text-[#9EA7B8] block font-bold">تاريخ التحصيل:</span>
                        <span className="font-mono font-bold text-white">{viewDetailCommRecord.collectionDate || 'غير محدد'}</span>
                      </div>
                      <div>
                        <span className="text-[10px] text-[#9EA7B8] block font-bold">طريقة السداد:</span>
                        <span className="font-bold text-teal-300">{viewDetailCommRecord.paymentMethod || 'نقدي'}</span>
                      </div>
                      <div>
                        <span className="text-[10px] text-[#9EA7B8] block font-bold">رقم الإيصال / السند:</span>
                        <span className="font-mono font-bold text-amber-300">{viewDetailCommRecord.referenceNumber || '—'}</span>
                      </div>
                      <div>
                        <span className="text-[10px] text-[#9EA7B8] block font-bold">تاريخ التحديث:</span>
                        <span className="font-mono text-[#9EA7B8]">{viewDetailCommRecord.updatedAt || '—'}</span>
                      </div>
                    </div>
                    {viewDetailCommRecord.notes && (
                      <div className="pt-2 border-t border-white/10 text-xs">
                        <span className="text-[10px] text-[#9EA7B8] block font-bold">ملاحظات التحصيل:</span>
                        <p className="text-slate-300 font-medium mt-0.5 leading-relaxed bg-white/5 p-2 rounded-lg">{viewDetailCommRecord.notes}</p>
                      </div>
                    )}
                  </div>
                )}

                {/* 3. Dues & Units Table Breakdown */}
                <div>
                  <h4 className="text-xs font-black text-[#D4A84F] mb-3 flex items-center gap-1.5">
                    <Users className="w-4 h-4" />
                    <span>بيانات استحقاقات وحدات ومستأجري العقار ({viewDetailCommRecord.dues.length} سجل)</span>
                  </h4>

                  <div className="overflow-x-auto rounded-xl border border-white/10 bg-[#08111F]/50">
                    <table className="w-full text-right text-xs">
                      <thead>
                        <tr className="bg-[#08111F] text-[#9EA7B8] text-[10px] font-bold border-b border-white/10">
                          <th className="p-2.5 text-center">#</th>
                          <th className="p-2.5">المستأجر والوحدة</th>
                          <th className="p-2.5 text-center">الشهر المالي</th>
                          <th className="p-2.5 text-center">القيمة الإيجارية</th>
                          <th className="p-2.5 text-center">المحصل</th>
                          <th className="p-2.5 text-center text-amber-400">عمولة الوحدة</th>
                          <th className="p-2.5 text-center">حالة تحصيل الإيجار</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-white/5 font-bold">
                        {viewDetailCommRecord.dues.length === 0 ? (
                          <tr>
                            <td colSpan={7} className="p-4 text-center text-[#9EA7B8]">
                              لا توجد استحقاقات تفصيلية مسجلة لهذا العقار.
                            </td>
                          </tr>
                        ) : (
                          viewDetailCommRecord.dues.map((d, dIdx) => {
                            const dueCollected = isDueCollected(d);
                            return (
                              <tr key={d.id || dIdx} className="hover:bg-white/[0.02]">
                                <td className="p-2.5 text-center text-[#9EA7B8] font-mono text-[11px]">{dIdx + 1}</td>
                                <td className="p-2.5">
                                  <span className="block text-white font-extrabold">{d.tenantName || 'مستأجر'}</span>
                                  {d.unitName && <span className="text-[10px] text-amber-300 font-mono block">وحدة: {d.unitName}</span>}
                                </td>
                                <td className="p-2.5 text-center font-mono text-amber-400">{d.forMonthYear}</td>
                                <td className="p-2.5 text-center font-mono text-white">{(d.rentAmount || 0).toLocaleString('ar-EG')} ج.م</td>
                                <td className="p-2.5 text-center font-mono text-teal-300">
                                  {dueCollected ? (d.collectedAmount || d.rentAmount || 0).toLocaleString('ar-EG') : 0} ج.م
                                </td>
                                <td className="p-2.5 text-center font-mono text-amber-400 font-black">
                                  {getDueCommissionAmount(d, owners, properties).toLocaleString('ar-EG')} ج.م
                                </td>
                                <td className="p-2.5 text-center">
                                  {dueCollected ? (
                                    <span className="px-2 py-0.5 rounded text-[10px] bg-emerald-500/20 text-emerald-300">محصل</span>
                                  ) : (
                                    <span className="px-2 py-0.5 rounded text-[10px] bg-rose-500/20 text-rose-300">غير محصل</span>
                                  )}
                                </td>
                              </tr>
                            );
              ...
              {/* Footer Actions */}
              <div className="flex items-center justify-between p-4 sm:p-5 border-t border-[#D4A84F]/20 bg-[#08111F]/80 shrink-0 gap-3 flex-wrap">
                <div className="flex items-center gap-2">
                  {viewDetailCommRecord.status !== 'collected' && viewDetailCommRecord.amountCollectedFromOwner === 0 && (
                    <button
                      type="button"
                      onClick={() => {
                        const rec = viewDetailCommRecord;
                        setViewDetailCommRecord(null);
                        handleOpenEditCommStatusModal(rec);
                      }}
                      className="px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-black text-xs rounded-xl hover:brightness-110 transition-all cursor-pointer shadow-md flex items-center gap-1.5"
                    >
                      <Coins className="w-4 h-4 text-amber-300" />
                      <span>تحصيل عمولة هذا العقار</span>
                    </button>
                  )}
                  ...
                  {(viewDetailCommRecord.status === 'collected' || viewDetailCommRecord.amountCollectedFromOwner > 0) && (
                    <button
                      ...
                      onClick={() => {
                        const rec = viewDetailCommRecord;
                        setViewDetailCommRecord(null);
                        handleRevertCommissionStatus(rec);
                      }}
                      className="px-3.5 py-2 bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 border border-rose-500/40 text-xs font-bold rounded-xl transition-all cursor-pointer flex items-center gap-1.5"
                    >
                      <RotateCcw className="w-3.5 h-3.5" />
                      <span>الرجوع في التحصيل</span>
                    </button>
                ...
                <button
                  type="button"
                  onClick={() => setViewDetailCommRecord(null)}
                  className="px-5 py-2 bg-white/10 hover:bg-white/20 text-white font-bold text-xs rounded-xl transition-all cursor-pointer mr-auto"
                >
                  إغلاق Window
                </button>
              </div>
            ...
            >
              ...
              {(() => {
                ...
                const rawUncollected = arrearsModalData.uncollectedDues.filter(d => {
                  const status = getDueCollectionStatus(d, todayISO, currentMonthISO, collections);
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3 p-3 bg-slate-100/90 rounded-2xl border border-slate-300">
                      ...
                      {arrearsModalTab === 'prepayment' && (() => {
                        ...
                        const tenantPrepaidCount = tObj ? validDues.filter(d => d.tenantId === tObj.id && d.forMonthYear && d.forMonthYear > currentMonthISO && (getDueCollectionStatus(d, todayISO, currentMonthISO, collections) === 'prepaid' || getDueCollectionStatus(d, todayISO, currentMonthISO, collections) === 'collected')).length : 0;
      ...
      {/* PROPERTY COMMISSION MONTHLY BREAKDOWN MODAL (تفاصيل تحصيل عمولة العقار) */}
      <AnimatePresence>
        {selectedPropertyCommGroup && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[120] bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-3 sm:p-5 overflow-hidden"
            dir="rtl"
          >
            <motion.div
              ...
              className="bg-[#0B1524] border-2 border-[#D4A84F]/40 rounded-2xl sm:rounded-3xl w-full max-w-5xl max-h-[92vh] flex flex-col shadow-[0_20px_50px_rgba(0,0,0,0.8)] relative overflow-hidden text-[#F8F9FB]"
            >
              {/* Header */}
              <div className="flex items-center justify-between border-b border-[#D4A84F]/30 p-4 sm:p-5 shrink-0 bg-gradient-to-r from-[#08111F] via-[#0D1B2D] to-[#0B1524] backdrop-blur-md">
                <div className="flex items-center gap-3.5">
                  ...
                  <div>
                    <h3 className="text-lg sm:text-xl font-black text-[#F8F9FB] flex flex-wrap items-center gap-2.5">
                      <span>تفاصيل تحصيل عمولة عقار:</span>
                      <span className="text-[#D4A84F] bg-[#D4A84F]/10 px-3 py-1 rounded-xl border border-[#D4A84F]/30">
                        {selectedPropertyCommGroup.propertyName}
                      </span>
                    </h3>
                    <div className="text-xs sm:text-sm text-slate-200 font-extrabold mt-1.5 flex flex-wrap items-center gap-3">
                      <span>
                        مالك العقار: <span className="text-white font-black">{selectedPropertyCommGroup.ownerName}</span>
                      </span>
                      <span className="text-[#D4A84F]/40">|</span>
                      <span>
                        آلية العقد: <span className="text-amber-300 font-black bg-amber-500/20 px-2.5 py-0.5 rounded-lg border border-amber-500/40">{selectedPropertyCommGroup.commissionRateText}</span>
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <button
                  onClick={() => setSelectedPropertyCommGroup(null)}
              ...
              {/* Scrollable Content */}
              <div className="p-4 sm:p-6 overflow-y-auto space-y-6 bg-[#0B1524]/90">
                
                {/* Overall Property Financials Banner (4 Cards) */}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
                  
                  {/* Card 1: Total Rent */}
                  <div className="p-4 bg-gradient-to-br from-[#08111F] to-[#0A1320] rounded-2xl border border-sky-500/35 space-y-1.5 shadow-lg">
                    <span className="text-xs sm:text-sm text-sky-200 font-black block">إجمالي الإيجار المستحق</span>
                    <div className="text-lg sm:text-2xl font-black text-white font-mono">
                      {selectedPropertyCommGroup.totalDueRent.toLocaleString('ar-EG')} <span className="text-xs font-sans text-sky-300 font-bold">ج.م</span>
                    </div>
                  </div>

                  {/* Card 2: Earned Commission */}
                  <div className="p-4 bg-gradient-to-br from-[#08111F] to-[#0A1320] rounded-2xl border border-amber-500/40 space-y-1.5 shadow-lg">
                    <span className="text-xs sm:text-sm text-amber-200 font-black block">إجمالي العمولة المستحقة</span>
                    <div className="text-lg sm:text-2xl font-black text-amber-300 font-mono">
                      {selectedPropertyCommGroup.earnedCommission.toLocaleString('ar-EG')} <span className="text-xs font-sans text-amber-300 font-bold">ج.م</span>
                    </div>
                  </div>

                  {/* Card 3: Collected from Owner */}
                  <div className="p-4 bg-gradient-to-br from-[#08111F] to-[#0A1320] rounded-2xl border border-emerald-500/40 space-y-1.5 shadow-lg">
                    <span className="text-xs sm:text-sm text-emerald-200 font-black block">إجمالي المحصل من المالك</span>
                    <div className="text-lg sm:text-2xl font-black text-emerald-300 font-mono">
                      {selectedPropertyCommGroup.amountCollectedFromOwner.toLocaleString('ar-EG')} <span className="text-xs font-sans text-emerald-300 font-bold">ج.م</span>
                    </div>
                  </div>

                  {/* Card 4: Remaining Commission */}
                  <div className="p-4 bg-gradient-to-br from-[#08111F] to-[#0A1320] rounded-2xl border border-rose-500/40 space-y-1.5 shadow-lg">
                    <span className="text-xs sm:text-sm text-rose-200 font-black block">إجمالي المتبقي من العمولة</span>
                    <div className="text-lg sm:text-2xl font-black text-rose-300 font-mono">
                      {selectedPropertyCommGroup.remainingCommission.toLocaleString('ar-EG')} <span className="text-xs font-sans text-rose-300 font-bold">ج.م</span>
                    </div>
                  </div>
                ...
                <div className="space-y-3.5">
                  <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[#D4A84F]/20 pb-2.5">
                    <h4 className="text-sm sm:text-base font-black text-[#D4A84F] flex items-center gap-2.5">
                      <Calendar className="w-5 h-5 text-[#D4A84F] stroke-[2.2]" />
                      <span>جدول تحصيل العمولة موزعة حسب الشهور المالية ({selectedPropertyCommGroup.statements.length} شهر)</span>
                    </h4>
                    <span className="text-xs text-slate-300 font-extrabold bg-[#08111F] px-3 py-1 rounded-xl border border-[#D4A84F]/30">
                      بيانات دقيقة وعالية التباين
                    </span>
                  </div>
                  ...
                  <div className="overflow-x-auto rounded-2xl border border-[#D4A84F]/30 bg-[#08111F]/90 shadow-xl">
                    <table className="w-full text-right text-xs sm:text-sm">
                      <thead>
                        <tr className="bg-[#0B1524] text-slate-100 text-xs sm:text-sm font-black border-b border-[#D4A84F]/30">
                          <th className="p-3.5 sm:p-4 text-center">#</th>
                          <th className="p-3.5 sm:p-4 text-center">الشهر المالي</th>
                          <th className="p-3.5 sm:p-4 text-center">إجمالي الإيجار</th>
                          <th className="p-3.5 sm:p-4 text-center text-amber-300">قيمة العمولة</th>
                          <th className="p-3.5 sm:p-4 text-center text-emerald-300">المحصل من المالك</th>
                          <th className="p-3.5 sm:p-4 text-center text-rose-300">المتبقي</th>
                          <th className="p-3.5 sm:p-4 text-center">حالة التحصيل</th>
                          <th className="p-3.5 sm:p-4 text-center">الإجراءات</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-[#D4A84F]/20 font-black text-slate-100">
                        {selectedPropertyCommGroup.statements.map((stmt, sIdx) => {
                          return (
                            <tr key={stmt.id} className="hover:bg-white/[0.05] transition-colors">
                              <td className="p-3.5 sm:p-4 text-center font-mono text-xs sm:text-sm text-slate-300 font-bold">{sIdx + 1}</td>
                              <td className="p-3.5 sm:p-4 text-center">
                                <span className="px-3 py-1 rounded-xl bg-[#132238] text-amber-300 font-mono font-black text-xs sm:text-sm border border-[#D4A84F]/30 shadow-inner">
                                  {formatMonthYearAr(stmt.forMonthYear)}
                                </span>
                              </td>
                              <td className="p-3.5 sm:p-4 text-center font-mono text-xs sm:text-sm text-slate-100 font-extrabold">
                                {stmt.totalDueRent.toLocaleString('ar-EG')} ج.م
                              </td>
                              <td className="p-3.5 sm:p-4 text-center font-mono text-sm sm:text-base text-amber-300 font-black">
                                {stmt.earnedCommission.toLocaleString('ar-EG')} ج.م
                              </td>
                              <td className="p-3.5 sm:p-4 text-center font-mono text-sm sm:text-base text-emerald-300 font-black">
                                {stmt.amountCollectedFromOwner.toLocaleString('ar-EG')} ج.م
                              </td>
                              <td className={`p-3.5 sm:p-4 text-center font-mono text-sm sm:text-base font-black ${stmt.remainingCommission > 0 ? 'text-rose-300' : 'text-emerald-300'}`}>
                                {stmt.remainingCommission.toLocaleString('ar-EG')} ج.م
                              </td>
                              <td className="p-3.5 sm:p-4 text-center">
                                {stmt.status === 'collected' ? (
                                  <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-black bg-emerald-500/25 text-emerald-200 border border-emerald-500/50 shadow-sm">
                                    <CheckCircle className="w-4 h-4 text-emerald-400" />
                                    <span>تم التحصيل بالكامل</span>
                                  </span>
                                ) : stmt.status === 'claimed' ? (
                                  <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-black bg-sky-500/25 text-sky-200 border border-sky-500/50 shadow-sm">
                                    <Clock className="w-4 h-4 text-sky-400" />
                                    <span>تمت المطالبة</span>
                                  </span>
                                ) : stmt.status === 'overdue' ? (
                                  <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-black bg-rose-500/25 text-rose-200 border border-rose-500/50 shadow-sm">
                                    <AlertTriangle className="w-4 h-4 text-rose-400" />
                                    <span>متأخرة</span>
                                  </span>
                                ) : (
                                  <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-black bg-amber-500/25 text-amber-200 border border-amber-500/50 shadow-sm">
                              ...
                              <td className="p-3.5 sm:p-4 text-center">
                                <div className="flex items-center justify-center gap-2 flex-wrap">
                                  {stmt.status === 'collected' || stmt.amountCollectedFromOwner > 0 ? (
                                    <button
                                      type="button"
                                      onClick={() => handleRevertCommissionStatus(stmt)}
                                      className="px-3 py-1.5 rounded-xl bg-rose-500/20 hover:bg-rose-500/35 text-rose-200 border border-rose-500/40 text-xs font-black transition-all cursor-pointer flex items-center gap-1.5 shadow-sm"
                                      title="الرجوع في تحصيل العمولة"
                                    >
                                    ...
                                    <button
                                      type="button"
                                      onClick={() => handleOpenEditCommStatusModal(stmt)}
                                      className="px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-black text-xs shadow-md border border-emerald-400/40 transition-all cursor-pointer flex items-center gap-1.5"
                                      title="تسجيل تحصيل عمولة هذا الشهر"
                                    >
              ...
              {/* Footer Actions */}
              <div className="flex items-center justify-end p-4 sm:p-5 border-t border-[#D4A84F]/30 bg-[#08111F] shrink-0">
                <button
                  type="button"
                  onClick={() => setSelectedPropertyCommGroup(null)}
                  className="px-6 py-2.5 bg-slate-800 hover:bg-slate-700 text-white font-black text-sm rounded-xl border border-slate-600 transition-all cursor-pointer shadow-md hover:scale-[1.02]"
                >
                  إغلاق النافذة
                </button>
              </div>
      ...
      {/* OFFICIAL REPORT PREVIEW & PRINT MODAL */}
      <RealEstateReportModal
        isOpen={isReportModalOpen}
        onClose={() => setIsReportModalOpen(false)}
        reportType={modalReportType}
        ...
        tenantFromMonth={tenantFromMonth}
        tenantToMonth={tenantToMonth}
        ownerStatementsFilter={propertyStatementsFilter}
        ownerAccountType={propertyAccountType}
        accountType={commAccountType}
        commissionsFilter={commissionsFilter}
        commSearchTerm={commSearchTerm}
        commStatements={filteredCommStatements}
      ...
      {/* TENANT COLLECTION RECEIPTS MODAL (سندات التحصيل الخاصة بالمستأجر) */}
