...
import PropertyCommissionReceiptsModal from './PropertyCommissionReceiptsModal';
...
export default function RealEstateFinancials({
  ...
  const formatMonthYearArabicText = (myStr: string): string => {
    if (!myStr || myStr === 'all') return 'جميع الفترات';
    const parts = myStr.split('-');
    if (parts.length === 2) {
      const y = parts[0];
      const m = parts[1];
      const found = AR_MONTHS.find(item => item.value === m);
      const mLabel = found ? found.label.replace(/^[0-9]+\s*-\s*/, '') : m;
      return `${mLabel} ${y}`;
    }
    return myStr;
  };
  ...
  const commissionStatements = useMemo(() => {
    ...
    properties.forEach(prop => {
      ...
      if (commAccountType === 'monthly') {
        ...
        const statusObj = (commissionStatuses || []).find(cs => cs.propertyId === prop.id && (cs.forMonthYear === 'all' || cs.forMonthYear === 'حساب إجمالي شامل' || cs.id === statusId));
  ...
  const [selectedPropertyForCommReceipts, setSelectedPropertyForCommReceipts] = useState<{
    id: string;
    name: string;
    ownerId?: string;
    ownerName?: string;
  } | null>(null);
  const [isCommReceiptsModalOpen, setIsCommReceiptsModalOpen] = useState<boolean>(false);

  const handleOpenCommissionReceipts = (groupOrProperty: {
    propertyId?: string;
    id?: string;
    propertyName?: string;
    name?: string;
    ...
    const actualPropId = groupOrProperty.propertyId || groupOrProperty.id || '';
    const propName = groupOrProperty.propertyName || groupOrProperty.name || '';
    const propObj = properties.find(p => p.id === actualPropId);
    const ownerObj = owners.find(o => o.id === (groupOrProperty.ownerId || propObj?.ownerId));

    setSelectedPropertyForCommReceipts({
      id: actualPropId,
      name: propName || propObj?.name || 'عقار',
      ownerId: groupOrProperty.ownerId || propObj?.ownerId,
      ownerName: groupOrProperty.ownerName || ownerObj?.name || 'المالك'
    });
    setIsCommReceiptsModalOpen(true);
  };
    ...
    <div className="space-y-6 text-[#F8F9FB]" dir="rtl">
                ...
                <div className="flex items-center gap-3.5">
                  ...
                  <div>
                    <div className="flex items-center gap-2.5">
                      <h4 className="text-base font-black text-[#F8F9FB]">{activePropObj?.name || 'عقار'}</h4>
              ...
              <table className="w-full text-right text-xs">
                ...
                <tbody className="divide-y divide-[#D4A84F]/10 font-bold">
                  {propertyBalances.map(pb => (
                    <tr key={pb.property?.id || Math.random()} className="hover:bg-[#08111F]/40 transition-all">
                      <td className="p-3">
                        <div className="font-extrabold text-[#F8F9FB]">{pb.property?.name || 'عقار غير محدد'}</div>
              ...
              <table className="w-full text-right text-xs">
                ...
                <tbody className="divide-y divide-[#D4A84F]/10 font-bold">
                  {ownerBalances.map(ob => (
                    <tr key={ob.owner?.id || Math.random()} className="hover:bg-[#08111F]/40 transition-all">
                      <td className="p-3 font-extrabold text-[#F8F9FB]">{ob.owner?.name || 'مالك غير محدد'}</td>
                      <td className="p-3 text-[#9EA7B8]">
                        {ob.owner?.commissionType === 'percentage' ? `${ob.owner?.commissionValue || 0}% نسبة` : `${ob.owner?.commissionValue || 0} ج.م`}
                      </td>
                ...
                {currentOwnerObj && (
                  <div className="p-4 bg-[#132238]/80 backdrop-blur-md rounded-2xl border border-[#D4A84F]/30 grid grid-cols-1 md:grid-cols-5 gap-4">
                    <div className="space-y-1">
                      ...
                      <h3 className="text-sm font-black text-[#F8F9FB]">{currentOwnerObj?.name || 'مالك'}</h3>
      ...
      {currentTab === 'commissions' && (
        <div className="space-y-5">
          ...
          {/* Commissions Section: Monthly View or Grouped by Property */}
          <div className="bg-[#132238]/80 backdrop-blur-md rounded-2xl border border-[#D4A84F]/30 p-4 sm:p-5 space-y-4 shadow-2xl">
            {/* Commissions Header */}
            <div className="border-b border-[#D4A84F]/20 pb-3.5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-[#08111F]/70 p-3.5 rounded-2xl">
              <div className="flex items-center gap-2.5">
                <div className="p-2 rounded-xl bg-gradient-to-br from-emerald-600/30 to-amber-500/20 text-[#D4A84F] border border-[#D4A84F]/40">
                  <Coins className="w-5 h-5 text-amber-300" />
                </div>
                <div>
                  <h3 className="text-sm sm:text-base font-black text-white flex items-center gap-2">
                    <span>كشف عمولات الإدارة العقارية</span>
                    <span className="px-2.5 py-0.5 rounded-lg bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 text-xs font-black">
                      عرض عمولات العقارات ({propertyCommGroups.length} عقار)
                    </span>
                  </h3>
                  <p className="text-xs text-slate-300 font-bold mt-0.5">
                    عرض إجمالي العمولات المستحقة والمحصلة والمتبقية لكل عقار، مع إمكانية تحصيل العمولة مباشرة وتحديث حالتها في قاعدة البيانات.
                  </p>
                </div>
              </div>
            </div>
            ...
            {propertyCommGroups.length === 0 ? (
                  <div className="p-8 text-center text-sm text-slate-300 font-bold bg-[#08111F]/70 rounded-2xl border border-white/10">
                    لا توجد سجلات عمولات مطابقة للفلاتر المحددة.
                  </div>
                ) : (
                  <>
                    <div className="hidden md:block overflow-x-auto rounded-2xl border-2 border-white/15 bg-[#070E18]">
                    <table className="w-full text-right text-xs">
                      <thead className="bg-[#101E31] text-slate-200 font-black text-[11px] border-b-2 border-white/15">
                        <tr>
                          <th className="p-3 text-center w-10">#</th>
                          ...
                          <th className="p-3 text-center text-amber-300 font-black">العمولة المستحقة</th>
                          <th className="p-3 text-center text-emerald-400 font-black">المحصل من المالك</th>
                          <th className="p-3 text-center text-rose-400 font-black">العمولة المتبقية</th>
                          <th className="p-3 text-center">حالة التحصيل</th>
                          <th className="p-3 text-center w-48">الإجراءات</th>
                        </tr>
                      </thead>
                        ...
                        {propertyCommGroups.map((group, idx) => {
                          ...
                          return (
                            <tr 
                              ...
                              className="hover:bg-white/[0.04] transition-colors bg-[#060D17]"
                            >
                              <td className="p-3 text-center font-mono text-[11px] text-slate-400">{idx + 1}</td>
                              <td className="p-3">
                                <div className="font-black text-white flex items-center gap-1.5 text-xs sm:text-sm">
                                  <Building2 className="w-4 h-4 text-[#D4A84F]" />
                                  <span>{group.propertyName}</span>
                                </div>
                                <span className="text-[11px] text-slate-300 block mt-0.5">المالك: {group.ownerName}</span>
                              </td>
                              <td className="p-3 text-center text-[11px]">
                                <span className="bg-[#132238] text-amber-300 px-2 py-0.5 rounded-md border border-white/15 font-sans">
                                  {group.commissionRateText}
                                </span>
                              </td>
                              <td className="p-3 text-center font-mono text-slate-300">
                                <span className="px-2.5 py-0.5 rounded-lg bg-[#132238] border border-white/15 text-xs text-white">
                                  {group.monthsCount} شهر
                                </span>
                              </td>
                              <td className="p-3 text-center font-mono text-slate-200">{group.totalDueRent.toLocaleString('ar-EG')} ج.م</td>
                              <td className="p-3 text-center font-mono text-amber-300 font-black text-sm">{group.earnedCommission.toLocaleString('ar-EG')} ج.م</td>
                              ...
                              <td className="p-3 text-center">
                                {isFullyCollected ? (
                                  <span className="px-2.5 py-1 rounded-full text-[10px] font-black bg-emerald-500/20 text-emerald-300 border border-emerald-500/50 inline-flex items-center gap-1">
                                    <CheckCircle className="w-3 h-3 text-emerald-400" /> تم التحصيل بالكامل
                                  </span>
                                ) : isPartiallyCollected ? (
                                  <span className="px-2.5 py-1 rounded-full text-[10px] font-black bg-amber-500/20 text-amber-300 border border-amber-500/50 inline-flex items-center gap-1">
                                    <Clock className="w-3 h-3 text-amber-400" /> تحصيل جزئي
                                  </span>
                                ) : (
                                  <span className="px-2.5 py-1 rounded-full text-[10px] font-black bg-rose-500/20 text-rose-300 border border-rose-500/50 inline-flex items-center gap-1">
                                ...
                                <div className="flex items-center justify-center gap-1.5 flex-wrap">
                                  {group.remainingCommission > 0 ? (
                                    <button
                                      ...
                                      className="px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-black text-xs shadow-md border-2 border-emerald-400/60 transition-all cursor-pointer inline-flex items-center justify-center gap-1.5 hover:scale-[1.02] active:scale-95"
                                      title="تحصيل العمولات الشهرية لهذا العقار"
                                    >
                                      <Coins className="w-3.5 h-3.5 text-amber-300" />
                                      <span>تحصيل العمولة</span>
                                    </button>
                                  ) : (
                                    <span className="px-2.5 py-1.5 rounded-xl bg-emerald-950/80 text-emerald-300 border border-emerald-500/50 text-[11px] font-black inline-flex items-center gap-1">
                                      <CheckCircle className="w-3.5 h-3.5 text-emerald-400" />
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => handleOpenCommissionReceipts(group)}
                                    className="px-2.5 py-1.5 rounded-xl bg-purple-500/20 hover:bg-purple-500/30 text-purple-300 border border-purple-500/40 text-xs font-black transition-all cursor-pointer inline-flex items-center justify-center gap-1 shadow-sm hover:scale-[1.02] active:scale-95"
                                    title="عرض جميع سندات تحصيل العمولات الخاصة بهذا العقار"
                                  >
                ...
                {/* Mobile Cards for Property Commissions */}
                <div className="md:hidden space-y-3 mt-3">
                  {propertyCommGroups.map((group, idx) => {
                    ...
                    return (
                      <div
                        key={group.propertyId}
                        className="bg-[#070E18] p-4 rounded-2xl border border-white/15 space-y-3"
                      >
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            <div className="flex items-center gap-2">
                              <span className="w-5 h-5 rounded-full bg-white/10 text-slate-300 text-xs font-mono font-bold flex items-center justify-center">
                                {idx + 1}
                              </span>
                              <h4 className="text-sm font-black text-white">{group.propertyName}</h4>
                            </div>
                            <span className="text-xs text-slate-400 block mt-1">المالك: {group.ownerName}</span>
                          </div>
                          <div>
                            {isFullyCollected ? (
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs bg-white/5 p-2.5 rounded-xl">
                          <div>
                            <span className="text-[10px] text-slate-400 block">آلية العمولة</span>
                            <span className="text-amber-300 font-bold">{group.commissionRateText}</span>
                          </div>
                          <div>
                            <span className="text-[10px] text-slate-400 block">الشهور المسجلة</span>
                            <span className="text-white font-mono font-bold">{group.monthsCount} شهر</span>
                          </div>
                          <div>
                            <span className="text-[10px] text-slate-400 block">العمولة المستحقة</span>
                            <span className="text-amber-300 font-mono font-black">{group.earnedCommission.toLocaleString('ar-EG')} ج.م</span>
                          </div>
                          <div>
                            <span className="text-[10px] text-slate-400 block">المحصل من المالك</span>
                            <span className="text-emerald-400 font-mono font-black">{group.amountCollectedFromOwner.toLocaleString('ar-EG')} ج.م</span>
                          </div>
                          <div className="col-span-2 pt-1 border-t border-white/10 flex justify-between items-center">
                            <span className="text-xs font-bold text-slate-300">المتبقي للتحصيل:</span>
                            <span className={group.remainingCommission > 0 ? "font-mono font-black text-sm text-rose-400" : "font-mono font-black text-sm text-emerald-400"}>
                              {group.remainingCommission.toLocaleString('ar-EG')} ج.م
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2 pt-1 flex-wrap">
                          {group.remainingCommission > 0 ? (
                            <button
                              ...
                              className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-black text-xs shadow-md border-2 border-emerald-400/60 transition-all cursor-pointer flex items-center justify-center gap-1.5 min-w-[110px]"
                            >
                              <Coins className="w-4 h-4 text-amber-300" />
                              <span>تحصيل العمولة</span>
                            </button>
                          ) : (
                            <div className="flex-1 py-2 rounded-xl bg-emerald-950/80 text-emerald-300 border border-emerald-500/50 text-xs font-black text-center flex items-center justify-center gap-1 min-w-[110px]">
                              <CheckCircle className="w-3.5 h-3.5 text-emerald-400" />
                              <span>محصلة بالكامل</span>
                            </div>
                          )}
                          ...
                          <button
                            type="button"
                            onClick={() => handleOpenCommissionReceipts(group)}
                            className="px-3 py-2 rounded-xl bg-purple-500/20 hover:bg-purple-500/30 text-purple-300 border border-purple-500/40 text-xs font-black transition-all cursor-pointer flex items-center justify-center gap-1 shadow-sm"
                            title="عرض سندات تحصيل العمولات الخاصة بهذا العقار"
                          >
                            <Receipt className="w-3.5 h-3.5 text-purple-400" />
                            <span>سندات التحصيل</span>
                          </button>
                          ...
                          <button
                            type="button"
                            onClick={() => handleOpenCommissionDetails(group)}
                            className="px-3 py-2 rounded-xl bg-amber-500/20 hover:bg-amber-500/30 text-[#D4A84F] border border-[#D4A84F]/40 text-xs font-black transition-all cursor-pointer flex items-center justify-center gap-1"
                          >
                            <Eye className="w-3.5 h-3.5 text-amber-400" />
                            <span>التفاصيل</span>
                          </button>
      ...
      {selectedDetailsGroup && (
        <PropertyCommissionDetailsModal
          ...
          onOpenReceipts={handleOpenCommissionReceipts}
          onPrint={(type) => {
      ...
      {/* Property Commission Receipts Modal */}
      {isCommReceiptsModalOpen && selectedPropertyForCommReceipts && (
        <PropertyCommissionReceiptsModal
          isOpen={isCommReceiptsModalOpen}
          onClose={() => {
            setIsCommReceiptsModalOpen(false);
            setSelectedPropertyForCommReceipts(null);
          }}
          property={selectedPropertyForCommReceipts}
          commissionStatuses={commissionStatuses}
          currentUser={currentUser}
          onOpenCollect={() => {
            const group = propertyCommGroups.find(g => g.propertyId === selectedPropertyForCommReceipts.id);
            if (group) {
              handleOpenCollectCommission(undefined, group);
            }
          }}
      ...
      {/* Edit Month Rent Modal (تعديل القيمة الإيجارية للشهر وتحديث المستحقات سحابياً) */}
      {editingRentDue && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#132238] border border-[#D4A84F]/40 rounded-2xl max-w-lg w-full p-6 space-y-5 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            {/* Header */}
            <div className="flex items-center justify-between border-b border-slate-700/60 pb-3">
              <div className="flex items-center gap-2.5">
                <div className="w-10 h-10 rounded-xl bg-[#D4A84F]/15 border border-[#D4A84F]/30 flex items-center justify-center text-[#D4A84F]">
                  <Edit2 className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="text-base font-black text-[#F8F9FB]">
                    تعديل القيمة الإيجارية لشهر ({editingRentDue.monthNameAr || formatMonthYearAr(editingRentDue.forMonthYear)})
                  </h3>
                  <p className="text-xs text-[#9EA7B8] font-bold mt-0.5">
                    المستأجر: {editingRentDue.tenantName || 'غير محدد'} • الوحدة: {editingRentDue.unitNumber || '—'}
                  </p>
                </div>
              </div>
              <button
            ...
            {/* Info Cards */}
            <div className="grid grid-cols-2 gap-3 text-xs bg-[#08111F]/70 p-3.5 rounded-xl border border-slate-700/50">
              <div>
                <span className="text-slate-400 block text-[11px]">القيمة الإيجارية الأصلية</span>
                <span className="font-mono font-black text-sm text-[#F8F9FB]">
                  {(editingRentDue.rentAmount || 0).toLocaleString('ar-EG')} ج.م
                </span>
              </div>
              <div>
                <span className="text-slate-400 block text-[11px]">الشهر المالي المستهدف</span>
                <span className="font-mono font-bold text-sm text-amber-300">
                  {editingRentDue.forMonthYear}
                </span>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-300 flex items-center justify-between">
                <span>القيمة الإيجارية الجديدة المستحقة لهذا الشهر (ج.م) *</span>
                <span className="text-[11px] text-amber-400">ستنعكس فوراً على كشف الحساب والإيجارات والتحصيل</span>
              </label>
              <div className="relative">
                <input
                  type="number"
                  ...
                  step="1"
                  value={newRentAmount}
                  onChange={(e) => setNewRentAmount(Number(e.target.value) || 0)}
                  className="w-full bg-[#08111F] border border-[#D4A84F]/40 focus:border-[#D4A84F] rounded-xl px-4 py-3 text-base font-mono font-black text-white outline-none text-left"
                  placeholder="0"
                />
            ...
            {/* Contract update checkbox */}
            <label className="flex items-center gap-2.5 p-3 rounded-xl bg-slate-800/40 border border-slate-700/40 cursor-pointer hover:bg-slate-800/60 transition-colors">
              <input
                type="checkbox"
                checked={updateTenantContractRent}
                onChange={(e) => setUpdateTenantContractRent(e.target.checked)}
                className="w-4 h-4 rounded text-[#D4A84F] focus:ring-[#D4A84F] bg-slate-900 border-slate-700"
              />
              <span className="text-xs font-bold text-slate-200">
                تحديث القيمة الإيجارية في عقد المستأجر أيضاً (ليصبح الإيجار الشهري الافتراضي {newRentAmount.toLocaleString('ar-EG')} ج.م)
              </span>
            </label>

            {/* Live Commission & Owner Net Preview */}
            {(() => {
              const rentVal = Number(newRentAmount) || 0;
              const propObj = properties.find(p => p.id === editingRentDue.propertyId);
              const ownerObj = owners.find(o => o.id === (editingRentDue.ownerId || propObj?.ownerId));
              ...
              return (
                <div className="grid grid-cols-2 gap-3 text-xs bg-emerald-500/[0.05] p-3 rounded-xl border border-emerald-500/20">
                  <div>
                    <span className="text-slate-400 block text-[11px]">عمولة المكتب المحتسبة</span>
                    <span className="font-mono font-bold text-amber-300">
                      {commAmt.toLocaleString('ar-EG')} ج.م
                    </span>
                  </div>
                  <div>
                    <span className="text-slate-400 block text-[11px]">صافي مستحق المالك</span>
                    <span className="font-mono font-bold text-emerald-400">
                      {netOwner.toLocaleString('ar-EG')} ج.م
                    </span>
                  </div>
                </div>
              );
            ...
            <div className="flex items-center gap-3 pt-2">
              <button
                ...
                className="flex-1 py-3 rounded-xl bg-gradient-to-r from-[#D4A84F] to-[#B38838] hover:brightness-110 text-slate-950 font-black text-xs shadow-lg transition-all cursor-pointer flex items-center justify-center gap-2 disabled:opacity-50"
              >
                {isSavingRentEdit ? (
                  <>
                  ...
                  <>
                    ...
                    <span>حفظ وتثبيت القيمة الإيجارية</span>
                  </>
                )}
              </button>
              <button
