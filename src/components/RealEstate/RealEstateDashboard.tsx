...
import { 
  ...
  ArrowLeft, Bell, Settings, Receipt, Wallet, Database, FileText
} from 'lucide-react';
  ...
  return (
    <div className="space-y-6 text-[#F8F9FB] pb-6" dir="rtl">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 relative z-10">
        {[
          { id: 'owners', label: 'الملاك وأصحاب العقارات', icon: Landmark },
          { id: 'properties', label: 'سجل المباني والعقارات', icon: Building2 },
          { id: 'units', label: 'الوحدات والشواغر', icon: Home },
          { id: 'tenants', label: 'عقود المستأجرين', icon: FileText },
          { id: 'rent_collections', label: 'تحصيل الإيجارات والسندات', icon: Receipt },
          { id: 'tenant_statements', label: 'كشوف حسابات المستأجرين', icon: Users },
          { id: 'owner_statements', label: 'كشوف تسوية الملاك', icon: Wallet },
          { id: 'commissions', label: 'العمولات والمستحقات', icon: Percent },
          { id: 'advances_expenses', label: 'المصروفات والسجلات المالية', icon: CreditCard },
          { id: 'backup', label: 'النسخ الاحتياطي والاستعادة', icon: Database },
        ].map(card => {
