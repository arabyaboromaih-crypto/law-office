...
export const isDetentionSession = (s?: HearingSession | null, parentCase?: Case | null, allCases?: Case[]) => {
  ...
  if (targetCase?.isInvestigationActive && targetCase.detentionRenewals && targetCase.detentionRenewals.length > 0) {
...
export default function SessionCard({
  ...
  const normSessDate = normalizeHearingDate(session.date) || session.date;
  const isToday = normSessDate === todayStr;
  const isPast = normSessDate < todayStr;
  const isFuture = normSessDate > todayStr;
  const hasDecision = !!session.decision && session.decision.trim() !== '';
  const isCompleted = session.status === 'completed' || (!session.nextHearingDate && hasDecision);
  const isPostponed = session.status === 'postponed' || (!!session.nextHearingDate && hasDecision);
  const isDecisionRecorded = hasDecision || session.status === 'completed';

  const hasRecordedDecisionOnSameDate = !isDecisionRecorded && !isPostponed && sessions.some(
    s => s.id !== session.id && s.caseId === session.caseId && (normalizeHearingDate(s.date) || s.date) === normSessDate && (s.status === 'completed' || s.status === 'postponed' || (!!s.decision && s.decision.trim() !== ''))
  );
  ...
  if (isCompleted) {
    ...
    statusBadge = (
      <span className="text-xs font-bold text-amber-900 bg-amber-50 border border-amber-300 px-3 py-1.5 rounded-xl flex items-center gap-1.5 shadow-3xs">
        <span className="w-5 h-5 rounded-full bg-amber-200 flex items-center justify-center shrink-0">
          <Clock className="w-3.5 h-3.5 text-amber-800" />
        </span>
        <span>{hasDecision ? (isDetention ? 'تم التأجيل وتحديد ميعاد' : isExpert ? 'مؤجلة لمباشرة الخبير' : 'جلسة مؤجلة (تم رصد القرار)') : 'جلسة مؤجلة'}</span>
      </span>
    );
  } else if (isToday) {
    statusBadge = (
        ...
        <div className="flex flex-wrap items-center gap-2 justify-stretch sm:justify-end">
          ...
          {onOpenEditSession && currentUser?.permissions?.editSession && (
            <button
              ...
              title={isDetention ? "تعديل بيانات جلسة تجديد الحبس" : isDecisionRecorded ? "تعديل قرار الجلسة والمنطوق" : "تعديل تفاصيل الجلسة يدوياً"}
            >
              <span className={`w-4 h-4 rounded-full flex items-center justify-center shrink-0 ${
                isDetention ? 'bg-rose-200 text-rose-800' : isDecisionRecorded ? 'bg-amber-200 text-amber-900' : 'bg-white text-slate-600'
              }`}>
                {isDetention ? <Lock className="w-3 h-3 text-rose-700" /> : <Edit className="w-3 h-3 text-slate-600" />}
              </span>
              <span>{isDetention ? 'تعديل التجديد' : isDecisionRecorded ? 'تعديل القرار' : 'تعديل'}</span>
            </button>
          )}
