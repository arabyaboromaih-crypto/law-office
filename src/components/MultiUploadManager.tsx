  ...
  const startSingleUpload = (taskId: string) => {
    ...
    promise
      .then((downloadURL) => {
        ...
        // Notify parent
        const fileName = task.file?.name || 'file';
        const customName = customNames[taskId] || fileName;
        const category = customCategories[taskId] || globalCategory;
        const sizeInMB = task.file?.size ? (task.file.size / (1024 * 1024)).toFixed(2) : '0';
