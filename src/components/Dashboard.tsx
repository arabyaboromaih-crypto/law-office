...
export default function Dashboard({ 
  cases, clients, companies, sessions: rawSessions, currentUser, onNavigateToTab, 
  ...
  const handleOpenOutcome = (s: HearingSession, isEdit = false) => {
    ...
    if (!isDet && !isExp) {
      ...
      const priorSessions = allSessions.filter(item => item.caseId === s.caseId && item.id !== s.id).sort((a, b) => b.date.localeCompare(a.date));
      const priorWithCircuit = priorSessions.find(item => item.date <= s.date && ((item.circuit && item.circuit.trim()) || (item.nextHearingCircuit && item.nextHearingCircuit.trim())));
      const suggested = s.nextHearingCircuit || s.circuit || priorWithCircuit?.circuit || priorWithCircuit?.nextHearingCircuit || parentCase?.circuit || '';
      setNextHearingCircuit(suggested);
    } else {
      setNextHearingCircuit('');
    }
  };
  ...
  const handleOutcomeSubmit = (e: React.FormEvent) => {
    ...
    const updated: HearingSession = {
      ...
      nextHearingCircuit: (!isDet && !isExp && nextHearingDate) ? (nextHearingCircuit.trim() || undefined) : undefined,
      whatHappened,
      requirements,
  ...
  const delayedCases = activeCases.filter(c => {
    ...
    return (
      ...
      status.includes('تأجل') || 
      status.includes('تأجيل')
    );
  });
  ...
  return (
    <div className="space-y-3.5 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        {/* Sessions column */}
        <div className="lg:col-span-8 bg-white border border-slate-200 rounded-xl shadow-xs p-3.5 sm:p-5 md:p-6 space-y-6">
            ...
            {(() => {
                ...
                <>
                  <div className="flex justify-between items-center border-b border-slate-100 pb-3 mb-4">
                    <h3 className="text-sm font-bold text-slate-800 flex items-center gap-1.5">
                      ...
                      <span>
                        {isModalReadOnly
                          ? 'استعراض قرار الجلسة (للعرض فقط)'
                          : isEditingOutcome
                          ? `تعديل قرار الجلسة (قضية رقم ${outcomeSession.caseNumber})`
                          : isExpertSession(outcomeSession)
                          ? `نموذج إثبات إجراء وقرار جلسة الخبير (قضية رقم ${outcomeSession.caseNumber})`
                          : isDet
                          ? `نموذج قرار جلسة تجديد الحبس (قضية رقم ${outcomeSession.caseNumber})`
                  ...
                  {isModalReadOnly && (
                    <div className="bg-emerald-50 border border-emerald-200 text-emerald-950 p-3 rounded-2xl text-xs flex flex-wrap items-center justify-between gap-2 shadow-3xs mb-4">
                      ...
                      <span className="text-[11px] bg-white text-emerald-800 border border-emerald-300 px-2.5 py-1 rounded-xl font-black shrink-0">
                        لتعديل القرار: اضغط زر «تعديل» المخصص للجلسة
                      </span>
                    </div>
                  )}
                  {isEditingOutcome && (
                    <div className="bg-amber-50 border border-amber-300 text-amber-950 p-3 rounded-2xl text-xs flex items-center gap-2 shadow-3xs mb-4">
                      ...
                      <span className="font-black">وضع تعديل القرار: سيتم حفظ التعديل على هذا السجل التاريخي دون إنشاء سجلات مكررة.</span>
                    </div>
                  )}
                    ...
                    <div className="grid grid-cols-2 gap-3 bg-amber-500/5 p-3 rounded-xl border border-amber-500/10">
                      <div>
                        <label className="block text-xs font-semibold text-slate-700 mb-1">
                          {isExpertSession(outcomeSession) ? 'مكتب أو جهة الخبراء' : isDet ? 'جهة التجديد / المحكمة' : 'المحكمة'}
                        </label>
                        {isModalReadOnly ? (
                          <input
                            type="text"
                            value={outcomeCourt}
                            readOnly
                            className="w-full px-3 py-2 bg-slate-100 border border-slate-200 rounded-lg text-xs text-right font-bold text-slate-700"
                          />
                        ) : (
                          <CourtSelect
                            value={outcomeCourt}
                            onChange={setOutcomeCourt}
                            placeholder={isExpertSession(outcomeSession) ? 'مكتب الخبراء المختص' : isDet ? 'جهة التجديد المنعقدة أمامها' : 'المحكمة المنعقدة أمامها'}
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-slate-700 mb-1">
                          {isExpertSession(outcomeSession) ? 'مكان أو مقر المباشرة' : isDet ? 'غرفة المشورة / الدائرة' : 'الدائرة (يدوياً)'}
                        </label>
                        <input
                          type="text"
                          ...
                          className="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg text-xs disabled:bg-slate-100 disabled:text-slate-700"
                          placeholder={isExpertSession(outcomeSession) ? 'مقر الخبير أو موقع المعاينة' : isDet ? 'دائرة تجديد الحبس' : 'الدائرة القضائية'}
                        />
                      </div>
                    </div>
                      ...
                      <textarea
                        placeholder={
                          isExpertSession(outcomeSession)
                            ...
                            : isDet
                            ? 'مثال: تجديد الحبس لمدة 15 يوماً على ذمة التحقيقات...'
                        ...
                        disabled={isModalReadOnly}
                        rows={2.5}
                        className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-lg text-xs disabled:bg-slate-100 disabled:text-slate-700"
                      />
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        disabled={isModalReadOnly}
                        className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-lg text-xs font-mono font-bold disabled:bg-slate-100 disabled:text-slate-700"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-slate-700 mb-1 flex items-center justify-between">
                        <span>الدائرة</span>
                        <span className="text-[10px] text-slate-400 font-normal">مقترحة من السابقة</span>
                      </label>
                      <input
                        type="text"
                        value={nextHearingCircuit}
                        onChange={(e) => setNextHearingCircuit(e.target.value)}
                        disabled={isModalReadOnly}
                        placeholder="الدائرة القضائية..."
                        className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-lg text-xs font-bold text-slate-900 disabled:bg-slate-100 disabled:text-slate-700"
                      />
                    </div>
                  </div>
                  <div>
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-dashed border-slate-300 text-center">
                ...
                <div className="mt-3 flex items-center justify-center gap-2">
                  <input
                    ...
                    disabled={isModalReadOnly}
                    className="px-3 py-1.5 bg-white border border-slate-300 rounded-lg text-xs font-mono w-56 disabled:bg-slate-100 disabled:text-slate-700"
                  />
                  {!isModalReadOnly && (
                    <button
                      type="button"
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                {!isModalReadOnly && (
                  ...
                  >
                    {isEditingOutcome ? 'حفظ التعديلات وتحديث القرار' : 'تحديث وإغلاق الجلسة'}
                  </button>
                )}
              </div>
