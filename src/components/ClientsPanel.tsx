...
export default function ClientsPanel({ 
  ...
  onNavigateToTab, onSetCasesSearchQuery, onUpdateCase, onAddSession, onUpdateSession, onDeleteSession, onOpenCaseFile,
  ...
  // Selected case ID for opening comprehensive SmartCaseFile directly for a specific case
  const [selectedViewingCaseId, setSelectedViewingCaseId] = useState<string | null>(null);
  const selectedViewingCase = selectedViewingCaseId ? (cases.find(c => c.id === selectedViewingCaseId) || null) : null;

  // Back button handlers
  useBackHandler(showClientModal, () => {
    setShowClientModal(false);
    setEditingClient(null);
  });
  useBackHandler(showCompanyModal, () => {
    setShowCompanyModal(false);
    setEditingCompany(null);
  });
  useBackHandler(!!archiveCoTarget, () => setArchiveCoTarget(null));
  useBackHandler(!!viewDocsCompany, () => setViewDocsCompany(null));
  useBackHandler(!!selectedClientDetails && !selectedViewingCaseId, () => setSelectedClientDetails(null));
  useBackHandler(!!selectedViewingCaseId, () => setSelectedViewingCaseId(null));

  const [viewingCompanyDoc, setViewingCompanyDoc] = useState<any | null>(null);
  ...
  const handleClientSubmit = async (e: React.FormEvent) => {
    ...
    // Check for duplicate client with the same National ID
    if (finalClientNationalId && finalClientNationalId.trim().length > 0) {
      const duplicateClient = clients.find(c => {
        ...
        return (existingNatId && existingNatId === finalClientNationalId) || c.nationalId.trim() === finalClientNationalId.trim();
      });

      if (duplicateClient) {
        setActiveClientFormTab('basic');
        alert(`🚫 تم منع التسجيل!\nيوجد موكل آخر مسجل بالفعل بنفس الرقم القومي (${finalClientNationalId}) وهو الموكل: (${duplicateClient?.name || 'غير محدد'}).\nلا يمكن تكرار تسجيل موكل مرتين بنفس الرقم القومي.`);
        return;
      }
    }
    ...
    try {
      ...
      if (!finalIdCardUrl && stagedIdCard) {
        try {
          const cloudUrl = await uploadToR2(stagedIdCard.file);
          finalIdCardUrl = cloudUrl || stagedIdCard.fileUrl;
        } catch (err) {
          finalIdCardUrl = stagedIdCard.fileUrl;
        }
      }

      let finalPoaUrl = powerOfAttorneyUrl;
      if (!finalPoaUrl && stagedPowerOfAttorney) {
        try {
          const cloudUrl = await uploadToR2(stagedPowerOfAttorney.file);
          finalPoaUrl = cloudUrl || stagedPowerOfAttorney.fileUrl;
        } catch (err) {
          finalPoaUrl = stagedPowerOfAttorney.fileUrl;
        }
      }
      ...
      const clientData: Client = {
        ...
        phone: clPhone.trim(),
        secondaryPhone: clSecondaryPhone.trim() || '',
        email: clEmail.trim() || '',
        address: clAddress.trim(),
        job: clJob.trim(),
        notes: clNotes.trim() || '',
        companyId: clCompanyId || '',
        idCardUrl: finalIdCardUrl || undefined,
        idCardName: idCardName || undefined,
        powerOfAttorneyUrl: finalPoaUrl || undefined,
      ...
      // Success notification after confirmed database execution
      ...
      console.error("Failed to submit client to database:", err);
      alert('حدث خطأ أثناء حفظ الموكل في قاعدة البيانات، يرجى المحاولة مرة أخرى.');
    } finally {
  ...
  // View Company Doc via direct proxy access with modal fallback
  const handleViewCompanyDoc = (doc: CompanyDoc | { id?: string; name: string; type: 'pdf' | 'word' | 'image'; fileUrl: string; downloadURL?: string; size?: string; uploadDate?: string; }) => {
    const rawUrl = doc.downloadURL || doc.fileUrl;
    ...
    if (doc.id) {
      getFileFromIndexedDB(doc.id).then(blob => {
        if (blob) {
          const blobUrl = URL.createObjectURL(blob);
          window.open(blobUrl, '_blank');
        } else {
          setViewingCompanyDoc(doc);
        }
      }).catch(() => {
        setViewingCompanyDoc(doc);
      });
    ...
    <div className="space-y-3.5 animate-fadeIn">
      ...
      {returnToClient && !selectedClientDetails && (
        <div className="bg-slate-900 border border-amber-500/30 p-4 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4 shadow-xl text-right animate-fadeIn" dir="rtl">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="text-xs font-black text-white">تتبع الجلسة نشط للموكل: {returnToClient?.name || 'موكل'}</h4>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
          {(filteredClients || []).map((cl) => {
            ...
            return (
              <div key={cl.id || Math.random()} className="bg-white border border-slate-200 rounded-xl p-5 shadow-3xs flex flex-col justify-between space-y-4">
                <div className="space-y-3">
                  ...
                  {cl.companyId && (
                    <div className="text-[10px] bg-blue-50 text-blue-800 p-1.5 rounded inline-flex items-center gap-2 font-semibold">
                      <span>🏢 مرتبط بشركة: {companies.find(co => co && co.id === cl.companyId)?.name || 'شركة'}</span>
                      <button
                        type="button"
                        onClick={(e) => {
                          e.stopPropagation();
                  ...
                  <div className="pt-2.5 border-t border-slate-100 space-y-2">
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-bold text-slate-700 font-mono text-[11px]">
                        {caseCount} قضايا • {companies.filter(co => co && (co.id === cl.companyId || co.partners?.some(p => p && (p.name === cl?.name || p.phone === cl?.phone || (cl?.nationalId && p.nationalId === cl.nationalId))))).length} شركات
                      </span>
                    </div>
                    <button
              ...
              <FormCard title="بيانات الاتصال والارتباط" icon={Phone}>
                ...
                <FormField label="ربط الشركة المرتبطة بالمواكل (اختياري)">
                  ...
                  >
                    ...
                    {(companies || []).map(co => (
                      <option key={co?.id || Math.random()} value={co?.id || ''}>{co?.name || 'شركة'}</option>
                    ))}
                  </select>
                </FormField>
          ...
          <div className="flex justify-between items-center gap-3 pt-4 border-t">
            ...
            <div className="flex items-center gap-2">
              ...
              <PrimaryButton 
                type="submit" 
                disabled={isSavingClient}
                id="confirm-save-client-btn"
                className={activeClientFormTab !== 'attachments' ? 'opacity-90 hover:opacity-100' : ''}
              >
                        ...
                        <div key={idx} className="bg-white border border-slate-200 hover:border-amber-400/50 rounded-xl p-4 shadow-xs hover:shadow-sm transition-all duration-200 flex flex-col justify-between relative overflow-hidden group">
                          ...
                          <div className="space-y-2 pr-2 text-right">
                            <div className="flex items-center justify-between">
                              <strong className="text-[11px] font-black text-slate-900">{p?.name || 'شريك'}</strong>
                        ...
                        {companyDocsList.map((d) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {(() => {
                                const rawUrl = d.downloadURL || d.fileUrl;
      ...
      <BaseModal
        ...
        description={archiveCoTarget ? `سيتم الاحتفاظ بكافة بيانات تأسيس شركة (${archiveCoTarget?.name || 'الشركة'}) والشركاء والمستندات وعقود التأسيس، مع إيقاف نشاطها الفعال بالمكتب` : ''}
        icon={Archive}
        size="md"
      >
        {archiveCoTarget && (
      ...
      {viewDocsCompany && (
        <BaseModal
          ...
          title={`ملف المستندات الكامل وعقود التأسيس: ${viewDocsCompany?.name || 'الشركة'}`}
                  ...
                  {viewDocsCompany.documents.map((doc, idx) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-xs font-black text-slate-900 group-hover:text-amber-600 transition-colors">{doc?.name || 'مستند'}</p>
                          <p className="text-[9px] text-slate-400 font-bold mt-0.5">
                            النوع: {(doc?.type || 'ملف').toUpperCase()} | الحجم: {(doc as any)?.size || 'غير معروف'}
                          </p>
                        </div>
                      </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-red-200 shadow-2xl w-full max-w-md p-6 text-right" dir="rtl">
            ...
            <p className="text-xs text-slate-600 mb-4 leading-relaxed">
              يرجى العلم أن حذف الموكل <strong className="text-slate-800">"{deleteClientTarget?.name || 'الموكل'}"</strong> نهائياً سيقوم بإزالته بالكامل من دفاتر وملفات المؤسسة ولا يمكن التراجع عن هذه الخطوة.
            </p>
      ...
      {deleteCompanyTarget && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-red-200 shadow-2xl w-full max-w-md p-6 text-right" dir="rtl">
            ...
            <p className="text-xs text-slate-600 mb-4 leading-relaxed">
              يرجى العلم أن حذف سجلات شركة <strong className="text-slate-800">"{deleteCompanyTarget?.name || 'الشركة'}"</strong> وعقودها نهائياً سيقوم بإزالتها بالكامل ولا يمكن التراجع عن هذه الخطوة.
            </p>
      ...
      {selectedClientDetails && (() => {
        const cl = clients.find(c => c && c.id === selectedClientDetails.id) || selectedClientDetails;
        const clientCases = cases.filter(c => c && (c.clientId === cl?.id || c.clientName === cl?.name));
        ...
        const handlePrintClientPortfolio = () => {
          ...
          const casesHtml = clientCases.map((c, idx) => {
            ...
            const opponentsListHtml = c.opponentsList && c.opponentsList.length > 0
              ? c.opponentsList.map(op => `${op?.name || ''} (${op?.role || 'خصم'})`).join(' ، ')
              : `${c.opponent?.name || 'غير محدد'} (${c.opponent?.role || 'خصم'})`;
              ...
              <div class="report-page">
                ...
                <table class="data-table">
                  ...
                  <tr>
                    ...
                    <td>${getEffectiveStageInfo(c).degreeLabel}</td>
                  </tr>
                  <tr>
                    <th>المحكمة المختصة</th>
                    <td>${getEffectiveStageInfo(c).court}</td>
                    <th>الدائرة القضائية</th>
                    <td>${getEffectiveStageInfo(c).circuit}</td>
                  </tr>
                  <tr>
                    ...
                    <table class="nested-table">
                      ...
                      <tbody>
                        ${clientCompanies.map(co => {
                          if (!co) return '';
                          const partnerInfo = co.partners?.find(p => p && (p.name === cl?.name || p.phone === cl?.phone || (cl?.nationalId && p.nationalId === cl.nationalId)));
                          const relation = co.id === cl?.companyId 
                            ? 'شركة أساسية مرتبطة بالموكل' 
                            : (partnerInfo ? 'شريك مساهم بالشركة' : 'مرتبط بالشركة');
                          const shareText = partnerInfo ? `${toAr(partnerInfo.shareValue?.toLocaleString() || '0')} ج.م` : '-';
                          const percentText = partnerInfo ? `${toAr(partnerInfo.participationPercentage || 0)}%` : '-';
                          return `
                            <tr>
                              <td><strong>${co.name || 'شركة'}</strong></td>
                              <td>${co.activityType || '-'}</td>
              ...
              <div className="flex justify-between items-center pb-4 border-b border-slate-100">
                <div>
                  <h3 className="text-base font-bold text-slate-800 flex items-center gap-2">
                    ...
                    <span>سجل قضايا وشركات الموكل: <span className="text-amber-600">{cl?.name || 'الموكل'}</span></span>
                  </h3>
                          ...
                          {clientCases.map(c => (
                            <tr key={c.id} className="hover:bg-amber-50/30 transition-colors">
                              <td className="p-2.5">
                                <button
                                  type="button"
                                  onClick={() => setSelectedViewingCaseId(c.id)}
                                  className="text-right text-slate-900 hover:text-amber-700 font-bold transition-colors flex items-center gap-1.5 cursor-pointer group"
                                  title="فتح ملف القضية الشامل لهذه القضية فقط"
                                >
                                  <FolderOpen className="w-3.5 h-3.5 text-amber-500 group-hover:text-amber-600 shrink-0" />
                                  <span className="group-hover:underline underline-offset-2">{c.caseNumberFirstInstance} / {c.caseYearFirstInstance}</span>
                                </button>
                              </td>
                              <td className="p-2.5">
                                ...
                                <span className="text-slate-400 block text-[10px] mt-0.5">{getEffectiveStageInfo(c).degreeLabel}</span>
                              </td>
                              <td className="p-2.5">
                                <span className="block font-bold">{getEffectiveStageInfo(c).court}</span>
                                <span className="text-slate-400 block text-[10px] mt-0.5">{getEffectiveStageInfo(c).circuit}</span>
                              </td>
                              <td className="p-2.5">
                                <span className="block font-semibold">{c.opponent?.name || 'غير محدد'}</span>
                                <span className="text-slate-400 block text-[10px] mt-0.5">{c.opponent?.role || 'خصم'}</span>
                              </td>
                              ...
                              <td className="p-2.5 text-center">
                                <button
                                  type="button"
                                  onClick={() => setSelectedViewingCaseId(c.id)}
                                  className="w-full bg-amber-500 hover:bg-amber-600 active:scale-95 text-slate-950 text-[10px] py-1.5 px-2 rounded-lg font-black transition-all flex items-center justify-center gap-1 cursor-pointer shadow-3xs hover:shadow-xs"
                                  title="فتح ملف القضية الشامل لهذه القضية فقط"
                                >
                                  <FolderOpen className="w-3.5 h-3.5 shrink-0" />
                                  <span>ملف القضية الشامل</span>
                                </button>
                              </td>
                            </tr>
                          ))}
                  ...
                  {clientCompanies.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {clientCompanies.map(co => {
                        ...
                        return (
                          <div key={co.id || Math.random()} className="bg-slate-50/50 p-4 border border-slate-200 flex flex-col justify-between gap-3">
                            <div className="space-y-2 text-right">
                              <div className="flex items-center justify-between">
                                <strong className="text-slate-800 text-xs font-black">{co.name || 'شركة'}</strong>
      ...
      {viewingCompanyDoc && (
        <DocumentViewerModal
          file={viewingCompanyDoc}
          onClose={() => setViewingCompanyDoc(null)}
        />
      )}

      {/* 📁 Comprehensive Smart Case File Modal for the Selected Case */}
      {selectedViewingCase && (
        <div className="relative z-[70]">
          <SmartCaseFile
            caseData={selectedViewingCase}
            currentUser={currentUser}
            users={users}
            sessions={sessions}
            clients={clients}
            onUpdateCase={async (updated) => {
              if (onUpdateCase) await onUpdateCase(updated);
            }}
            onAddSession={async (sess) => {
              if (onAddSession) await onAddSession(sess);
            }}
            onUpdateSession={async (sess) => {
              if (onUpdateSession) await onUpdateSession(sess);
            }}
            onDeleteSession={async (sessionId) => {
              if (onDeleteSession) await onDeleteSession(sessionId);
            }}
            onClose={() => setSelectedViewingCaseId(null)}
          />
        </div>
      )}
