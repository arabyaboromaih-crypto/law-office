...
export default function AgendaPanel({ 
  ...
  const isModalReadOnly = !isEditingOutcome && !!outcomeSession && (!!outcomeSession.decision || outcomeSession.status === 'completed');
  ...
  const handleOpenOutcome = (s: HearingSession) => {
    ...
    const isRecorded = !!s.decision || s.status === 'completed';
  ...
  const handleOutcomeSubmit = (e: React.FormEvent) => {
    ...
    const updated: HearingSession = {
      ...outcomeSession,
      status: hasDecision ? 'completed' : (nextHearingDate ? 'postponed' : (outcomeSession.status || 'pending')),
              ...
              <div className="flex justify-between items-center pt-3 border-t border-slate-100">
                ...
                <div className="flex gap-2.5">
                  ...
                  {!isModalReadOnly && (
                    ...
                    >
                      {isEditingOutcome ? 'حفظ التعديلات وتحديث الأجندة' : 'حفظ وتحديث ملف القضية التلقائي'}
                    </button>
                  )}
                </div>
              </div>
