...
import { extractHearingDate, deduplicateSessions, normalizeHearingDate, findMatchingSession } from '../utils/hearingSync';
...
interface AgendaPanelProps {
  ...
  onUpdateSession: (s: HearingSession) => Promise<void> | void;
...
export default function AgendaPanel({ 
  sessions: rawSessions, cases, users, currentUser, onAddSession, onUpdateSession, onDeleteSession, onNavigateToTab, onSearchCase, onOpenCaseFile, onUpdateCase
}: AgendaPanelProps) {
  // Deduplicate all sessions per (caseId + date + sessionCategory) strictly
  const sessions = useMemo(() => {
    return deduplicateSessions(rawSessions || [], cases);
  }, [rawSessions, cases]);
  ...
  const [outcomeNextHearingCircuit, setOutcomeNextHearingCircuit] = useState('');
  ...
  const isModalReadOnly = !isEditingOutcome && !!outcomeSession && ((!!outcomeSession.decision && outcomeSession.decision.trim() !== '') || outcomeSession.status === 'completed');
  ...
  const [editSessionDecision, setEditSessionDecision] = useState('');
  const [editSessionNextHearingDate, setEditSessionNextHearingDate] = useState('');
  const [editSessionNextHearingCircuit, setEditSessionNextHearingCircuit] = useState('');
  const [editSessionWhatHappened, setEditSessionWhatHappened] = useState('');
  const [editSessionRequirements, setEditSessionRequirements] = useState('');
  const [editSessionNotes, setEditSessionNotes] = useState('');
  const [editSessionStatus, setEditSessionStatus] = useState<HearingSession['status']>('pending');
  ...
  const [editDetentionStartDate, setEditDetentionStartDate] = useState('');
  const [editDetentionDecisionNumber, setEditDetentionDecisionNumber] = useState('');
  const [editDetentionNextAuthority, setEditDetentionNextAuthority] = useState('');
  ...
  cases.forEach(c => {
    if (c.files) {
      c.files.forEach(file => {
        ...
        if (!fileDate) return;
        const exists = !!findMatchingSession(sessions, {
          caseId: c.id,
    ...
    if ((c.isReferredToExperts || c.expertReferral?.isReferred) && c.expertReferral?.sessions) {
      c.expertReferral.sessions.forEach(expSess => {
        ...
        const normExpDate = normalizeHearingDate(expSess.date);
        const exists = !!findMatchingSession(sessions, {
          caseId: c.id,
          caseNumber: c.caseNumberFirstInstance,
          caseYear: c.caseYearFirstInstance,
    ...
    if (c.isInvestigationActive && c.detentionRenewals && c.detentionRenewals.length > 0) {
      c.detentionRenewals.forEach((ren, idx) => {
        ...
        if (!renDate) return;
        const normRenDate = normalizeHearingDate(renDate);
        const exists = !!findMatchingSession(sessions, {
          caseId: c.id,
          caseNumber: c.investigationNumber || c.caseNumberFirstInstance,
  ...
  const handleAddSingleDiscoveredSession = (item: typeof discoveredSessions[0]) => {
    ...
    const newSession: HearingSession = {
      ...
      opponentName: item.caseObj.opponent?.name || 'غير محدد',
  ...
  const filteredSessions = sessions.filter(s => {
    // Only hide sessions of explicitly archived cases
    const parentCase = cases.find(c => c.id === s.caseId);
    if (parentCase && parentCase.isArchived) {
      return false;
    }
  ...
  const filteredDiscoveredSessions = discoveredSessions.filter(item => {
    ...
    return (
      ...
      (item.caseObj?.opponent?.name && item.caseObj.opponent.name.toLowerCase().includes(q)) ||
  ...
  const detentionSessionsCount = sessions.filter(s => {
    const parentCase = cases.find(c => c.id === s.caseId);
    return !parentCase?.isArchived && isDetentionSession(s, parentCase, cases);
  }).length;
  ...
  const todaySessionsCount = filteredSessions.filter(s => (normalizeHearingDate(s.date) || s.date) === todayStr).length;
  const tomorrowObj = new Date();
  tomorrowObj.setDate(tomorrowObj.getDate() + 1);
  const tomorrowStr = getLocalYYYYMMDD(tomorrowObj);
  const tomorrowSessionsCount = filteredSessions.filter(s => (normalizeHearingDate(s.date) || s.date) === tomorrowStr).length;
  const upcomingSessionsCount = filteredSessions.filter(s => (normalizeHearingDate(s.date) || s.date) > todayStr).length;
  ...
  const unrecordedSessions = sessions.filter(s => {
    ...
    const isEnded = (normalizeHearingDate(s.date) || s.date) <= todayStr;
    ...
    return isEnded;
  }).sort((a, b) => (normalizeHearingDate(b.date) || b.date).localeCompare(normalizeHearingDate(a.date) || a.date));
  ...
  const dailySessionsList = filteredSessions.filter(s => (normalizeHearingDate(s.date) || s.date) === dateFormattedStr);
  ...
  // Helper to suggest/autofill circuit from the previous session of the same case
  const getSuggestedPreviousCircuit = (s: HearingSession, parentCase?: Case) => {
    if (s.nextHearingCircuit && s.nextHearingCircuit.trim()) {
      return s.nextHearingCircuit.trim();
    }
    if (s.circuit && s.circuit.trim()) {
      return s.circuit.trim();
    }
    const allSessions = (rawSessions && rawSessions.length > 0 ? rawSessions : sessions) || [];
    const caseSessions = allSessions
      .filter(item => item.caseId === s.caseId)
      .sort((a, b) => b.date.localeCompare(a.date));

    // Find the previous session of the same case that has a circuit
    const prevSession = caseSessions.find(item => item.id !== s.id && item.date <= s.date && ((item.circuit && item.circuit.trim()) || (item.nextHearingCircuit && item.nextHearingCircuit.trim())));
    if (prevSession) {
      return prevSession.circuit?.trim() || prevSession.nextHearingCircuit?.trim() || '';
    }
    return parentCase?.circuit?.trim() || '';
  };
  ...
  const handleOpenOutcome = (s: HearingSession) => {
    const parentCase = cases.find(c => c.id === s.caseId);
    const isDet = isDetentionSession(s, parentCase, cases);
    const isRecorded = (!!s.decision && s.decision.trim() !== '') || s.status === 'completed';

    if (isDet && parentCase && !isRecorded) {
    ...
    const isRegular = !isDet && !isExpertSession(s);
    setOutcomeNextHearingCircuit(s.nextHearingCircuit || (isRegular ? getSuggestedPreviousCircuit(s, parentCase) : ''));
    setWhatHappened(s.whatHappened || '');
    setRequirements(s.requirements || '');
    setRollPhotoName(s.rollPhotoUrl ? (s.rollPhotoUrl === 'roll_attached_url' ? 'صورة رول الجلسة المرفوعة' : s.rollPhotoUrl) : '');
    setIsRollUploaded(!!s.rollPhotoUrl);
    setOutcomeCourt(s.court || parentCase?.court || '');
    setOutcomeCircuit(s.circuit || parentCase?.circuit || '');
    ...
    setOutcomeDetentionRenewalNumber(s.detentionRenewalNumber !== undefined && s.detentionRenewalNumber !== null ? String(s.detentionRenewalNumber) : '');
    ...
    try {
      ...
      const updated: HearingSession = {
        ...
        nextHearingCircuit: isRegular && nextHearingDate
          ...
          : (isRegular ? (outcomeNextHearingCircuit.trim() || outcomeSession.nextHearingCircuit || undefined) : undefined),
        whatHappened: whatHappened ? whatHappened.trim() : undefined,
        requirements: requirements ? requirements.trim() : undefined,
        rollPhotoUrl: isRollUploaded ? (rollPhotoName || outcomeSession.rollPhotoUrl || 'roll_attached_url') : (rollPhotoName ? rollPhotoName : outcomeSession.rollPhotoUrl),
  ...
  // Edit Session Handlers - Opens the dedicated Manual Session Edit Modal with all data prefilled
  const handleOpenEditSession = (s: HearingSession) => {
    const parentCase = cases.find(c => c.id === s.caseId);
    const isDet = isDetentionSession(s, parentCase);

    const isExp = isExpertSession(s) || !!parentCase?.isReferredToExperts || !!parentCase?.expertReferral?.isReferred;
    const isRegular = !isDet && !isExp;

    // If session decision was already recorded, edit decision using the Decision Modal directly
    if (s.decision || s.status === 'completed') {
    ...
    setEditingSession(s);
    setEditSessionCourt(s.court || parentCase?.court || '');
    setEditSessionCircuit(s.circuit || parentCase?.circuit || '');
    setEditSessionDate(s.date || '');
    setEditSessionTime(s.time || '09:00');
    setEditSessionSubject(s.subject || '');
    setEditSessionLawyer(s.assignedLawyerId || parentCase?.assignedLawyerId || '');
    setEditSessionDecision(s.decision || '');
    setEditSessionNextHearingDate(s.nextHearingDate || '');
    setEditSessionNextHearingCircuit(isRegular ? getSuggestedPreviousCircuit(s, parentCase) : '');
    setEditSessionWhatHappened(s.whatHappened || '');
    setEditSessionRequirements(s.requirements || '');
    setEditSessionNotes(s.notes || '');
    setEditSessionStatus(s.status || (s.decision ? 'completed' : 'pending'));

    setEditDetentionStartDate(s.detentionStartDate || parentCase?.detentionStartDate || '');
    setEditDetentionDurationDays(s.detentionDurationDays || 15);
    setEditDetentionRenewalNumber(s.detentionRenewalNumber !== undefined && s.detentionRenewalNumber !== null ? String(s.detentionRenewalNumber) : '');
    setEditDetentionDecisionNumber(s.detentionDecisionNumber || '');
    setEditDetentionAuthority(s.detentionAuthority || s.court || (isDet ? 'النيابة العامة' : ''));
    setEditDetentionNextAuthority(s.detentionNextAuthority || 'غرفة المشورة / النيابة');
  };
  ...
  const handleEditSessionSubmit = (e: React.FormEvent) => {
    ...
    const isExp = isExpertSession(editingSession) || !!parentCase?.isReferredToExperts || !!parentCase?.expertReferral?.isReferred;

    const trimmedDecision = editSessionDecision ? editSessionDecision.trim() : '';
    const hasDecision = trimmedDecision !== '';
    const nextDate = editSessionNextHearingDate ? editSessionNextHearingDate.trim() : undefined;
    const durDays = Number(editDetentionDurationDays) || 15;

    const finalStatus: HearingSession['status'] = hasDecision 
      ? 'completed' 
      : (nextDate ? 'postponed' : (editSessionStatus || editingSession.status || 'pending'));

    const updated: HearingSession = {
      ...editingSession,
      court: editSessionCourt || editingSession.court,
      circuit: editSessionCircuit || editingSession.circuit,
      date: editSessionDate,
      time: editSessionTime || editingSession.time || '09:00',
      subject: editSessionSubject,
      assignedLawyerId: editSessionLawyer || undefined,
      assignedLawyerName: assignedUser ? assignedUser.fullName : editingSession.assignedLawyerName,
      status: finalStatus,
      decision: hasDecision ? trimmedDecision : (editingSession.decision || undefined),
      nextHearingDate: nextDate || (hasDecision ? editingSession.nextHearingDate : undefined),
      nextHearingCircuit: (!isDet && !isExp && nextDate)
        ? (editSessionNextHearingCircuit.trim() || undefined)
        : (!isDet && !isExp ? (editSessionNextHearingCircuit.trim() || editingSession.nextHearingCircuit || undefined) : undefined),
      whatHappened: editSessionWhatHappened ? editSessionWhatHappened.trim() : editingSession.whatHappened,
      requirements: editSessionRequirements ? editSessionRequirements.trim() : editingSession.requirements,
      notes: editSessionNotes ? editSessionNotes.trim() : editingSession.notes,
      ...
      detentionNextAuthority: isDet ? (editDetentionNextAuthority || 'غرفة المشورة / النيابة') : editingSession.detentionNextAuthority,
      detentionDurationDays: isDet ? durDays : editingSession.detentionDurationDays,
      detentionDuration: isDet ? `${durDays} يوم` : editingSession.detentionDuration,
      detentionRenewalNumber: isDet ? (editDetentionRenewalNumber !== '' ? Number(editDetentionRenewalNumber) : editingSession.detentionRenewalNumber) : editingSession.detentionRenewalNumber,
      detentionDecisionNumber: isDet ? (editDetentionDecisionNumber || editingSession.detentionDecisionNumber) : editingSession.detentionDecisionNumber,
      detentionStartDate: isDet ? (editDetentionStartDate || editingSession.detentionStartDate || parentCase?.detentionStartDate) : editingSession.detentionStartDate,

      // Expert fields
      isExpertSession: isExp ? true : editingSession.isExpertSession
    };

    onUpdateSession(updated);

    // Sync any edits directly into parent case to keep data unified and prevent overwrites
    if (parentCase && onUpdateCase) {
      let updatedCase: Case = { ...parentCase };

      if (isDet && parentCase.detentionRenewals && parentCase.detentionRenewals.length > 0) {
        const renList = [...parentCase.detentionRenewals];
        const existingRenIdx = renList.findIndex(r => 
          (editingSession.detentionRenewalId && r.id === editingSession.detentionRenewalId) ||
          ((r.date || r.renewalDate) === editingSession.date)
        );

        const updatedRen: DetentionRenewalRecord = {
          id: existingRenIdx >= 0 ? renList[existingRenIdx].id : (editingSession.detentionRenewalId || `ren-${Date.now()}`),
          renewalNumber: (existingRenIdx >= 0 ? renList[existingRenIdx].renewalNumber : undefined) || (editDetentionRenewalNumber !== '' ? Number(editDetentionRenewalNumber) : renList.length + 1),
          date: editSessionDate,
          renewalDate: editSessionDate,
          court: editSessionCourt || parentCase.court,
          circuit: editSessionCircuit || parentCase.circuit,
          authority: editDetentionAuthority || 'النيابة العامة',
          durationDays: durDays,
          duration: `${durDays} يوم`,
          decision: trimmedDecision || (existingRenIdx >= 0 ? renList[existingRenIdx].decision : undefined),
          decisionNumber: editDetentionDecisionNumber || (existingRenIdx >= 0 ? renList[existingRenIdx].decisionNumber : undefined),
          notes: [editSessionSubject, editSessionWhatHappened, editSessionRequirements].filter(Boolean).join(' | ') || (existingRenIdx >= 0 ? renList[existingRenIdx].notes : undefined),
          nextRenewalDate: nextDate || (existingRenIdx >= 0 ? renList[existingRenIdx].nextRenewalDate : undefined),
          nextAuthority: editDetentionNextAuthority || (existingRenIdx >= 0 ? renList[existingRenIdx].nextAuthority : undefined),
          detentionStartDate: editDetentionStartDate || parentCase.detentionStartDate || undefined
        };

        if (existingRenIdx >= 0) {
          renList[existingRenIdx] = updatedRen;
        } else {
          renList.push(updatedRen);
        }
        updatedCase.detentionRenewals = renList;
        updatedCase.isInvestigationActive = true;
      }

      if (isExp && parentCase.expertReferral) {
        const exp = { ...parentCase.expertReferral };
        const expSessions = exp.sessions ? [...exp.sessions] : [];
        const matchedIdx = expSessions.findIndex(s => 
          (editingSession.expertSessionId && s.id === editingSession.expertSessionId) ||
          s.date === editingSession.date
        );

        if (matchedIdx >= 0) {
          expSessions[matchedIdx] = {
            ...expSessions[matchedIdx],
            date: editSessionDate,
            time: editSessionTime || expSessions[matchedIdx].time || '10:00',
            location: editSessionCircuit || expSessions[matchedIdx].location || exp.expertOffice || 'مكتب الخبراء',
            decisionOrAction: trimmedDecision || expSessions[matchedIdx].decisionOrAction || '',
            nextSessionDate: nextDate || expSessions[matchedIdx].nextSessionDate,
            status: finalStatus,
            notes: editSessionSubject || expSessions[matchedIdx].notes
          };
        }
        exp.sessions = expSessions;
        if (exp.firstSessionDate === editingSession.date) {
          exp.firstSessionDate = editSessionDate;
        }
        updatedCase.expertReferral = exp;
        updatedCase.isReferredToExperts = true;
      }

      // If the case's current nextHearingDate matches the session date before edit or needs updating
      if (parentCase.nextHearingDate === editingSession.date || !parentCase.nextHearingDate) {
        updatedCase.nextHearingDate = nextDate || editSessionDate;
        updatedCase.nextHearingTime = editSessionTime || parentCase.nextHearingTime || '09:00';
      }
      if (editSessionCourt) updatedCase.court = editSessionCourt;
      if (editSessionCircuit) updatedCase.circuit = editSessionCircuit;
      if (editSessionLawyer) updatedCase.assignedLawyerId = editSessionLawyer;
  ...
  const handlePrintAgenda = (scope: string) => {
    ...
    if (printWindow) {
      ...
      const regularCount = sortedPrintData.filter(s => !isExpertSession(s) && !isDetentionSession(s, cases.find(c => c.id === s.caseId), cases)).length;
      const expertCount = sortedPrintData.filter(s => isExpertSession(s)).length;
      const detentionCount = sortedPrintData.filter(s => isDetentionSession(s, cases.find(c => c.id === s.caseId), cases)).length;
            ...
            <div class="sessions-container">
              ...
              ` : 
                sortedPrintData.map((s, idx) => {
                  ...
                  const isDet = isDetentionSession(s, c, cases);
                  ...
                  const extraClients = (c?.clientsList || []).filter(cl => cl && cl.name && cl.name.trim() !== mainClientName.trim());
                  ...
                  const extraOpponents = (c?.opponentsList || []).filter(op => op && op.name && op.name.trim() !== mainOpponentName.trim());
  ...
  const renderSessionStatusAndActions = (session: HearingSession) => {
    const isDecisionRecorded = (!!session.decision && session.decision.trim() !== '') || session.status === 'completed';
    const sessNormDate = normalizeHearingDate(session.date) || session.date;
    const isToday = sessNormDate === todayStr;
    const isPast = sessNormDate < todayStr;
    const isFuture = sessNormDate > todayStr;
    const isPostponed = session.status === 'postponed' || (session.decision && session.nextHearingDate);

    const hasRecordedDecisionOnSameDate = !isDecisionRecorded && sessions.some(
      s => s.id !== session.id && s.caseId === session.caseId && (normalizeHearingDate(s.date) || s.date) === sessNormDate && ((!!s.decision && s.decision.trim() !== '') || s.status === 'completed')
    );
      ...
      <div className="shrink-0 flex flex-wrap items-center gap-2">
        ...
        {currentUser?.permissions?.editSession && (
          <button
            ...
            title={isDecisionRecorded ? "تعديل قرار الجلسة والمنطوق" : "تعديل تفاصيل الجلسة يدوياً"}
          >
            <span className={`w-4 h-4 rounded-full flex items-center justify-center shrink-0 ${
              isDecisionRecorded ? 'bg-amber-200 text-amber-900' : 'bg-white'
            }`}>
              <Edit className="w-3 h-3 text-slate-600" />
            </span>
            <span>{isDecisionRecorded ? 'تعديل القرار' : 'تعديل'}</span>
          </button>
        )}
          ...
          if (isDecisionRecorded) {
            return (
              <button
                ...
                title="عرض واستعراض قرار الجلسة (للعرض فقط)"
              >
            ...
            return (
              <span 
                className="text-xs text-slate-500 font-bold bg-slate-100/80 border border-slate-200 px-3 py-1.5 rounded-xl flex items-center gap-1"
  ...
  return (
    <div className="space-y-4 animate-fadeIn" dir="rtl">
      ...
      {(() => {
        const tomorrowSessionsList = filteredSessions.filter(s => (normalizeHearingDate(s.date) || s.date) === tomorrowStr);
              ...
              {filteredDiscoveredSessions.map((item, idx) => {
                ...
                >
                  <div className="space-y-1 flex-1 min-w-0">
                    ...
                    <p className="text-[11px] font-bold text-slate-900 truncate">{item.caseObj?.clientName || 'الموكل'}</p>
                    <p className="text-[9px] text-slate-500 truncate font-mono">الملف: {item.file?.name || 'مستند'}</p>
                  </div>
            ...
            {viewMode === 'weekly' && (
              <div className="grid grid-cols-1 md:grid-cols-7 gap-3">
                {weekDays.map((day) => {
                  ...
                  const daySessions = filteredSessions.filter(s => (normalizeHearingDate(s.date) || s.date) === dayStr);
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-7 gap-1.5">
                  {monthCells.map((day, idx) => {
                    ...
                    const daySessions = filteredSessions.filter(s => (normalizeHearingDate(s.date) || s.date) === dStr);
          ...
          {(() => {
            ...
            return (
              <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto animate-fadeIn">
                  ...
                  <h3 className="text-sm font-black text-slate-900 flex items-center gap-2">
                    ...
                    <span>
                      {isModalReadOnly
                        ...
                        : isEditingOutcome
                        ? isDet
                          ? `تعديل بيانات وقرار جلسة تجديد الحبس (قضية رقم ${outcomeSession.caseNumber})`
                          : isExp
                          ? `تعديل بيانات وإجراء جلسة الخبير (قضية رقم ${outcomeSession.caseNumber})`
                          : `تعديل بيانات وقرار الجلسة (قضية رقم ${outcomeSession.caseNumber})`
                        : isDet
                ...
                {isModalReadOnly && (
                  <div className="bg-emerald-50 border border-emerald-200 text-emerald-950 p-3 rounded-2xl text-xs flex flex-wrap items-center justify-between gap-2 shadow-3xs mb-4">
                    <div className="flex items-center gap-2 font-black">
                      <FileText className="w-4 h-4 text-emerald-700 shrink-0" />
                      <span>استعراض قرار الجلسة (للعرض فقط كسجل تاريخي محمي)</span>
                    </div>
                    <span className="text-[11px] bg-white text-emerald-800 border border-emerald-300 px-2.5 py-1 rounded-xl font-black shrink-0">
                      لتعديل القرار: استخدم زر «التعديل» المخصص للجلسة
                    </span>
                  </div>
                )}
                {isEditingOutcome && (
                  <div className="bg-amber-50 border border-amber-300 text-amber-950 p-3 rounded-2xl text-xs flex items-center gap-2 shadow-3xs mb-4">
                    <Edit className="w-4 h-4 text-amber-800 shrink-0" />
                    <span className="font-black">وضع تعديل القرار: سيتم تحديث هذا السجل التاريخي دون إنشاء سجلات مكررة.</span>
                  </div>
                )}
              ...
              <div className="grid grid-cols-2 gap-3 bg-amber-500/10 p-3.5 rounded-2xl border border-amber-500/20">
                <div>
                  <label className="block text-xs font-bold text-slate-800 mb-1">
                    {isDet ? 'سلطة التحقيق / جهة التجديد' : isExpertSession(outcomeSession) ? 'مكتب أو جهة الخبراء' : 'المحكمة'}
                  </label>
                  {isModalReadOnly ? (
                    <input
                      type="text"
                    ...
                    <CourtSelect
                      ...
                      placeholder={isDet ? 'النيابة العامة / جهة التحقيق' : isExpertSession(outcomeSession) ? 'مكتب الخبراء المختص' : 'المحكمة المنعقدة أمامها'}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-800 mb-1">
                    {isDet ? 'الدائرة / الغرفة' : isExpertSession(outcomeSession) ? 'مكان أو مقر المباشرة' : 'الدائرة (يدوياً)'}
                  </label>
                  <input
                    type="text"
                    ...
                    placeholder={isDet ? 'دائرة تجديد الحبس' : isExpertSession(outcomeSession) ? 'مقر الخبير أو موقع المعاينة' : 'الدائرة القضائية'}
                  />
                </div>
              </div>
              ...
              {!isExpertSession(outcomeSession) && !isDet ? (
                <div className="space-y-3">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs font-bold text-slate-800 mb-1">
                        تحديد الجلسة القادمة (إن وجد)
                      </label>
                      <input
                        type="date"
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-800 mb-1 flex items-center justify-between">
                        <span>الدائرة</span>
                        <span className="text-[10px] font-normal text-slate-500">مقترحة من الجلسة السابقة ويمكن تعديلها</span>
                      </label>
                      <input
                        type="text"
                        value={outcomeNextHearingCircuit}
                        onChange={(e) => setOutcomeNextHearingCircuit(e.target.value)}
                        disabled={isModalReadOnly}
                        placeholder="الدائرة القضائية للجلسة القادمة..."
                        className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-xl text-xs disabled:bg-slate-100 disabled:text-slate-700 font-bold text-slate-900"
                      />
                    </div>
                  </div>
                  <div>
      ...
      <AddHearingModal
        ...
        onAddSession={async (newSess, detentionMeta) => {
          ...
          if (detentionMeta?.isDetention && onUpdateCase) {
            ...
            if (targetCase && targetCase.isInvestigationActive) {
        ...
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto text-right animate-fadeIn" dir="rtl">
            ...
            {(() => {
              ...
              const isExp = isExpertSession(editingSession) || !!editParentCase?.isReferredToExperts || !!editParentCase?.expertReferral?.isReferred;
              ...
              return (
                <>
                  ...
                  <form onSubmit={handleEditSessionSubmit} className="space-y-4">
                    ...
                    {/* Decision, Next Hearing Date & Outcome Details */}
                    <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200 space-y-3">
                      <div className="flex items-center gap-1.5 font-black text-xs text-slate-900 border-b border-slate-200/80 pb-2">
                        <FileText className="w-3.5 h-3.5 text-amber-600" />
                        <span>القرار والمنطوق وتفاصيل المتابعة (اختياري)</span>
                      </div>

                      <div>
                        <label className="block text-xs font-bold text-slate-800 mb-1 text-right">
                          {isDet ? 'قرار جلسة التجديد (المنطوق)' : 'قرار الجلسة / ما انتهت إليه'}
                        </label>
                        <textarea
                          value={editSessionDecision}
                          onChange={(e) => setEditSessionDecision(e.target.value)}
                          rows={2}
                          placeholder={isDet ? 'مثال: تجديد حبس المتهم 15 يوماً على ذمة التحقيقات' : 'مثال: حجز الدعوى للحكم، أو التأجيل للمستندات...'}
                          className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl text-xs text-right font-medium resize-none"
                        />
                      </div>
                      ...
                      {!isExp && !isDet ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          <div>
                            <label className="block text-xs font-bold text-slate-800 mb-1 text-right">تحديد الجلسة القادمة (إن وجدت)</label>
                            <input
                              type="date"
                              value={editSessionNextHearingDate}
                              onChange={(e) => setEditSessionNextHearingDate(e.target.value)}
                              className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl text-xs font-mono font-bold text-right"
                            />
                          </div>
                          <div>
                            <label className="block text-xs font-bold text-slate-800 mb-1 text-right flex items-center justify-between">
                              <span>الدائرة</span>
                              <span className="text-[10px] font-normal text-slate-500">مقترحة من الجلسة السابقة ويمكن تعديلها</span>
                            </label>
                            <input
                              type="text"
                              value={editSessionNextHearingCircuit}
                              onChange={(e) => setEditSessionNextHearingCircuit(e.target.value)}
                              placeholder="أدخل أو عدّل الدائرة..."
                              className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl text-xs font-bold text-right text-slate-900"
                            />
                          </div>
                        </div>
                      ) : (
                        <div>
                          <label className="block text-xs font-bold text-slate-800 mb-1 text-right">تاريخ الجلسة القادمة (إن وجدت)</label>
                          <input
                            type="date"
                            value={editSessionNextHearingDate}
                            onChange={(e) => setEditSessionNextHearingDate(e.target.value)}
                      ...
                      <div>
                        <label className="block text-xs font-bold text-slate-800 mb-1 text-right">حالة الجلسة</label>
                        <select
                          value={editSessionDecision ? 'completed' : (editSessionNextHearingDate ? 'postponed' : editSessionStatus)}
                          onChange={(e) => setEditSessionStatus(e.target.value as HearingSession['status'])}
                          className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl text-xs font-semibold text-right"
                        >
                          <option value="pending">قادمة / قيد الانتظار</option>
                          <option value="completed">منتهية (تم إثبات القرار)</option>
                          <option value="postponed">مؤجلة لجلسة قادمة</option>
                        </select>
                      </div>

                      <div>
                        <label className="block text-xs font-bold text-slate-800 mb-1 text-right">ما تم بالجلسة والإجراءات</label>
                        <input
                          type="text"
                          value={editSessionWhatHappened}
                          onChange={(e) => setEditSessionWhatHappened(e.target.value)}
                          placeholder="مثال: حضر الأستاذ ومعه المستندات وطلب أجلاً للمذكرات"
                          className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl text-xs font-medium text-right"
                        />
                      </div>

                      <div>
                        <label className="block text-xs font-bold text-slate-800 mb-1 text-right">الطلبات والمطلوب للجلسة القادمة</label>
                        <input
                          type="text"
                          value={editSessionRequirements}
                          onChange={(e) => setEditSessionRequirements(e.target.value)}
                          placeholder="مثال: إعلان بالدعوى الفرعية وسداد الأمانة"
                          className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl text-xs font-medium text-right"
                        />
                      </div>

                      <div>
                        <label className="block text-xs font-bold text-slate-800 mb-1 text-right">ملاحظات إضافية</label>
                        <input
                          type="text"
                          value={editSessionNotes}
                          onChange={(e) => setEditSessionNotes(e.target.value)}
                          placeholder="أي ملاحظات أو تنبيهات إضافية للمكتب"
                          className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl text-xs font-medium text-right"
                        />
                      </div>
                    </div>
      ...
      {detentionModalCase && detentionModalCase.isInvestigationActive && onUpdateCase && (
