            ...
            <AnimatePresence mode="wait">
              {activeTab === 'overview' && (
                <OverviewTab 
                  ...
                  logAction={logAction}
                  onOpenDetentionModal={() => setShowDetentionModal(true)}
                />
              )}
              {activeTab === 'sessions' && (
                <SessionsTab 
...
function OverviewTab({ 
  ...
  const handleSave = async () => {
    ...
    if (detentionRenewals && detentionRenewals.length > 0) {
    ...
    const updated: Case = {
      ...
      investigationNumber: isInvestigationActive ? (investigationNumber || undefined) : undefined,
      investigationYear: isInvestigationActive ? (investigationYear || undefined) : undefined,
      ...
      investigationProcedures: isInvestigationActive ? investigationProcedures.map((p, idx) => ({
        ...p,
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm space-y-6">
        ...
        <div className="space-y-6">
          ...
          {(localCase.isInvestigationActive || localCase.investigationNumber || localCase.investigationAuthority || localCase.investigationDefendantStatus || (localCase.detentionRenewals && localCase.detentionRenewals.length > 0) || (localCase.investigationProcedures && localCase.investigationProcedures.length > 0) || localCase.investigationNotes || ['جنح', 'جنايات', 'مستأنف', 'مخالفات', 'عسكري', 'أمن دولة', 'اقتصادية', 'تحقيق', 'حبس'].some(t => (localCase.type || '').includes(t) || (localCase.court || '').includes(t))) && (
...
function SessionsTab({ 
  ...
  const handleSaveDecision = async () => {
    ...
    const isDetention = !isExpert && (isDetentionSession(editingSession) || editingSession.isDetentionRenewal || editingSession.id.includes('session-detention'));
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <AddHearingModal
        ...
        onAddSession={async (newSess, detentionMeta) => {
          ...
          const updatedRenewals = detentionMeta?.isDetention ? [
          ...
          await onUpdateCase({
            ...localCase,
            isInvestigationActive: detentionMeta?.isDetention ? true : localCase.isInvestigationActive,
            nextHearingDate: newSess.date,
            nextHearingTime: newSess.time,
            detentionRenewals: detentionMeta?.isDetention ? updatedRenewals : localCase.detentionRenewals
          });
          setShowAddForm(false);
