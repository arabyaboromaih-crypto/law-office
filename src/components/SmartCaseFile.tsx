...
function OverviewTab({ 
  ...
  const handleSave = async () => {
    ...
    const updated: Case = {
      ...
      isInvestigationActive,
      investigationNumber: investigationNumber || undefined,
      investigationYear: investigationYear || undefined,
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm space-y-6">
        ...
        <div className="space-y-6">
          ...
          {(localCase.isInvestigationActive || localCase.investigationNumber || localCase.investigationAuthority || localCase.investigationDefendantStatus || (localCase.detentionRenewals && localCase.detentionRenewals.length > 0) || (localCase.investigationProcedures && localCase.investigationProcedures.length > 0) || localCase.investigationNotes) && (
              ...
              {localCase.detentionRenewals && localCase.detentionRenewals.length > 0 && (
                <div className="space-y-1.5 pt-1">
                  <span className="text-[11px] font-bold text-amber-950 block flex items-center gap-1">
                    <span>🔒 سجل قرارات تجديد الحبس الاحتياطي:</span>
                    <span className="text-[10px] bg-amber-200 text-amber-900 px-1.5 py-0.2 rounded font-bold">{localCase.detentionRenewals.length} قرار</span>
                  </span>
                    ...
                    <table className="w-full text-right text-[11px] border-collapse bg-white rounded-xl border border-amber-200/70">
                      <thead>
                        <tr className="bg-amber-100/60 text-amber-950 font-bold border-b border-amber-200/70">
                          ...
                          <th className="p-1.5">جهة التجديد</th>
                          <th className="p-1.5">التجديد القادم والملاحظات</th>
                        </tr>
                      </thead>
                      <tbody>
                        {localCase.detentionRenewals.map((ren, idx) => (
                          <tr key={ren.id || idx} className="border-b border-slate-100 last:border-0 hover:bg-amber-50/30 transition-colors">
      ...
      >
        ...
        <div className="bg-slate-50 border border-slate-150 rounded-2xl px-6 py-3 flex items-center justify-between gap-2 shrink-0 overflow-x-auto select-none mb-6" dir="rtl">
          {[
            ...
            { id: 'investigation', label: 'مرحلة التحقيق والنيابة', desc: 'التحقيق والتجديدات', icon: ShieldAlert }
          ].map((step, idx) => {
              ...
              <FormCard title="بيانات مرحلة التحقيق والتحقيقات الجنائية" icon={ShieldAlert}>
                <div className="bg-amber-50 border border-amber-200 p-4 rounded-2xl mb-4 space-y-3">
                  ...
                  <p className="text-[11px] text-amber-800 font-medium leading-relaxed">
                    عند التفعيل، سيتم إدراج كامل بيانات وقرارات ومحاضر النيابة العامة ضمن ملف القضية الشامل والتقرير المطبوع.
                  </p>
                </div>
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <AddHearingModal
        ...
        onAddSession={async (newSess, detentionMeta) => {
          await onAddSession(newSess);
          if (!localCase.nextHearingDate || new Date(newSess.date) > new Date()) {
            await onUpdateCase({
              ...localCase,
