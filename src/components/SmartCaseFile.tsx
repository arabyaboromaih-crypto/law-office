...
import { deduplicateSessions } from '../utils/hearingSync';
...
export default function SmartCaseFile({
  ...
  const caseSessions = deduplicateSessions(sessions.filter(s => s.caseId === localCase.id), [localCase])
      ...
      {showNoteModal && (
        <CaseNoteModal
          ...
          onSaveNote={async (note, updatedCase) => {
            ...
            if (note) {
              await logAction('إضافة ملاحظة', `تمت إضافة ملاحظة لملف القضية: ${note.text.substring(0, 35)}...`);
            }
          }}
          onDeleteNote={async (noteId, updatedCase) => {
            await onUpdateCase(updatedCase);
            setLocalCase(updatedCase);
            await logAction('حذف ملاحظة', 'تم حذف ملاحظة من ملف القضية');
          }}
        />
      )}
    </div>
...
function SessionsTab({ 
  ...
  const handleSaveDecision = async () => {
    ...
    const isExpert = isExpertSession(editingSession) || !!editingSession.isExpertSession || !!localCase.isReferredToExperts || !!localCase.expertReferral?.isReferred;
    let updatedExpertReferral = localCase.expertReferral ? { ...localCase.expertReferral } : (isExpert ? { isReferred: true, status: 'قيد مباشرة الخبير' } : undefined);
    
    if (isExpert && updatedExpertReferral) {
      let expSessions = updatedExpertReferral.sessions ? [...updatedExpertReferral.sessions] : [];
      let matchedExpIdx = -1;
      if (editingSession.expertSessionId && !editingSession.expertSessionId.startsWith('next-')) {
        matchedExpIdx = expSessions.findIndex(s => s.id === editingSession.expertSessionId);
      }
      if (matchedExpIdx === -1) {
        matchedExpIdx = expSessions.findIndex(s => s.date === editingSession.date);
      }

      const currentDecision = decision || '';
      const currentStatus = sessionStatus || (nextHearingDate ? 'postponed' : (currentDecision ? 'completed' : 'pending'));

      if (matchedExpIdx !== -1) {
        expSessions[matchedExpIdx] = {
          ...expSessions[matchedExpIdx],
          date: editingSession.date,
          decisionOrAction: currentDecision || expSessions[matchedExpIdx].decisionOrAction || '',
          nextSessionDate: nextHearingDate || expSessions[matchedExpIdx].nextSessionDate,
          status: currentStatus,
          location: outcomeCircuit || editingSession.circuit || expSessions[matchedExpIdx].location || updatedExpertReferral.expertOffice || 'مكتب الخبراء',
          notes: whatHappened || editingSession.notes || expSessions[matchedExpIdx].notes
        };
      } else {
        expSessions.push({
          id: (editingSession.expertSessionId && !editingSession.expertSessionId.startsWith('next-')) ? editingSession.expertSessionId : `exp-sess-${Date.now()}`,
          date: editingSession.date,
          time: editingSession.time || '10:00',
          sessionType: 'مناقشة ومباشرة',
          location: outcomeCircuit || editingSession.circuit || updatedExpertReferral.expertOffice || 'مكتب الخبراء',
          decisionOrAction: currentDecision,
          nextSessionDate: nextHearingDate || undefined,
          status: currentStatus,
          notes: whatHappened || 'جلسة خبراء'
        });
      }

      // If next hearing date is specified, ensure there is a corresponding pending session item for that future date
      if (nextHearingDate && nextHearingDate !== editingSession.date) {
        const alreadyHasNextDate = expSessions.some(s => s.date === nextHearingDate);
        if (!alreadyHasNextDate) {
          expSessions.push({
            ...
            status: 'pending',
            notes: `جلسة خبير قادمة مؤجلة بناءً على قرار جلسة ${editingSession.date}: ${currentDecision}`
          });
        }
      }
    ...
    const isDetention = !isExpert && !!localCase.isInvestigationActive && isDetentionSession(editingSession, localCase);
    let updatedRenewals = localCase.detentionRenewals ? [...localCase.detentionRenewals] : [];
    if (isDetention) {
      let matchedIdx = -1;
      if (editingSession.detentionRenewalId) {
        matchedIdx = updatedRenewals.findIndex(r => r.id === editingSession.detentionRenewalId);
      }
      if (matchedIdx === -1) {
        matchedIdx = updatedRenewals.findIndex(r => r.renewalDate === editingSession.date || r.date === editingSession.date || r.nextRenewalDate === editingSession.date);
      }
      ...
      if (matchedIdx !== -1) {
        updatedRenewals[matchedIdx] = {
          ...updatedRenewals[matchedIdx],
          date: editingSession.date,
          renewalDate: editingSession.date,
          decision: decision || updatedRenewals[matchedIdx].decision || 'تجديد حبس',
          nextRenewalDate: nextHearingDate || updatedRenewals[matchedIdx].nextRenewalDate || '',
          authority: outcomeCourt || editingSession.court || updatedRenewals[matchedIdx].authority || 'النيابة العامة',
          notes: whatHappened || updatedRenewals[matchedIdx].notes || ''
        };
      } else {
        updatedRenewals.push({
          id: editingSession.detentionRenewalId || `ren-${Date.now()}`,
          date: editingSession.date,
          renewalDate: editingSession.date,
          decision: decision || 'تجديد حبس',
          nextRenewalDate: nextHearingDate || '',
          authority: outcomeCourt || editingSession.court || 'النيابة العامة',
          durationDays: editingSession.detentionDurationDays || 15,
          notes: whatHappened || 'جلسة تجديد حبس'
        });
      }
    }

    if (isDetention) {
      const latestNextDate = nextHearingDate || (updatedRenewals.length > 0 ? updatedRenewals[updatedRenewals.length - 1].nextRenewalDate : undefined);
      await onUpdateCase({
        ...localCase,
        isInvestigationActive: localCase.isInvestigationActive,
        nextHearingDate: latestNextDate || localCase.nextHearingDate,
        detentionRenewals: updatedRenewals,
        status: decision ? `قرار التجديد: ${decision}` : localCase.status
      });
    } else if (isExpert) {
      const latestExpertNextDate = nextHearingDate || (updatedExpertReferral?.sessions && updatedExpertReferral.sessions.length > 0 ? updatedExpertReferral.sessions[updatedExpertReferral.sessions.length - 1].nextSessionDate : undefined);
      await onUpdateCase({
        ...localCase,
        isReferredToExperts: true,
        expertReferral: updatedExpertReferral,
        nextHearingDate: latestExpertNextDate || localCase.nextHearingDate,
        nextHearingTime: latestExpertNextDate ? '10:00' : localCase.nextHearingTime,
        status: nextHearingDate ? `مباشرة الخبير (تحددت جلسة ${nextHearingDate}): ${decision || ''}` : (decision ? `مباشرة الخبير: ${decision}` : localCase.status)
      });
    } else if (sessionStatus === 'postponed' && nextHearingDate) {
      await onUpdateCase({
        ...localCase,
        nextHearingDate,
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="flex items-center justify-end gap-2 border-t border-slate-100 pt-2.5 mt-3 pr-2">
                        ...
                        <a
                          href={directProxyUrl || file.fileUrl || file.downloadURL}
