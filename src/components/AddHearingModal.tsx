...
export const AddHearingModal: React.FC<AddHearingModalProps> = ({
  ...
  useEffect(() => {
    if (defaultCaseId) {
      ...
      if (!activeCases[0].isInvestigationActive) {
        setIsDetention(false);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const eff = getEffectiveStageInfo(targetCase);
      const effectiveIsDetention = isDetention && !!targetCase.isInvestigationActive;
      const renId = `ren-${Date.now()}`;
      const sessionId = effectiveIsDetention 
        ? `session-detention-prev-${targetCase.id}-${renId}`
        : `sess-${Date.now()}`;
      ...
      const newSess: HearingSession = {
        ...
        opponentName: effectiveIsDetention ? (targetCase.investigationAuthority || 'النيابة العامة') : targetCase.opponent.name,
        court: finalCourt || (effectiveIsDetention ? (detentionAuthority || 'النيابة العامة') : 'غير محدد'),
        circuit: circuit || (effectiveIsDetention ? 'غرفة المشورة / التجديدات' : 'غير محدد'),
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-5 min-h-0 text-slate-100 custom-scrollbar">
            ...
            <div className="bg-[#0E1D33] border-2 border-slate-700/80 rounded-2xl p-4 space-y-4 shadow-md">
              ...
              {/* Toggle 1: تجديد حبس احتياطي - يظهر حصرياً عند تفعيل مرحلة التحقيق في القضية */}
              {targetCase?.isInvestigationActive && (
                <>
