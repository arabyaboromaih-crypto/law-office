...
export default function CaseNoteModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNoteText('');
      setSuccessMessage('تم حفظ الملاحظة بنجاح بملف القضية.');
      setTimeout(() => {
        setSuccessMessage(null);
      }, 4000);
    } catch (err: any) {
      console.error('Error saving case note:', err);
      setErrorMessage('حدث خطأ أثناء حفظ الملاحظة في قاعدة البيانات. يرجى المحاولة مرة أخرى.');
    } finally {
      setIsSaving(false);
    }
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="flex items-center justify-end gap-2.5 pt-1">
              ...
              <button
                ...
                title="حفظ الملاحظة بملف القضية في Firestore"
              >
                {isSaving ? (
                  <>
                    <Loader2 className="w-4 h-4 animate-spin text-slate-950" />
                    <span>جاري حفظ الملاحظة بالملف...</span>
                  </>
                ) : (
                  <>
