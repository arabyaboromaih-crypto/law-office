...
export default function CasesPanel({ 
  ...
  useEffect(() => {
    if (selectedCaseId && cases.length > 0) {
      ...
      const targetCase = cases.find(c => c.id === externalSearchQuery);
      if (targetCase) {
        setShowDocViewer(targetCase);
        setDocViewerTab('docs');
      }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const caseData: Case = {
      ...
      isInvestigationActive: !!isInvestigationActive,
      investigationNumber: isInvestigationActive ? finalInvestigationNumber : undefined,
      investigationYear: isInvestigationActive ? finalInvestigationYear : undefined,
      investigationAuthority: isInvestigationActive ? (investigationAuthority || undefined) : undefined,
      investigationStartDate: isInvestigationActive ? (investigationStartDate || undefined) : undefined,
      investigationDefendantStatus: isInvestigationActive ? (investigationDefendantStatus || undefined) : undefined,
      detentionRenewals: isInvestigationActive ? (detentionRenewals || []).map((r, idx) => ({
      ...
      investigationNotes: isInvestigationActive ? (investigationNotes || undefined) : undefined,
      investigationProcedures: isInvestigationActive ? (editingCase ? (editingCase.investigationProcedures || []) : []) : []
    };
                      ...
                      <FormGrid cols={4}>
                        <FormField label="اختر موكل مسجل بالنظام (اختياري)">
                          <select
                            ...
                            className={`w-full px-3 py-1.5 bg-white border rounded-lg text-xs transition-all ${
          ...
          {activeFormTab === 'attachments' && (
            <div className="space-y-6">
              ...
              <div className="sticky bottom-0 bg-white/95 backdrop-blur-md p-4 sm:p-5 border-t border-slate-200/90 flex items-center justify-between rounded-2xl shrink-0 z-20 shadow-sm mt-6" dir="rtl">
                ...
                <div>
                  {activeFormTab !== 'investigation' && (activeFormTab !== 'judicial' || isInvestigationActive) ? (
                    <button
                      type="button"
                      onClick={() => {
                        const tabs: ('investigation' | 'judicial' | 'litigants' | 'financials' | 'attachments')[] = ['investigation', 'judicial', 'litigants', 'financials', 'attachments'];
                ...
                <div className="flex items-center gap-3">
                  ...
                  {activeFormTab !== 'attachments' && (
                    <button
                      type="submit"
                      className="px-5 py-2.5 bg-amber-500 hover:bg-amber-600 text-slate-950 text-xs sm:text-sm rounded-xl font-black transition-all shadow-sm hover:shadow-md active:scale-95 flex items-center gap-1.5 cursor-pointer"
                    >
                  ...
                  {activeFormTab === 'attachments' && (
                    <button
                      type="submit"
                      className="px-6 py-2.5 bg-amber-500 hover:bg-amber-600 text-slate-950 text-xs sm:text-sm rounded-xl font-black transition-all shadow-md hover:shadow-lg active:scale-95 flex items-center gap-2 cursor-pointer"
                    >
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 mb-1.5">
                  {isExpertSession(decisionSession)
                    ...
                    : isDetentionSession(decisionSession)
