...
import { Case, Client, CaseType, LitigationDegree, User, CaseFile, Company, CompanyPartner, CompanyDoc, HearingSession, LegalTask, CaseClient, Opponent, DetentionRenewalRecord, CaseNote } from '../types';
...
import { 
  ...
  Building2, Phone, Mail, MapPin, UserPlus, Users, FileUp, AlertTriangle, AlertCircle, MessageSquare, Upload, RefreshCw,
  ZoomIn, ZoomOut, RotateCw, Maximize2, Minimize2, ChevronRight, ChevronLeft, ChevronDown, ChevronUp,
  ArrowRight, Globe, FolderOpen, Briefcase, Link, Clock, Lock
} from 'lucide-react';
import { 
  BaseModal, FormCard, SectionHeader, FormGrid, FormField, 
  PrimaryButton, SecondaryButton, DangerButton 
} from './FormComponents';
import { suggestHearingSession, checkIfRelatesToHearing, extractHearingDate, getClosestUpcomingSession } from '../utils/hearingSync';
...
import { generateCaseReportHTML } from '../utils/caseReportGenerator';
import { 
  generateAllCasesComprehensiveReportHTML, 
  generateAllCompaniesComprehensiveReportHTML, 
  openPrintReportWindow 
} from '../utils/comprehensivePdfReportGenerator';
...
interface CasesPanelProps {
  ...
  users: User[];
  currentUser: User;
  onAddCase: (c: Case) => Promise<void> | void;
  ...
  onUpdateClient?: (c: Client) => Promise<void> | void;
          ...
          <div className="overflow-y-auto max-h-44 divide-y divide-slate-50">
            {filtered.length === 0 ? (
              <div className="p-3 text-center">
                <p className="text-slate-400 text-[11px] mb-2 font-medium">لا توجد صفة مطابقة بالضبط</p>
                {search.trim() && (
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className="w-full py-1.5 px-3 bg-amber-500 hover:bg-amber-600 text-slate-950 rounded-lg text-xs font-black transition-colors cursor-pointer flex items-center justify-center gap-1"
                  >
                    <Plus className="w-3.5 h-3.5" />
                    <span>استخدام: "{search.trim()}" كصفة بالدعوى</span>
                  </button>
                )}
              </div>
            ) : (
              <>
                ...
                {search.trim() && !options.includes(search.trim()) && (
                  <div
                    ...
                    className="px-3 py-2 text-xs cursor-pointer transition-colors bg-amber-50 hover:bg-amber-100 text-amber-900 font-bold border-t border-amber-200 flex items-center justify-between"
                  >
                    <span>استخدام: "{search.trim()}" كصفة بالدعوى</span>
...
export default function CasesPanel({ 
  ...
  sessions = [], onAddSession, onUpdateSession, onDeleteSession, tasks = [], onNavigateToTab,
  externalSearchQuery, onClearExternalSearch, selectedCaseId, onClearSelectedCaseId,
  ...
  // Effect to automatically open case document viewer when navigated with a specific caseId
  useEffect(() => {
    if (selectedCaseId && cases.length > 0) {
      const targetCase = cases.find(c => c.id === selectedCaseId);
      if (targetCase) {
        setShowDocViewer(targetCase);
        setDocViewerTab('docs');
      }
    } else if (externalSearchQuery && cases.length > 0) {
      const targetCase = cases.find(c => c.id === externalSearchQuery || c.caseNumberFirstInstance === externalSearchQuery);
      if (targetCase) {
        setShowDocViewer(targetCase);
        setDocViewerTab('docs');
      }
    }
  }, [selectedCaseId, externalSearchQuery, cases]);
  ...
  const [onlyInvestigationFilter, setOnlyInvestigationFilter] = useState<boolean>(false);
  ...
  const [activeFormTab, setActiveFormTab] = useState<'investigation' | 'judicial' | 'litigants' | 'financials' | 'attachments'>('investigation');
  ...
  const [documentsModalCase, setDocumentsModalCase] = useState<Case | null>(null);
  const [noteModalCase, setNoteModalCase] = useState<Case | null>(null);
  const [viewingCaseFile, setViewingCaseFile] = useState<{ file: CaseFile; caseData?: Case } | null>(null);
  ...
  // Back button handlers for modals and viewers
  useBackHandler(showFormModal, () => {
    setShowFormModal(false);
    setEditingCase(null);
  });
  useBackHandler(!!showDocViewer, () => setShowDocViewer(null));
  useBackHandler(!!showReportViewer, () => setShowReportViewer(null));
  useBackHandler(!!detentionModalCase, () => setDetentionModalCase(null));
  useBackHandler(!!documentsModalCase, () => setDocumentsModalCase(null));
  useBackHandler(!!noteModalCase, () => setNoteModalCase(null));
  useBackHandler(showCompanyModal, () => setShowCompanyModal(false));
  useBackHandler(showNewClientForm, () => setShowNewClientForm(false));
  useBackHandler(showAddSessionForm, () => setShowAddSessionForm(false));
  useBackHandler(!!archiveTarget, () => setArchiveTarget(null));
  useBackHandler(!!deleteTarget, () => setDeleteTarget(null));
  useBackHandler(!!decisionSession, () => setDecisionSession(null));
  useBackHandler(!!editingCompany, () => setEditingCompany(null));
  useBackHandler(!!archiveCoTarget, () => setArchiveCoTarget(null));
  useBackHandler(!!viewDocsCompany, () => setViewDocsCompany(null));
  useBackHandler(!!deleteCompanyTarget, () => setDeleteCompanyTarget(null));
  ...
  const [noHearingSpecified, setNoHearingSpecified] = useState(false);
  ...
  // Investigation stage state
  const [isInvestigationActive, setIsInvestigationActive] = useState<boolean>(false);
  const [investigationNumber, setInvestigationNumber] = useState<string>('');
  const [investigationYear, setInvestigationYear] = useState<string>('2025');
  const [investigationAuthority, setInvestigationAuthority] = useState<string>('');
  const [investigationStartDate, setInvestigationStartDate] = useState<string>('');
  const [investigationDefendantStatus, setInvestigationDefendantStatus] = useState<string>('محبوس احتياطياً');
  const [detentionRenewals, setDetentionRenewals] = useState<DetentionRenewalRecord[]>([]);
  const [investigationNotes, setInvestigationNotes] = useState<string>('');
  const [showCancelInvestigationConfirm, setShowCancelInvestigationConfirm] = useState<boolean>(false);
  ...
  // Manual Client Modal state (إضافة موكل يدويًا من نافذة القضية)
  const [showManualClientModal, setShowManualClientModal] = useState(false);
  const [manualClientTargetIndex, setManualClientTargetIndex] = useState<number>(0);
  const [manualClientName, setManualClientName] = useState('');
  const [manualClientRole, setManualClientRole] = useState('مدعي');
  const [manualClientNationalId, setManualClientNationalId] = useState('');
  const [manualClientPhone, setManualClientPhone] = useState('');
  const [manualClientSecondaryPhone, setManualClientSecondaryPhone] = useState('');
  const [manualClientJob, setManualClientJob] = useState('');
  const [manualClientAddress, setManualClientAddress] = useState('');
  const [manualClientEmail, setManualClientEmail] = useState('');
  const [manualClientNotes, setManualClientNotes] = useState('');
  const [manualClientCompanyId, setManualClientCompanyId] = useState('');
  const [isSavingManualClient, setIsSavingManualClient] = useState(false);
  const [manualClientSuccessMsg, setManualClientSuccessMsg] = useState('');
  ...
  useBackHandler(showManualClientModal, () => setShowManualClientModal(false));
  ...
  const filteredCases = activeCases.filter(c => {
    ...
    const matchesInvestigation = !onlyInvestigationFilter || (c.isInvestigationActive || c.status === 'قيد التحقيق');

    return matchesSearch && matchesType && matchesDegree && matchesCourt && matchesInvestigation;
  });
  ...
  const filteredCompanies = activeCompanies.filter(co => {
    ...
    const matchesSearch = 
      (co.name || '').includes(q) ||
      (co.commercialRegister || '').includes(q) ||
      (co.taxCard || '').includes(q) ||
      (co.activityType || '').includes(q);
  ...
  const handleAddPartner = () => {
    ...
    if (registerPartnerAsClient) {
      ...
      const existingCl = clients.find(c => c && ((partPhone && c.phone === partPhone) || c.name === partName));
      if (existingCl) {
        if (onUpdateClient) {
  ...
  const handleLinkPartnerAsClient = (partner: CompanyPartner) => {
    const targetCompanyId = currentCompanyId || (editingCompany ? editingCompany.id : `company-${Date.now()}`);
    const existingCl = clients.find(c => c && ((partner?.phone && c.phone === partner.phone) || c.name === partner?.name));
    if (existingCl) {
      if (onUpdateClient) {
        onUpdateClient({ ...existingCl, companyId: targetCompanyId });
        if (onAddAuditLog) {
  ...
  // View Company Document via direct in-app popup modal
  const handleViewCompanyDoc = (doc: CompanyDoc, _company?: Company) => {
    setViewingCaseFile({ file: doc as any });
  };
  ...
  const handleCompanySubmit = (e: React.FormEvent) => {
    ...
    if (coPartners.length > 0) {
      ...
      const existingCl = clients.find(c => c && ((firstPartner?.phone && c.phone === firstPartner.phone) || c.name === firstPartner?.name));
      if (existingCl) {
  ...
  const handleOpenAdd = () => {
    ...
    setInvestigationYear('2025');
    setInvestigationAuthority('');
    setInvestigationStartDate('');
    setInvestigationDefendantStatus('محبوس احتياطياً');
    setDetentionRenewals([]);
    setInvestigationNotes('');
    setShowCancelInvestigationConfirm(false);
    ...
    setExpertNotes('');
    setActiveFormTab('investigation');
  ...
  const handleOpenEdit = (c: Case) => {
    ...
    const isNoHearing = !c.nextHearingDate || !!c.noUpcomingHearing;
    setNoHearingSpecified(isNoHearing);
    if (isNoHearing) {
      setNextHearing('');
      setNextHearingTime('09:00');
    } else {
      const upcoming = getClosestUpcomingSession(c, sessions);
      const resolvedNextHearing = upcoming.hasUpcoming ? (upcoming.date || '') : (c.nextHearingDate || '');
      const resolvedNextHearingTime = upcoming.hasUpcoming ? (upcoming.time || '09:00') : (c.nextHearingTime || '09:00');
      setNextHearing(resolvedNextHearing);
      setNextHearingTime(resolvedNextHearingTime);
    }
    setStatus(c.status);
    setClientName(c.clientName);
    const matchedClient = clients.find(cl => cl && (cl.id === c.clientId || cl.name === c.clientName));
    ...
    const opp = c.opponent || { name: '', role: 'مدعى عليه', address: '', lawyer: '', phone: '', lawyerPhone: '', notes: '' };
    setOppName(opp?.name || '');
    setOppRole(opp?.role || 'مدعى عليه');
    setOppAddress(opp?.address || '');
    setOppLawyer(opp?.lawyer || '');
    setOppPhone(opp?.phone || '');
    setOppLawyerPhone(opp?.lawyerPhone || '');
    
    const loadedOpponents: Opponent[] = c.opponentsList && c.opponentsList.length > 0
      ? c.opponentsList
      : [{
          name: opp?.name || '',
          role: opp?.role || 'مدعى عليه',
          address: opp?.address || '',
          lawyer: opp.lawyer || '',
          phone: opp.phone || '',
          lawyerPhone: opp.lawyerPhone || '',
          notes: opp.notes || ''
        }];
    ...
    setIsInvestigationActive(!!c.isInvestigationActive);
    setInvestigationNumber(c.investigationNumber || '');
    setInvestigationYear(c.investigationYear || '2025');
    setInvestigationAuthority(c.investigationAuthority || '');
    setInvestigationStartDate(c.investigationStartDate || '');
    setInvestigationDefendantStatus(c.investigationDefendantStatus || 'محبوس احتياطياً');
    setDetentionRenewals(c.detentionRenewals || []);
    setInvestigationNotes(c.investigationNotes || '');
    ...
    setActiveFormTab(c.isInvestigationActive ? 'investigation' : 'judicial');
    setShowFormModal(true);
  };
  ...
  // Open Manual Client Addition Modal
  const handleOpenManualClientModal = (targetIndex: number = 0) => {
    setManualClientTargetIndex(targetIndex);
    const existingRole = (targetIndex >= 0 && formClients[targetIndex]?.role) ? formClients[targetIndex].role : 'مدعي';
    const existingName = (targetIndex >= 0 && formClients[targetIndex]?.name) ? formClients[targetIndex].name : '';
    setManualClientName(existingName);
    setManualClientRole(existingRole || 'مدعي');
    setManualClientNationalId('');
    setManualClientPhone((targetIndex >= 0 && formClients[targetIndex]?.phone) || '');
    setManualClientSecondaryPhone('');
    setManualClientJob('');
    setManualClientAddress('');
    setManualClientEmail((targetIndex >= 0 && formClients[targetIndex]?.email) || '');
    setManualClientNotes('');
    setManualClientCompanyId('');
    setShowManualClientModal(true);
  };

  // Save Manual Client to Database & Link to Current Case
  const handleSaveManualClient = async (e?: React.FormEvent) => {
    if (e) e.preventDefault();
    if (isSavingManualClient) return;

    const trimmedName = manualClientName.trim();
    if (!trimmedName) {
      alert('⚠️ يرجى إدخال اسم الموكل بالكامل.');
      return;
    }

    const trimmedPhone = manualClientPhone.trim();

    let finalClientNationalId = manualClientNationalId.trim();
    if (finalClientNationalId.length > 0) {
      const { isValid, normalizedValue } = validateNationalId(finalClientNationalId);
      if (!isValid) {
        alert('⚠️ الرقم القومي يجب أن يتكون من 14 رقمًا صحيحًا.');
        return;
      }
      finalClientNationalId = normalizedValue;
    }

    // Duplicate check: check if an existing client has the same national ID OR same name and phone to prevent duplicate records
    const existingClient = (clients || []).find(c => {
      if (!c) return false;
      if (finalClientNationalId && c.nationalId) {
        const { normalizedValue: existingNatId } = validateNationalId(c.nationalId);
        if (existingNatId && existingNatId === finalClientNationalId) return true;
        if (c.nationalId.trim() === finalClientNationalId) return true;
      }
      if (trimmedPhone && c.phone && c.phone.trim() === trimmedPhone && (c.name || '').trim() === trimmedName) {
        return true;
      }
      if (!finalClientNationalId && !trimmedPhone && (c.name || '').trim() === trimmedName) {
        return true;
      }
      return false;
    });

    setIsSavingManualClient(true);
    try {
      const targetClientId = existingClient ? existingClient.id : `client-${Date.now()}`;
      const clientObj: Client = {
        id: targetClientId,
        name: trimmedName,
        nationalId: finalClientNationalId || existingClient?.nationalId || undefined,
        phone: trimmedPhone || existingClient?.phone || '',
        secondaryPhone: manualClientSecondaryPhone.trim() || existingClient?.secondaryPhone || undefined,
        email: manualClientEmail.trim() || existingClient?.email || undefined,
        address: manualClientAddress.trim() || existingClient?.address || '',
        job: manualClientJob.trim() || existingClient?.job || '',
        notes: manualClientNotes.trim() || existingClient?.notes || undefined,
        companyId: manualClientCompanyId || existingClient?.companyId || undefined
      };

      // Save to Firestore database & local state
      if (existingClient && onUpdateClient) {
        await onUpdateClient(clientObj);
      } else if (onAddClient) {
        await onAddClient(clientObj);
      } else {
        throw new Error('خدمة حفظ الموكل غير متاحة حالياً');
      }

      if (onAddAuditLog && currentUser) {
        onAddAuditLog(currentUser, existingClient ? 'edit' : 'add', `إضافة موكل يدويًا من نافذة القضية: ${clientObj.name}${clientObj.phone ? ` - هاتف: ${clientObj.phone}` : ''}`);
      }

      // Automatically link to current case form
      const newCaseClient: CaseClient = {
        name: clientObj.name,
        phone: clientObj.phone,
        email: clientObj.email || '',
        id: clientObj.id,
        role: manualClientRole || 'مدعي'
      };

      if (manualClientTargetIndex >= 0 && manualClientTargetIndex < formClients.length) {
        updateFormClient(manualClientTargetIndex, {
          name: clientObj.name,
          phone: clientObj.phone,
          email: clientObj.email || '',
          id: clientObj.id,
          role: manualClientRole || formClients[manualClientTargetIndex]?.role || 'مدعي'
        });
      } else {
        // Quick action or appended client
        if (formClients.length === 1 && (!formClients[0]?.name || !formClients[0].name.trim())) {
          updateFormClient(0, {
            name: clientObj.name,
            phone: clientObj.phone,
            email: clientObj.email || '',
      ...
      if (manualClientTargetIndex <= 0 || !clientName.trim()) {
      ...
      // ONLY after real successful database execution: show exact success notification
      showToast('تم حفظ الموكل بنجاح', 'success');
      setManualClientSuccessMsg('تم حفظ الموكل بنجاح');
      setTimeout(() => setManualClientSuccessMsg(''), 5000);
    } catch (err: any) {
      console.error('Failed to save manual client in database:', err);
      showToast(`فشل حفظ الموكل في قاعدة البيانات: ${err?.message || 'يرجى المحاولة مرة أخرى'}`, 'error');
      alert('⚠️ حدث خطأ أثناء حفظ الموكل في قاعدة البيانات. يرجى التحقق من الاتصال والمحاولة مرة أخرى.');
    } finally {
      setIsSavingManualClient(false);
    }
  };
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    const validClients = formClients.filter(cl => cl && cl.name && cl.name.trim() !== '');
    if (validClients.length === 0) {
      setActiveFormTab('litigants');
      alert('⚠️ تنبيه هام: يرجى تحديد واختيار الموكل (أطراف الدعوى) للتمكن من حفظ القضية وتسجيلها.');
      return;
    }

    // Filter out empty opponents
    const validOpponents = formOpponents.filter(opp => opp && opp.name && opp.name.trim() !== '');
    if (validOpponents.length === 0) {
      setActiveFormTab('litigants');
      alert('⚠️ تنبيه هام: يرجى إضافة واختيار اسم الخصم (أطراف الدعوى) للتمكن من حفظ القضية وتسجيلها.');
      return;
    }
    ...
    try {
      ...
      const selectedClientId = clients.find(cl => cl && cl.name === firstClient?.name)?.id || firstClient?.id || `client-${Date.now()}`;
      const primaryClientName = firstClient?.name || 'موكل';
      ...
      // Calculate effective litigation degree automatically from active/registered stage
      const autoDegree = computeCaseDegree(
        {
          caseNumberSecondInstance: caseNo2nd,
          courtSecondInstance: court2nd,
          cassationNumber: cassationNumber,
          courtCassation: courtCass,
      ...
      if (autoDegree === 'نقض') {
        primaryCourt = courtCass || 'محكمة النقض';
        primaryCircuit = circuitCass || 'دائرة النقض';
      } else if (autoDegree === 'استئناف') {
        primaryCourt = court2nd || 'محكمة الاستئناف';
        primaryCircuit = circuit2nd || 'دائرة الاستئناف';
      } else if (autoDegree === 'تحقيق') {
        primaryCourt = investigationAuthority || court1st || 'جهة التحقيق / النيابة العامة';
        primaryCircuit = 'غرفة المشورة / النيابة';
      } else {
        primaryCourt = court1st || court || 'محكمة أول درجة';
      ...
      const finalCaseNo1st = (caseNo1st || '').trim() || (isInvestigationActive ? ((investigationNumber || '').trim() || 'قيد القيد') : 'قيد القيد');
      const finalCaseYear1st = (caseYear1st || '').trim() || new Date().getFullYear().toString();
      const finalInvestigationNumber = (investigationNumber || '').trim() || editingCase?.investigationNumber || (isInvestigationActive ? 'قيد القيد' : undefined);
      const finalInvestigationYear = editingCase?.investigationYear || undefined;
      ...
      if (!noHearingSpecified) {
        ...
        } else if (editingCase && (!nextHearing || nextHearing === editingCase.nextHearingDate)) {
          const upcoming = getClosestUpcomingSession(editingCase, sessions);
          if (upcoming.hasUpcoming && upcoming.date) {
            finalNextHearing = upcoming.date;
          }
        }
      }
      ...
      const caseData: Case = {
        ...
        detentionRenewals: (detentionRenewals && detentionRenewals.length > 0)
          ...
          : (editingCase?.detentionRenewals || []),
        investigationNotes: investigationNotes || editingCase?.investigationNotes || undefined,
        investigationProcedures: editingCase?.investigationProcedures || []
      };

      // Prevent duplicate case creation (only if case number is meaningful and not a placeholder)
      if (!editingCase && finalCaseNo1st && finalCaseNo1st.trim() !== 'قيد القيد' && finalCaseNo1st.trim() !== 'غير محدد') {
        const duplicateCase = cases.find(c => 
          !c.isArchived &&
          (c.caseNumberFirstInstance || '').trim() === finalCaseNo1st.trim() &&
          (c.caseYearFirstInstance || '').trim() === finalCaseYear1st.trim() &&
          (c.court || '').trim() === (primaryCourt || '').trim()
        );
        if (duplicateCase) {
          alert(`⚠️ تنبيه: توجد قضية مسجلة مسبقاً بنفس رقم وتاريخ أول درجة (${finalCaseNo1st} لسنة ${finalCaseYear1st} أمام ${primaryCourt}).\nيرجى مراجعة ملف القضية المسجل لتجنب تكرار القضايا.`);
          return;
        }
      }

      if (editingCase) {
        await onUpdateCase(caseData);
      } else {
        await onAddCase(caseData);
      }

      // ONLY after confirmed database execution: show exact success notification
      showToast('تم حفظ القضية بنجاح', 'success');
      setShowFormModal(false);
    } catch (err: any) {
      console.error('Failed to save case in database:', err);
      showToast(`فشل حفظ القضية في قاعدة البيانات: ${err?.message || 'يرجى المحاولة مرة أخرى'}`, 'error');
      alert('⚠️ حدث خطأ أثناء حفظ القضية في قاعدة البيانات. يرجى التحقق من الاتصال والمحاولة مرة أخرى.');
    } finally {
  ...
  const handleAddSessionFromViewer = (c: Case) => {
    ...
    const eff = getEffectiveStageInfo(c);

    const newSession: HearingSession = {
      id: `session-${Date.now()}`,
      caseId: c.id,
      caseNumber: eff.caseNumber || c.caseNumberFirstInstance,
      caseYear: eff.caseYear || c.caseYearFirstInstance,
      clientName: c.clientName,
      opponentName: c.opponent?.name || 'غير محدد',
      court: eff.court || c.court || 'غير محدد',
      circuit: eff.circuit || c.circuit || 'غير محدد',
      type: c.type,
      date: newSessDate,
  ...
  // Direct access to proxy URL upon clicking, with modal fallback
    ...
    if (file.id) {
      getFileFromIndexedDB(file.id).then(blob => {
        if (blob) {
          ...
          setViewingCaseFile({ file, caseData: editingCase || undefined });
        }
      }).catch(() => {
        setViewingCaseFile({ file, caseData: editingCase || undefined });
      });
          ...
          <div class="cover-page">
            ...
            <table class="cover-meta-table">
              ...
              <tr>
                ...
                <td><strong>${c.clientsList && c.clientsList.length > 0 ? c.clientsList.map(cl => `${cl?.name || ''} (${cl?.role || 'موكل'})`).join(' - ') : (c.clientName || 'غير محدد')}</strong></td>
              </tr>
              <tr>
                <td class="label">الخصوم</td>
                <td><strong>${c.opponentsList && c.opponentsList.length > 0 ? c.opponentsList.map(opp => `${opp?.name || ''} (${opp?.role || 'خصم'})`).join(' - ') : `${c.opponent?.name || 'غير محدد'} (${c.opponent?.role || 'خصم'})`}</strong></td>
              </tr>
              <tr>
                  ...
                  ` : `
                    <div class="doc-placeholder-text">
                      ...
                      ` : file.category === 'صحف الدعاوى' ? `
                        <p>نرفق صحيفة وعريضة افتتاحه القضية المقيدة والمودعة رسمياً بقلم كتاب محكمة ${c.court}، لإعلان الخصم ${c.opponent?.name || 'الطرف الآخر'} بالمثول والرد على صحف الطلبات المدونة بالعريضة.</p>
                      ` : `
  ...
  // Direct print of a beautifully detailed PDF Report (Navy & Gold Theme)
  const handlePrintDetailedReport = (c: Case) => {
    ...
    const htmlContent = generateCaseReportHTML(c, clients, users, caseSessions, {
  ...
  // Handler for comprehensive PDF registry file printing for Cases or Companies
  const handlePrintComprehensivePDF = () => {
    if (activeSubTab === 'cases') {
      const htmlContent = generateAllCasesComprehensiveReportHTML(
        activeCases,
        clients,
        users,
        sessions || [],
      ...
      openPrintReportWindow(htmlContent);
    } else {
      const htmlContent = generateAllCompaniesComprehensiveReportHTML(
        activeCompanies,
        activeCases,
        users,
        sessions || [],
...
- الجلسة القادمة المقررة: ${(() => {
  const up = getClosestUpcomingSession(c, sessions);
  return up.hasUpcoming ? `${up.date}${up.time ? ` @ ${up.time}` : ''}` : 'لم تحدد بعد';
})()}
...
سيتم الآن فتح تطبيق WhatsApp Web أو تطبيق الواتساب لإرسال التقرير والرسالة التعريفية تلقائياً للموكل (${matchedClient?.name || c.clientName || 'الموكل'}) على الرقم: ${matchedClient?.phone || cleanPhone || 'المسجل'}`);
  };
  ...
  return (
    <div className="space-y-3.5">
      ...
      {returnToClient && (
        <div className="bg-slate-900 border border-amber-500/30 p-4 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4 shadow-xl text-right animate-fadeIn" dir="rtl">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="text-xs font-black text-white">أنت تستعرض ملفات مرتبطة بالموكل: {returnToClient?.name || 'الموكل'}</h4>
      ...
      {/* Sub tabs: Cases or Companies & PDF Print Button */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 w-full">
        ...
        {/* زر طباعة ملف PDF بجوار التبويبين */}
        <button
          onClick={handlePrintComprehensivePDF}
          title={activeSubTab === 'cases' ? 'طباعة ملف وسجل القضايا الشامل بصيغة PDF' : 'طباعة ملف وسجل الشركات الشامل بصيغة PDF'}
          className="px-5 py-2.5 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 hover:from-slate-800 hover:to-slate-700 text-amber-400 hover:text-amber-300 border border-amber-500/40 hover:border-amber-400 rounded-2xl font-black text-xs shadow-md hover:shadow-lg transition-all flex items-center gap-2.5 cursor-pointer group active:scale-95 self-start sm:self-auto"
        >
          <div className="w-6 h-6 rounded-lg bg-amber-500/10 border border-amber-500/30 flex items-center justify-center text-amber-400 group-hover:scale-110 group-hover:bg-amber-500/20 transition-all">
            <Printer className="w-3.5 h-3.5" />
          </div>
          <span className="text-xs sm:text-sm font-black">طباعة ملف PDF</span>
          <span className="text-[10px] sm:text-[11px] px-2.5 py-0.5 rounded-full bg-amber-500/20 text-amber-300 font-bold border border-amber-500/30">
            {activeSubTab === 'cases' ? 'ملف القضايا الشامل' : 'ملف الشركات الشامل'}
          </span>
        </button>
      </div>
          ...
          <div className="bg-gradient-to-b from-slate-50 to-white border-t-4 border-t-amber-500 border border-slate-200 rounded-2xl p-5 shadow-sm animate-fadeIn">
            <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <button
                  type="button"
                  onClick={() => setOnlyInvestigationFilter(!onlyInvestigationFilter)}
                  className={`px-3.5 py-2.5 rounded-xl text-xs font-black flex items-center gap-1.5 transition-all cursor-pointer ${
                    onlyInvestigationFilter
                      ? 'bg-amber-500 text-slate-950 shadow-md shadow-amber-500/20'
                      : 'bg-slate-100/60 border border-slate-200 text-slate-700 hover:bg-slate-200/60'
                  }`}
                  title="تصفية قضايا مرحلة التحقيق والحبس الاحتياطي"
                >
                  <ShieldAlert className={`w-4 h-4 ${onlyInvestigationFilter ? 'text-slate-950' : 'text-amber-600'}`} />
                  <span>قضايا التحقيق والتجديد</span>
                </button>
          ...
          {filteredCases.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {filteredCases.map((c) => {
                ...
                if (eff.degreeLabel === 'استئناف') {
                  cardAccentBorder = 'border-r-4 border-r-sky-600';
                } else if (eff.degreeLabel === 'نقض') {
                  cardAccentBorder = 'border-r-4 border-r-indigo-700';
                } else if (eff.degreeLabel === 'تحقيق') {
                  cardAccentBorder = 'border-r-4 border-r-rose-600';
                }
                ...
                // Dynamically resolve closest actual upcoming session directly from the Sessions Agenda (single source of truth)
                const upcomingInfo = getClosestUpcomingSession(c, sessions);
                const hasNextHearing = upcomingInfo.hasUpcoming;
                const nextHearingDate = upcomingInfo.date;
                const nextHearingTime = upcomingInfo.time;
                const isExpertNext = upcomingInfo.isExpert;
                const isDetentionNext = upcomingInfo.isDetention;
                const nowForHearing = new Date();
                const todayStrForHearing = `${nowForHearing.getFullYear()}-${String(nowForHearing.getMonth() + 1).padStart(2, '0')}-${String(nowForHearing.getDate()).padStart(2, '0')}`;
                const isPastHearing = nextHearingDate ? nextHearingDate < todayStrForHearing : false;
                  ...
                  >
                    ...
                    {(currentUser.role === 'admin' || currentUser.role === 'lawyer' || currentUser.permissions.deleteCase) && (
                      <button
                        ...
                        className="absolute top-4 left-4 w-7 h-7 flex items-center justify-center text-red-500 hover:text-red-700 bg-red-50/80 hover:bg-red-100 border border-red-200/60 rounded-full transition-all cursor-pointer z-10 shadow-3xs"
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                        <div className="flex flex-wrap items-center gap-1.5">
                          ...
                          {c.officeFileNo && (
                            <span className="bg-amber-500/10 text-amber-900 text-[10px] px-2 py-0.5 rounded-lg font-black flex items-center gap-1.5 border border-amber-500/20 shadow-3xs">
                              <span className="w-4 h-4 rounded-full bg-amber-500/20 border border-amber-500/30 flex items-center justify-center shrink-0">
                                <FolderOpen className="w-2.5 h-2.5 text-amber-700" />
                              </span>
                              {c.officeFileNo}
                            </span>
                          )}
                          {c.isInvestigationActive && (
                            <span className="bg-amber-500/15 text-amber-950 text-[10px] px-2 py-0.5 rounded-lg font-black flex items-center gap-1.5 border border-amber-500/30 animate-pulse">
                              <span className="w-4 h-4 rounded-full bg-amber-500/25 border border-amber-500/40 flex items-center justify-center shrink-0">
                                <ShieldAlert className="w-2.5 h-2.5 text-amber-800" />
                              </span>
                              مرحلة التحقيق
                            </span>
                          )}
                      ...
                      <div className="space-y-2">
                        <div className="flex items-start gap-2.5">
                          <div className="w-6 h-6 rounded-full bg-amber-500/10 border border-amber-500/25 text-amber-700 flex items-center justify-center shrink-0 mt-0.5 shadow-3xs">
                        ...
                        <div className="bg-slate-50 border border-slate-150 p-3 rounded-xl text-xs space-y-2">
                          <div className="flex items-center gap-2 text-slate-700">
                            <span className="w-5 h-5 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-800 flex items-center justify-center shrink-0 text-[10px]">🏛️</span>
                            <span className="font-black text-slate-800">{eff.court}</span>
                          </div>
                          <div className="flex items-center gap-2 text-slate-500 text-[11px]">
                            <span className="w-5 h-5 rounded-full bg-slate-200/70 border border-slate-300/60 text-slate-700 flex items-center justify-center shrink-0 text-[10px]">⚖️</span>
                            <span>الدائرة المختصة: <strong className="text-slate-700 font-extrabold">{eff.circuit}</strong></span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-slate-100/40 border border-slate-200/60 rounded-xl p-3.5 space-y-3.5 text-xs">
                        ...
                        <div className="space-y-1.5">
                          <div className="flex items-center gap-2 text-emerald-800 font-black">
                            <span className="w-5 h-5 rounded-full bg-emerald-500/15 border border-emerald-300 flex items-center justify-center shrink-0 text-emerald-700">
                          ...
                          <div className="mr-3.5 space-y-1.5">
                            {c.clientsList && c.clientsList.length > 0 ? (
                              c.clientsList.map((cl, idx) => {
                                ...
                                return (
                                  <div key={idx} className="text-slate-800 text-[11px] flex flex-wrap items-center justify-between gap-1.5 border-b border-slate-200/40 pb-1.5 last:border-0 last:pb-0">
                                    <span className="font-extrabold">• {cl?.name || 'موكل'} <span className="text-slate-400 font-medium text-[10px]">{cl?.role ? `(${cl.role})` : ''}</span></span>
                                    {cleanPhone && (
                                      <div className="flex items-center gap-1.5">
                                        <a href={`tel:${cleanPhone}`} className="bg-white border border-slate-200 hover:border-amber-500/50 hover:bg-amber-50 text-slate-700 px-2 py-0.5 rounded-lg font-mono text-[10px] flex items-center gap-1 transition-all shadow-3xs" title="اتصال هاتفي مباشر">
                                          <span className="w-4 h-4 rounded-full bg-amber-500/15 border border-amber-500/30 text-amber-700 flex items-center justify-center shrink-0">
                                            <Phone className="w-2.5 h-2.5" />
                                          </span>
                                          <span>{cleanPhone}</span>
                                        </a>
                                        <a href={`https://wa.me/${cleanPhone.replace(/[^0-9]/g, '')}`} target="_blank" rel="noreferrer" className="bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 text-emerald-700 px-1.5 py-0.5 rounded-lg text-[10px] transition-all flex items-center justify-center" title="إرسال رسالة واتساب الفورية">
                                          <span className="w-4 h-4 rounded-full bg-emerald-500/20 border border-emerald-300 flex items-center justify-center shrink-0">
                                            <MessageSquare className="w-2.5 h-2.5 text-emerald-800" />
                                          </span>
                                        </a>
                                      </div>
                                    )}
                              ...
                              <div className="text-slate-850 font-extrabold">• {c.clientName || 'غير محدد'}</div>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="space-y-1.5 border-t border-slate-200/60 pt-3">
                          <div className="flex items-center gap-2 text-rose-800 font-black">
                            <span className="w-5 h-5 rounded-full bg-rose-500/15 border border-rose-300 flex items-center justify-center shrink-0 text-rose-700">
                          ...
                          <div className="mr-3.5 space-y-1.5">
                            {c.opponentsList && c.opponentsList.length > 0 ? (
                              c.opponentsList.map((opp, idx) => {
                                ...
                                return (
                                  <div key={idx} className="text-slate-700 text-[11px] flex flex-wrap items-center justify-between gap-1.5 border-b border-slate-200/40 pb-1.5 last:border-0 last:pb-0">
                                    <span className="font-bold">• {opp?.name || 'خصم'} <span className="text-slate-400 font-medium text-[10px]">{opp?.role ? `(${opp.role})` : ''}</span></span>
                                    <div className="flex items-center gap-1.5">
                                      {cleanPhone && (
                                        <a href={`tel:${cleanPhone}`} className="bg-white border border-slate-200 hover:border-rose-500/30 text-slate-600 px-2 py-0.5 rounded-lg font-mono text-[10px] flex items-center gap-1" title="اتصال هاتفي بالخصم">
                                          <span className="w-4 h-4 rounded-full bg-slate-100 border border-slate-200 text-slate-600 flex items-center justify-center shrink-0">
                                            <Phone className="w-2.5 h-2.5" />
                                          </span>
                                          <span>{cleanPhone}</span>
                                        </a>
                              ...
                              <div className="text-slate-700 font-bold">
                                • {c.opponent?.name || 'غير محدد'} <span className="text-slate-400 font-medium text-[10px]">{c.opponent?.role ? `(${c.opponent.role})` : ''}</span>
                      ...
                      <div className="space-y-2">
                        ...
                        {hasNextHearing ? (
                          isExpertNext ? (
                            <div className="bg-indigo-500/10 border border-indigo-500/20 text-indigo-950 p-2.5 rounded-xl text-xs flex items-center justify-between shadow-3xs">
                              <span className="font-black flex items-center gap-2">
                                <span className="w-5 h-5 rounded-full bg-indigo-500/20 border border-indigo-500/35 flex items-center justify-center shrink-0 text-indigo-800">
                                  <Scale className="w-2.5 h-2.5 text-indigo-800" />
                                </span>
                                <span>جلسة الخبير القادمة:</span>
                              </span>
                              <span className="font-mono font-black">{toAr(nextHearingDate)} {nextHearingTime ? `@ ${nextHearingTime}` : ''}</span>
                            </div>
                          ) : (isDetentionNext && c.isInvestigationActive) ? (
                            <div className="bg-rose-500/10 border border-rose-500/20 text-rose-950 p-2.5 rounded-xl text-xs flex items-center justify-between shadow-3xs">
                              <span className="font-black flex items-center gap-2">
                                <span className="w-5 h-5 rounded-full bg-rose-500/20 border border-rose-500/35 flex items-center justify-center shrink-0 text-rose-800">
                                  <ShieldAlert className="w-2.5 h-2.5 text-rose-800" />
                                </span>
                                <span>تاريخ الجلسة القادمة – تجديد حبس:</span>
                              </span>
                              <span className="font-mono font-black">{toAr(nextHearingDate)} {nextHearingTime ? `@ ${nextHearingTime}` : ''}</span>
                            </div>
                          ) : isPastHearing ? (
                            <div className="bg-amber-500/10 border border-amber-500/30 text-amber-950 p-2.5 rounded-xl text-xs flex items-center justify-between shadow-3xs">
                              <span className="font-black flex items-center gap-2">
                                <span className="w-5 h-5 rounded-full bg-amber-500/20 border border-amber-500/35 flex items-center justify-center shrink-0 text-amber-800">
                                  <Calendar className="w-2.5 h-2.5 text-amber-800" />
                                </span>
                                <span>الجلسة المحددة (بانتظار القرار):</span>
                              </span>
                              <span className="font-mono font-black text-amber-950">{toAr(nextHearingDate)} {nextHearingTime ? `@ ${nextHearingTime}` : ''}</span>
                            </div>
                          ) : (
                            <div className="bg-amber-500/10 border border-amber-500/20 text-amber-950 p-2.5 rounded-xl text-xs flex items-center justify-between shadow-3xs animate-pulse">
                              <span className="font-black flex items-center gap-2">
                                <span className="w-5 h-5 rounded-full bg-amber-500/20 border border-amber-500/35 flex items-center justify-center shrink-0 text-amber-800">
                                  <Calendar className="w-2.5 h-2.5 text-amber-800" />
                                </span>
                          ...
                          <div className="bg-slate-50 border border-slate-200 text-slate-500 p-2.5 rounded-xl text-xs flex items-center justify-between">
                            <span className="font-bold flex items-center gap-2 text-slate-400">
                              <span className="w-5 h-5 rounded-full bg-slate-200/70 border border-slate-300/50 flex items-center justify-center shrink-0 text-slate-400">
                                <Calendar className="w-2.5 h-2.5 text-slate-400" />
                              </span>
                              <span>{c.isReferredToExperts || c.expertReferral?.isReferred ? 'جلسة الخبير القادمة:' : 'الجلسة القادمة:'}</span>
                            </span>
                        ...
                        {c.prosecutorName && (
                          <p className="text-[10px] text-slate-400 font-bold pr-1.5 flex items-center gap-1.5">
                            <span className="w-4 h-4 rounded-full bg-slate-200/70 border border-slate-300/50 flex items-center justify-center shrink-0 text-[9px]">💼</span>
                            <span>عضو النيابة المختص بالملف: <strong className="text-slate-600 font-black">{c.prosecutorName}</strong></span>
                          </p>
                        )}
                      </div>
                    ...
                    <div className="border-t border-slate-150 pt-4 flex flex-col gap-2.5">
                      {c.isInvestigationActive && (
                        <button
                          onClick={() => setDetentionModalCase(c)}
                          className="w-full py-2 px-3 bg-amber-500/15 hover:bg-amber-500/25 border border-amber-500/40 text-amber-950 text-xs font-black rounded-xl transition-all duration-150 flex items-center justify-center gap-2 cursor-pointer shadow-3xs hover:scale-[1.005] active:scale-98"
                        >
                          <span className="w-5 h-5 rounded-full bg-amber-500/25 border border-amber-500/40 flex items-center justify-center shrink-0 text-amber-900">
                            <Lock className="w-2.5 h-2.5 text-amber-800" />
                          </span>
                          <span>جلسات تجديد الحبس الاحتياطي {c.detentionRenewals && c.detentionRenewals.length > 0 ? `(${c.detentionRenewals.length})` : ''}</span>
                        </button>
                      )}

                      {/* Attach Case Documents Button */}
                      <button
                        type="button"
                        onClick={() => setDocumentsModalCase(c)}
                        className="w-full py-2.5 px-3 bg-amber-50 hover:bg-amber-100/90 border border-amber-300/80 text-amber-950 text-xs font-black rounded-xl transition-all duration-150 flex items-center justify-center gap-2 cursor-pointer shadow-3xs hover:scale-[1.005] active:scale-98"
                        title="إرفاق وإدارة مستندات القضية"
                      >
                        <span className="w-5 h-5 rounded-full bg-amber-500/20 border border-amber-500/30 flex items-center justify-center shrink-0 text-amber-900">
                          <Paperclip className="w-2.5 h-2.5 text-amber-800" />
                        </span>
                        <span>إرفاق المستندات</span>
                        {(c.files || []).length > 0 && (
                          <span className="bg-amber-200 text-amber-900 text-[10px] font-black px-1.5 py-0.5 rounded-full mr-1">
                            {(c.files || []).length}
                          </span>
                        )}
                      </button>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {currentUser.permissions.printCase && (
                          <button
                            ...
                            className="py-2.5 px-3 bg-slate-900 hover:bg-slate-800 text-amber-400 hover:text-amber-300 text-xs font-black rounded-xl shadow-xs transition-all duration-150 flex items-center justify-center gap-2 cursor-pointer hover:scale-[1.01] active:scale-95"
                          >
                            <span className="w-5 h-5 rounded-full bg-amber-400/20 border border-amber-400/30 flex items-center justify-center shrink-0 text-amber-300">
                              <Eye className="w-2.5 h-2.5" />
                            </span>
                            <span>ملف القضية الشامل ({(c.files || []).length})</span>
                          </button>
                        )}
                        {currentUser.permissions.printCase && (
                          <button
                            onClick={() => setShowReportViewer(c)}
                            className="py-2.5 px-3 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 text-emerald-800 text-xs font-black rounded-xl transition-all duration-150 flex items-center justify-center gap-2 cursor-pointer hover:scale-[1.01] active:scale-95 shadow-3xs"
                          >
                            <span className="w-5 h-5 rounded-full bg-emerald-500/15 border border-emerald-300 flex items-center justify-center shrink-0 text-emerald-700">
                              <FileText className="w-2.5 h-2.5" />
                            </span>
                            <span>عرض التقرير القضائي</span>
                          </button>
                        )}
                      </div>

                      <div className="flex flex-wrap items-center justify-end gap-3 text-xs border-t border-slate-100/60 pt-3">
                        {c.isInvestigationActive && (
                          <button
                            onClick={() => setDetentionModalCase(c)}
                            className="text-amber-800 hover:text-amber-950 font-extrabold flex items-center gap-1 cursor-pointer transition-colors"
                            title="جلسات وقرارات تجديد الحبس الاحتياطي"
                          >
                            <span className="w-5 h-5 rounded-full bg-amber-500/15 border border-amber-300 flex items-center justify-center shrink-0 text-amber-900 text-[10px]">🔒</span>
                            <span>تجديد الحبس {c.detentionRenewals && c.detentionRenewals.length > 0 ? `(${c.detentionRenewals.length})` : ''}</span>
                          </button>
                        )}

                        <button
                          type="button"
                          onClick={() => setNoteModalCase(c)}
                          className="text-amber-800 hover:text-amber-950 font-extrabold flex items-center gap-1.5 cursor-pointer transition-colors"
                          title="إضافة ملاحظة على ملف القضية"
                        >
                          <span className="w-5 h-5 rounded-full bg-amber-500/15 border border-amber-300 flex items-center justify-center shrink-0 text-amber-900 text-[10px]">
                            <MessageSquare className="w-2.5 h-2.5" />
                          </span>
                          <span>إضافة ملاحظة {c.caseNotes && c.caseNotes.length > 0 ? `(${c.caseNotes.length})` : ''}</span>
                        </button>

                        {currentUser.permissions.editCase && (
                          <button
                            onClick={() => handleOpenEdit(c)}
                            className="text-blue-600 hover:text-blue-800 hover:underline font-extrabold flex items-center gap-1.5 cursor-pointer transition-colors"
                          >
                            <span className="w-5 h-5 rounded-full bg-blue-500/10 border border-blue-200 flex items-center justify-center shrink-0 text-blue-600 text-[10px]">✏️</span>
                        ...
                        {currentUser.permissions.archiveCase && (
                          <button
                            ...
                            className="text-emerald-700 hover:text-emerald-900 hover:underline font-extrabold flex items-center gap-1.5 cursor-pointer transition-colors"
                          >
                            <span className="w-5 h-5 rounded-full bg-emerald-500/10 border border-emerald-200 flex items-center justify-center shrink-0 text-emerald-700">
      ...
      <BaseModal
        ...
        icon={Gavel}
        size="3xl"
        headerClassName="px-4 sm:px-6 py-2 sm:py-2.5"
        bodyClassName="px-3.5 sm:px-6 py-3 sm:py-4 space-y-4"
        subHeader={
          <div className="flex items-center justify-between gap-1 sm:gap-2 overflow-x-auto select-none py-0.5">
            {[
              { id: 'judicial', label: 'البيانات القضائية', desc: 'درجة التقاضي والمحكمة', icon: Gavel },
              ...(isInvestigationActive ? [{ id: 'investigation', label: 'مرحلة التحقيق والتجديد', desc: 'النيابة والحبس الاحتياطي', icon: ShieldAlert }] : []),
              ...
              const stepsOrder = [
                'judicial',
                ...(isInvestigationActive ? ['investigation'] : []),
                'litigants',
                'financials',
                'attachments'
              ];
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1.5 sm:gap-2 text-right transition-all duration-200 outline-none cursor-pointer py-1 px-2 sm:px-2.5 rounded-xl whitespace-nowrap shrink-0 ${
                    isActive ? 'bg-white text-slate-900 shadow-xs ring-1 ring-slate-200/80 font-bold' : 'text-slate-600 hover:bg-white/60'
                  }`}
                >
                  <div className={`w-7 h-7 sm:w-7.5 sm:h-7.5 rounded-lg flex items-center justify-center transition-all duration-300 shrink-0 ${
                    ...
                    {isCompleted ? <CheckCircle className="w-3.5 h-3.5 text-emerald-600" /> : <Icon className="w-3.5 h-3.5" />}
                  </div>
                  <div className="text-right">
                    <p className={`text-xs sm:text-[12.5px] font-black leading-tight ${isActive ? 'text-slate-900' : 'text-slate-600'}`}>
                      {step.label}
                    </p>
                    <p className="hidden md:block text-[9.5px] text-slate-400 font-semibold leading-tight mt-0.5">
                      {step.desc}
                    </p>
                  </div>
                </button>
      ...
      >
        {showFormModal && (
          <>
            ...
            <form 
              ...
              className="space-y-6"
            >
          {/* Investigation Tab (مرحلة التحقيق والتجديد المستقلة) */}
          {activeFormTab === 'investigation' && isInvestigationActive && (
            <div className="space-y-6">
              {isInvestigationActive ? (
                <>
                  {(investigationDefendantStatus === 'تمت الإحالة للمحاكمة' || status === 'محالة للمحاكمة الجنائية') ? (
                    <div className="p-4 bg-emerald-50 border-2 border-emerald-300 rounded-2xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-sm">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="text-sm font-black text-emerald-950 flex items-center gap-2">
                            <span>تمت إحالة القضية للمحكمة الجنائية المختصة 🏛️</span>
                            <span className="px-2.5 py-0.5 bg-emerald-200/80 text-emerald-900 rounded-lg text-[11px] font-bold">مرفقة بملف القضية (للعرض)</span>
                          </h3>
                          <p className="text-[11px] font-bold text-emerald-900/80 mt-0.5">
                            بيانات مرحلة التحقيق وتجديدات النيابة السابقة محفوظة ومرفقة بملف القضية ومتاحة للعرض فقط، وقسم البيانات القضائية مفتوح ومتاح للعمل.
                          </p>
                        </div>
                      </div>
                      ...
                      <button
                        ...
                        className="px-4 py-2 bg-emerald-700 hover:bg-emerald-800 text-white rounded-xl text-xs font-black shadow-sm transition-all flex items-center gap-1.5 shrink-0 cursor-pointer"
                      >
                        <span>الانتقال للبيانات القضائية ➔</span>
                      </button>
                    </div>
                  ) : (
                    <div className="p-4 bg-amber-50 border-2 border-amber-300 rounded-2xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-sm">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="text-sm font-black text-amber-950 flex items-center gap-2">
                            <span>مرحلة التحقيق وتجديدات الحبس الاحتياطي لدى النيابة العامة مفعلة</span>
                            <span className="px-2 py-0.5 bg-amber-200/80 text-amber-900 rounded-lg text-[11px] font-bold">مفتوحة 🔓</span>
                          </h3>
                          <p className="text-[11px] font-bold text-amber-900/80 mt-0.5">
                            يتم تسجيل وتثبيت قرارات وتجديدات الحبس ومزامنة مواعيد الجلسات تلقائياً مع أجندة الجلسات.
                          </p>
                        </div>
                      </div>
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          setInvestigationStartDate('');
                          setInvestigationNotes('');
                          if (status === 'قيد التحقيق' || status === 'تجديد حبس احتياطي') {
                            setStatus('متداولة بجلسات المحكمة');
                          }
                        }}
                        className="px-4 py-2 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-300 rounded-xl text-xs font-black transition-all flex items-center gap-1.5 shrink-0 shadow-sm cursor-pointer"
                      >
                        <X className="w-4 h-4 text-rose-600" />
                        <span>إلغاء تفعيل مرحلة التحقيق</span>
                      </button>
                    </div>
                  )}

                  <FormCard title="بيانات مرحلة التحقيق والنيابة العامة" icon={ShieldAlert}>
                    <div className="space-y-4">
                      <FormGrid cols={4}>
                        <FormField label="رقم التحقيق / النيابة" isMono>
                          <input
                            type="text"
                            placeholder="مثال: 13676"
                            value={investigationNumber}
                            onChange={(e) => setInvestigationNumber(e.target.value)}
                            className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:bg-white font-mono text-left"
                            dir="ltr"
                          />
                        </FormField>

                        <FormField label="جهة التحقيق / النيابة المختصة">
                          <select
                            value={investigationAuthority}
                            onChange={(e) => setInvestigationAuthority(e.target.value)}
                            className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:bg-white font-bold"
                          >
                            <option value="">اختر جهة التحقيق...</option>
                            <option value="النيابة العامة">النيابة العامة</option>
                            <option value="نيابة الأموال العامة">نيابة الأموال العامة</option>
                            <option value="نيابة الشؤون المالية والتجارية">نيابة الشؤون المالية والتجارية</option>
                            <option value="نيابة أمن الدولة العليا">نيابة أمن الدولة العليا</option>
                            <option value="نيابة الطفل">نيابة الطفل</option>
                            <option value="نيابة الأسرة">نيابة الأسرة</option>
                            <option value="نيابة المرور">نيابة المرور</option>
                            {investigationAuthority && !["النيابة العامة", "نيابة الأموال العامة", "نيابة الشؤون المالية والتجارية", "نيابة أمن الدولة العليا", "نيابة الطفل", "نيابة الأسرة", "نيابة المرور"].includes(investigationAuthority) && (
                              <option value={investigationAuthority}>{investigationAuthority}</option>
                            )}
                          </select>
                        </FormField>

                        <FormField label="تاريخ بدء التحقيق" isMono>
                          <input
                            type="date"
                        ...
                        <FormField label="الحالة الحالية للمتهم">
                          <select
                            value={investigationDefendantStatus}
                            onChange={(e) => {
                              const val = e.target.value;
                              setInvestigationDefendantStatus(val);
                              if (val === 'تمت الإحالة للمحاكمة') {
                                setStatus('محالة للمحاكمة الجنائية');
                              } else if (val === 'محبوس احتياطياً') {
                                setStatus('تجديد حبس احتياطي');
                              }
                            }}
                            className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:bg-white font-bold"
                          >
                            <option value="محبوس احتياطياً">محبوس احتياطياً ⏳</option>
                            <option value="مخلى سبيله">مخلى سبيله ⚖️</option>
                            <option value="مخلى سبيله بكفالة">مخلى سبيله بكفالة 💰</option>
                            <option value="هارب / مطلوب">هارب / مطلوب 🚨</option>
                            <option value="تمت الإحالة للمحاكمة">تمت الإحالة للمحاكمة الجنائية 🏛️</option>
                            <option value="حفظ التحقيق">حفظ التحقيق / ألا وجه لإقامة الدعوى 📁</option>
                          </select>
                        </FormField>
                      </FormGrid>

                      {/* Detention Renewal Records */}
                      <div className="border-t border-slate-100 pt-4 mt-2">
                        <div className="flex items-center justify-between mb-3">
                          <div>
                            <h4 className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                              <Clock className="w-4 h-4 text-amber-600" />
                              <span>سجل تجديدات الحبس الاحتياطي ({detentionRenewals.length})</span>
                            </h4>
                            <p className="text-[10px] text-slate-500 font-medium mt-0.5">
                              تثبيت مواعيد الجلسات وتحديث الجلسة القادمة ومزامنتها تلقائياً مع الأجندة
                            </p>
                          </div>
                          <button
                            type="button"
                            onClick={() => {
                              ...
                              setDetentionRenewals([
                                ...detentionRenewals,
                                {
                                  ...
                                  nextAuthority: 'تجديد غرفة المشورة',
                                  notes: ''
                                }
                              ]);
                            }}
                            className="px-3 py-1.5 bg-amber-500/10 hover:bg-amber-500/20 text-amber-800 border border-amber-500/30 rounded-xl text-xs font-bold transition-all flex items-center gap-1 cursor-pointer"
                          >
                            <Plus className="w-3.5 h-3.5" />
                            <span>إضافة تجديد حبس</span>
                          </button>
                        </div>

                        {detentionRenewals.length === 0 ? (
                          <div className="p-4 bg-slate-50 border border-slate-200/60 rounded-xl text-center text-xs text-slate-500">
                            لا توجد تجديدات مسجلة بعد. اضغط على "+ إضافة تجديد حبس" لإضافة وتثبيت جلسة تجديد.
                          </div>
                        ) : (
                          <div className="space-y-2">
                            {detentionRenewals.map((ren, idx) => (
                              <div key={ren.id || idx} className="grid grid-cols-12 gap-2 items-center bg-slate-50 border border-slate-200 p-2.5 rounded-xl text-xs">
                                <div className="col-span-12 sm:col-span-2">
                                  <label className="block text-[10px] font-bold text-slate-600 mb-0.5">تاريخ التجديد</label>
                                  <input
                                    type="date"
                                    value={ren.renewalDate || ren.date || ''}
                                    onChange={(e) => {
                                      const updated = [...detentionRenewals];
                                      updated[idx].renewalDate = e.target.value;
                                      updated[idx].date = e.target.value;
                                      setDetentionRenewals(updated);
                                    }}
                                ...
                                <div className="col-span-12 sm:col-span-2">
                                  <label className="block text-[10px] font-bold text-amber-800 mb-0.5">جلسة التجديد القادمة</label>
                                  <input
                                    type="date"
                                    value={ren.nextRenewalDate || ''}
                                    onChange={(e) => {
                                      const updated = [...detentionRenewals];
                                    ...
                                    className="w-full px-2 py-1 bg-amber-50/70 border border-amber-300 rounded-lg text-xs font-mono font-bold text-amber-950"
                                  />
                                </div>

                                <div className="col-span-6 sm:col-span-1">
                                  <label className="block text-[10px] font-bold text-slate-600 mb-0.5">المدة (يوم)</label>
                                  <input
                                    type="number"
                                    value={ren.durationDays || 15}
                                    onChange={(e) => {
                                      const updated = [...detentionRenewals];
                                ...
                                <div className="col-span-6 sm:col-span-2">
                                  <label className="block text-[10px] font-bold text-slate-600 mb-0.5">الجهة الصادرة للقرار</label>
                                  <select
                                    value={ren.authority || ''}
                                    onChange={(e) => {
                                      const updated = [...detentionRenewals];
                                    ...
                                    className="w-full px-2 py-1 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-800 focus:ring-amber-500 focus:border-amber-500"
                                  >
                                    <option value="">اختر الجهة...</option>
                                    <option value="النيابة العامة">النيابة العامة</option>
                                    <option value="تجديد جزئي">تجديد جزئي</option>
                                    <option value="تجديد غرفة المشورة">تجديد غرفة المشورة</option>
                                    <option value="تجديد الجنايات">تجديد الجنايات</option>
                                    {ren.authority && !["النيابة العامة", "تجديد جزئي", "تجديد غرفة المشورة", "تجديد الجنايات"].includes(ren.authority) && (
                                      <option value={ren.authority}>{ren.authority}</option>
                                    )}
                                  </select>
                                </div>

                                <div className="col-span-12 sm:col-span-2">
                                  <label className="block text-[10px] font-bold text-amber-800 mb-0.5">الجهة المنظور أمامها (القادمة)</label>
                                  <select
                                    value={ren.nextAuthority || ''}
                                    onChange={(e) => {
                                      const updated = [...detentionRenewals];
                                      updated[idx].nextAuthority = e.target.value;
                                      setDetentionRenewals(updated);
                                    }}
                                    className="w-full px-2 py-1 bg-amber-50/70 border border-amber-300 rounded-lg text-xs font-bold text-amber-950 focus:ring-amber-500 focus:border-amber-500"
                                  >
                                    <option value="">اختر الجهة...</option>
                                    <option value="النيابة العامة">النيابة العامة</option>
                                    <option value="تجديد جزئي">تجديد جزئي</option>
                                    ...
                                    {ren.nextAuthority && !["النيابة العامة", "تجديد جزئي", "تجديد غرفة المشورة", "تجديد الجنايات", "استئناف أمر الحبس"].includes(ren.nextAuthority) && (
                                      <option value={ren.nextAuthority}>{ren.nextAuthority}</option>
                                    )}
                                  </select>
                                </div>

                                <div className="col-span-10 sm:col-span-2">
                                  <label className="block text-[10px] font-bold text-slate-600 mb-0.5">القرار / ملاحظات</label>
                                  <input
                                    type="text"
                                    placeholder="منطوق القرار وملاحظات الجلسة..."
                                    value={ren.decision || ren.notes || ''}
                                    onChange={(e) => {
                                      const updated = [...detentionRenewals];
                                ...
                                <div className="col-span-2 sm:col-span-1 flex justify-end pt-3">
                                  <button
                                    type="button"
                                    onClick={() => {
                                      setDetentionRenewals(detentionRenewals.filter((_, i) => i !== idx));
                                    }}
                      ...
                      <FormField label="ملاحظات وتفاصيل مرحلة التحقيق">
                        <textarea
                          rows={2}
                          placeholder="أي ملاحظات إضافية عن مسار التحقيق وقرارات النيابة ومواجهات المتهم..."
                      ...
                      {/* Live Badge Preview */}
                      <div className="p-3 bg-amber-50 border border-amber-200/70 rounded-xl flex items-center gap-2 text-xs text-amber-900">
                        <span className="font-bold">معاينة الشكل داخل بطاقة القضية بالقائمة:</span>
                        <span className="inline-flex items-center gap-1 px-3 py-1 bg-amber-500/15 border border-amber-500/30 text-amber-800 rounded-lg font-bold text-xs">
                          تحقيقات {investigationNumber ? `رقم ${investigationNumber}` : ''} {investigationAuthority ? `- ${investigationAuthority}` : ''} 🔑
                        </span>
                      </div>

                      {/* Referral Action */}
                      {investigationDefendantStatus !== 'تمت الإحالة للمحاكمة' ? (
                        <div className="p-3.5 bg-amber-500/10 border border-amber-500/30 rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs text-amber-950 font-bold mt-2">
                          <div className="flex items-center gap-2">
                            <Lock className="w-4 h-4 text-amber-700 shrink-0" />
                            <span>بيانات المحكمة ودرجة التقاضي مقفلة حالياً. هل تم صدور أمر الإحالة للمحكمة المختصة؟</span>
                          </div>
                          <button
                            type="button"
                            onClick={() => {
                              setInvestigationDefendantStatus('تمت الإحالة للمحاكمة');
                              setStatus('محالة للمحاكمة الجنائية');
                              setActiveFormTab('judicial');
                            }}
                            className="px-3.5 py-1.5 bg-amber-600 hover:bg-amber-700 text-white rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 shrink-0 shadow-sm cursor-pointer"
                          >
                            <Gavel className="w-3.5 h-3.5" />
                            <span>إحالة للمحكمة الآن وفتح درجة التقاضي 🔓</span>
                          </button>
                        </div>
                      ) : (
                        <div className="p-3.5 bg-emerald-50 border border-emerald-200 rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs text-emerald-950 font-bold mt-2">
                          <div className="flex items-center gap-2">
                            <CheckCircle className="w-4 h-4 text-emerald-600 shrink-0" />
                            <span>تمت الإحالة للمحكمة الجنائية المختصة! قسم "البيانات القضائية" مفتوح ومتاح للعمل الآن.</span>
                          </div>
                          <button
                            type="button"
                            onClick={() => setActiveFormTab('judicial')}
                            className="px-3.5 py-1.5 bg-emerald-700 hover:bg-emerald-800 text-white rounded-lg text-xs font-bold transition-all shrink-0 cursor-pointer"
                          >
                            الانتقال للبيانات القضائية ➔
                          </button>
                        </div>
                      )}
                    </div>
                ...
                <div className="p-8 bg-slate-50 border border-slate-200 rounded-2xl text-center space-y-3">
                  ...
                  <h4 className="text-sm font-black text-slate-800">مرحلة التحقيق والتجديد لدى النيابة العامة غير مفعلة</h4>
                  <p className="text-xs font-bold text-slate-500 max-w-md mx-auto">
                    هذا الخيار خاص بالتحقيقات الجنائية وتجديدات الحبس الاحتياطي. إذا كانت الدعوى متداولة مباشرة أمام المحكمة، يمكنك المتابعة في قسم "البيانات القضائية".
                  </p>
                  <button
                    type="button"
                    onClick={() => {
                      setIsInvestigationActive(true);
                      setStatus('قيد التحقيق');
                    }}
                    className="px-5 py-2.5 bg-amber-600 hover:bg-amber-700 text-white rounded-xl text-xs font-bold shadow-sm transition-all cursor-pointer flex items-center gap-2 mx-auto"
                  >
                    <Plus className="w-4 h-4" />
                    <span>تفعيل مرحلة التحقيق والتجديد الآن ⚡</span>
                  </button>
                </div>
              )}
            </div>
          ...
          {activeFormTab === 'judicial' && (
            <div className="space-y-6">
              ...
              <FormCard title="بيانات درجة التقاضي ونوع الدعوى" icon={Gavel}>
                <FormGrid cols={3}>
                  <FormField label="درجة التقاضي الحالية (محددة تلقائيًا)">
                    <div className="w-full px-3 py-2 bg-amber-500/10 border border-amber-500/20 rounded-xl text-xs font-black text-amber-950 flex items-center justify-between shadow-3xs">
                      ...
                      <span className="text-[10px] text-amber-800 font-bold bg-white/80 px-2 py-0.5 rounded-md border border-amber-500/10">
                        تحدّد تلقائيًا حسب المراحل المفعّلة
                      </span>
                    </div>
                  </FormField>
                ...
                <div className="flex flex-wrap items-center gap-6 mt-4 pt-2.5 border-t border-slate-100/60">
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    <input
                      type="checkbox"
                      checked={isInvestigationActive}
                      onChange={(e) => {
                        const active = e.target.checked;
                        setIsInvestigationActive(active);
                        if (active) {
                          setStatus('قيد التحقيق');
                          setActiveFormTab('investigation');
                        } else {
                          ...
                          if (activeFormTab === 'investigation') {
                            setActiveFormTab('judicial');
                          }
                        }
                      }}
                    ...
                    />
                    <span className="text-xs font-bold text-slate-700">تفعيل مرحلة التحقيق (النيابة العامة والحبس الاحتياطي)</span>
                  </label>
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-amber-500/10 border border-amber-500/20 p-4 rounded-2xl">
                ...
                <div className="flex items-center gap-2.5 flex-wrap">
                  <button
                    type="button"
                    onClick={() => handleOpenManualClientModal(-1)}
                    className="px-3.5 py-1.5 bg-amber-500 hover:bg-amber-600 text-slate-950 text-xs rounded-xl font-black transition-all flex items-center gap-1.5 shadow-sm active:scale-95 cursor-pointer ring-1 ring-amber-400/40"
              ...
              {manualClientSuccessMsg && (
                <div className="p-3 bg-emerald-50 border-2 border-emerald-400 rounded-xl flex items-center justify-between gap-2 shadow-xs animate-in fade-in duration-200">
                  <div className="flex items-center gap-2 text-emerald-900 text-xs font-black">
                    <CheckCircle className="w-4 h-4 text-emerald-600 shrink-0" />
                    <span>{manualClientSuccessMsg}</span>
                  </div>
                  <button
                    type="button"
                    onClick={() => setManualClientSuccessMsg('')}
                  ...
                  {formClients.map((cl, idx) => (
                    <div key={idx} className="bg-slate-50 hover:bg-slate-100/50 border border-slate-150 rounded-xl p-4 transition-all duration-200 relative">
                      ...
                      {(!cl?.name || !cl.name.trim()) && (
                        <div className="mt-3 p-3 bg-amber-500/10 border border-amber-500/30 rounded-xl text-amber-900 flex items-center gap-2.5 text-xs font-bold shadow-xs">
                          <AlertTriangle className="w-4 h-4 text-amber-600 shrink-0 animate-pulse" />
                          <span>تنبيه هام: لم يتم تحديد الموكل بعد! يرجى اختيار موكل مسجل من القائمة أعلاه أو إدخال اسم الموكل بالكامل.</span>
                        </div>
                      )}
          ...
          {activeFormTab === 'attachments' && (
            <div className="space-y-6">
                        ...
                        {uploadedFiles.map((f) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {/* Card Actions */}
                              {(() => {
                                const rawUrl = f.downloadURL || f.fileUrl;
                                  ...
                                  <div className="flex items-center justify-end gap-2 border-t border-slate-100 pt-2.5 mt-3 pr-2">
                                    {directProxyUrl ? (
                                      <a
                                        ...
                                        className="px-3 py-1 bg-slate-950 hover:bg-slate-900 text-amber-400 hover:text-amber-300 text-[10px] font-extrabold rounded-lg transition-all flex items-center gap-1 cursor-pointer"
              ...
              {/* Modal Actions */}
              <div className="sticky bottom-0 bg-white/95 backdrop-blur-md p-4 sm:p-5 border-t border-slate-200/90 flex items-center justify-between rounded-2xl shrink-0 z-20 shadow-sm mt-6" dir="rtl">
                
                {/* Right Side: Back or Cancel */}
                <div>
                  {(() => {
                    const currentTabsSequence: ('judicial' | 'investigation' | 'litigants' | 'financials' | 'attachments')[] = [
                      'judicial',
                      ...(isInvestigationActive ? ['investigation' as const] : []),
                      'litigants',
                      'financials',
                      'attachments'
                    ];
                    const currentIdx = currentTabsSequence.indexOf(activeFormTab);
                    return currentIdx > 0 ? (
                      <button
                        type="button"
                        onClick={() => {
                          setActiveFormTab(currentTabsSequence[currentIdx - 1]);
                        }}
                        className="px-5 py-2.5 bg-white border border-slate-300 hover:bg-slate-50 text-slate-800 text-xs sm:text-sm rounded-xl font-bold transition-all flex items-center gap-2 shadow-xs active:scale-95 cursor-pointer"
                      >
                      ...
                      <button
                        ...
                        className="px-5 py-2.5 bg-white border border-slate-300 hover:bg-slate-50 text-slate-600 hover:text-slate-900 text-xs sm:text-sm rounded-xl font-bold transition-all active:scale-95 cursor-pointer"
                      >
                        إلغاء التغييرات
                      </button>
                    );
                ...
                <div className="flex items-center gap-3">
                  {(() => {
                    ...
                    const currentIdx = currentTabsSequence.indexOf(activeFormTab);
                    return currentIdx < currentTabsSequence.length - 1 ? (
                      <button
                        type="button"
                        onClick={() => {
                          // Validate current tab before moving forward
                          if (activeFormTab === 'litigants') {
                            const activeClients = formClients.filter(cl => cl && cl.name && cl.name.trim() !== '');
                            if (activeClients.length === 0) {
                              alert('⚠️ تنبيه هام: لم يتم تحديد الموكل! يرجى اختيار موكل من القائمة أو كتابة اسمه بالكامل قبل المتابعة.');
                              return;
                            }
                            for (let idx = 0; idx < formClients.length; idx++) {
                              if (!formClients[idx]?.name || !formClients[idx].name.trim()) {
                                alert(`⚠️ تنبيه هام: لم يتم تحديد الموكل رقم ${idx + 1}! يرجى اختيار موكل مسجل بالنظام أو كتابة اسمه بالكامل.`);
                                return;
                              }
                              if (!formClients[idx].role || !formClients[idx].role.trim()) {
                                formClients[idx].role = 'مدعي';
                              }
                            }
                            
                            const activeOpponents = formOpponents.filter(opp => opp && opp.name && opp.name.trim() !== '');
                            if (activeOpponents.length === 0) {
                              alert('⚠️ تنبيه: يرجى إضافة اسم خصم واحد على الأقل للمتابعة.');
                              return;
                            }
                            for (let idx = 0; idx < formOpponents.length; idx++) {
                              if (!formOpponents[idx]?.name || !formOpponents[idx].name.trim()) {
                                alert(`يرجى كتابة اسم الخصم رقم ${idx + 1}.`);
                                return;
                              }
                              if (!formOpponents[idx].role || !formOpponents[idx].role.trim()) {
                                formOpponents[idx].role = 'مدعى عليه';
                              }
                            }
                          }
                        ...
                        className="px-5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white text-xs sm:text-sm rounded-xl font-bold transition-all flex items-center gap-2 shadow-sm hover:shadow-md active:scale-95 cursor-pointer"
                      >
                        التالي
                  ...
                  {activeFormTab !== 'attachments' && (
                    <button
                      ...
                      className="px-5 py-2.5 bg-amber-500 hover:bg-amber-600 text-slate-950 text-xs sm:text-sm rounded-xl font-black transition-all shadow-sm hover:shadow-md active:scale-95 flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
                    >
                      {isSubmittingCase ? (
                        <>
                          <RefreshCw className="w-4 h-4 animate-spin" />
                          <span>جاري حفظ القضية...</span>
                        </>
                      ) : (
                        <>
                  ...
                  {activeFormTab === 'attachments' && (
                    <button
                      ...
                      disabled={isSubmittingCase}
                      className="px-6 py-2.5 bg-amber-500 hover:bg-amber-600 text-slate-950 text-xs sm:text-sm rounded-xl font-black transition-all shadow-md hover:shadow-lg active:scale-95 flex items-center gap-2 cursor-pointer disabled:opacity-50"
                    >
                      {isSubmittingCase ? (
                        <>
                        ...
                        <>
                          ...
                          <span>{editingCase ? 'حفظ تعديلات الملف النهائي' : 'حفظ القضية'}</span>
                        </>
                      )}
                    </button>
                  )}
        ...
        {/* Investigation Cancel Confirmation Modal */}
        {showCancelInvestigationConfirm && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-100 space-y-4 text-right" dir="rtl">
              <div className="flex items-center gap-3 text-rose-600">
                <div className="w-10 h-10 rounded-xl bg-rose-100 flex items-center justify-center shrink-0">
                  <AlertTriangle className="w-6 h-6 text-rose-600" />
                </div>
                <div>
                  <h3 className="font-black text-slate-900 text-base">تأكيد إلغاء مرحلة التحقيق</h3>
                  <p className="text-xs text-slate-500 font-bold">إلغاء تفعيل التحقيق والتجديد المستقل</p>
                </div>
              </div>

              <p className="text-xs text-slate-700 leading-relaxed font-bold bg-rose-50/70 p-3.5 rounded-xl border border-rose-100">
                هل أنت متأكد من رغبتك في إلغاء مرحلة التحقيق والتجديد لدى النيابة العامة لهذه القضية؟
                <br />
                سيتم إلغاء تجميد بيانات درجات التقاضي والمحكمة وتحويل حالة القضية إلى متداولة بجلسات المحكمة.
              </p>

              <div className="flex items-center justify-end gap-2.5 pt-2">
                <button
                  type="button"
                  onClick={() => setShowCancelInvestigationConfirm(false)}
                ...
                <button
                  ...
                  className="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-xs font-black shadow-md transition-all cursor-pointer flex items-center gap-1.5 active:scale-95"
                >
                  <X className="w-4 h-4" />
                  <span>نعم، إلغاء مرحلة التحقيق</span>
                </button>
              </div>
            </div>
          </div>
        ...
        {/* Manual Client Addition Modal (إضافة موكل يدويًا دون مغادرة نافذة القضية) */}
        {showManualClientModal && (
          <div className="fixed inset-0 z-[130] flex items-center justify-center p-3 sm:p-4 bg-slate-950/75 backdrop-blur-xs animate-in fade-in duration-200 overflow-y-auto">
            <div 
              className="bg-white rounded-2xl max-w-2xl w-full max-h-[92vh] flex flex-col shadow-2xl border border-slate-200 text-right overflow-hidden my-auto" 
              dir="rtl"
            >
              {/* Header */}
              <div className="flex items-center justify-between px-5 py-3.5 bg-gradient-to-r from-slate-900 to-slate-800 text-white shrink-0 border-b border-slate-700">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-black text-white text-sm sm:text-base flex items-center gap-2">
                      <span>إضافة موكل جديد يدويًا</span>
                      <span className="px-2 py-0.5 bg-amber-400/20 text-amber-300 rounded text-[10px] font-bold border border-amber-400/30">
                        حفظ فوري بقاعدة البيانات
                      </span>
                    </h3>
                    <p className="text-[11px] text-slate-300 font-medium mt-0.5">
                      إدخال بيانات الموكل وحفظه في النظام وربطه بالقضية الحالية دون الخروج من هذه النافذة
                    </p>
                  </div>
                </div>
                <button
              ...
              {/* Body */}
              <form onSubmit={handleSaveManualClient} className="p-4 sm:p-6 overflow-y-auto space-y-4 flex-1">
                {/* Row 1: Name & Role */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">
                      اسم الموكل بالكامل <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="text"
                      required
                      autoFocus
                      placeholder="مثال: عبدالله محمد إبراهيم"
                      value={manualClientName}
                      onChange={(e) => setManualClientName(e.target.value)}
                      className="w-full px-3 py-2 bg-white border border-slate-300 focus:border-amber-500 focus:ring-2 focus:ring-amber-400/20 rounded-xl text-xs font-bold text-slate-900 outline-none transition-all"
                    />
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">
                      الصفة بالدعوى <span className="text-red-500">*</span>
                    </label>
                    <SearchableDropdown
                      value={manualClientRole}
                      onChange={(val) => setManualClientRole(val)}
                      options={capacityOptions}
                      placeholder="اختر الصفة بالدعوى"
                      required
                    />
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full px-3 py-2 bg-white border border-slate-300 focus:border-amber-500 focus:ring-2 focus:ring-amber-400/20 rounded-xl text-xs font-mono text-left outline-none transition-all"
                      />
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        dir="ltr"
                        placeholder="01xxxxxxxxx"
                        value={manualClientSecondaryPhone}
                        onChange={(e) => setManualClientSecondaryPhone(e.target.value)}
                        className="w-full px-3 py-2 bg-white border border-slate-300 focus:border-amber-500 focus:ring-2 focus:ring-amber-400/20 rounded-xl text-xs font-mono text-left outline-none transition-all"
                      />
                      <Phone className="w-3.5 h-3.5 text-slate-400 absolute right-3 top-2.5 pointer-events-none" />
                    </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                  <div>
                    ...
                    <input
                      ...
                      value={manualClientNationalId}
                      onChange={(e) => setManualClientNationalId(e.target.value)}
                      className="w-full px-3 py-2 bg-white border border-slate-300 focus:border-amber-500 focus:ring-2 focus:ring-amber-400/20 rounded-xl text-xs font-mono text-left outline-none transition-all"
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">
                      المهنة / الوظيفة (اختياري)
                    </label>
                    <div className="relative">
                      <input
                        type="text"
                        placeholder="مثال: محاسب، تاجر، مهندس..."
                        value={manualClientJob}
                        onChange={(e) => setManualClientJob(e.target.value)}
                        className="w-full px-3 py-2 bg-white border border-slate-300 focus:border-amber-500 focus:ring-2 focus:ring-amber-400/20 rounded-xl text-xs font-medium text-slate-900 outline-none transition-all"
                      />
                      <Briefcase className="w-3.5 h-3.5 text-slate-400 absolute right-3 top-2.5 pointer-events-none" />
                    </div>
                  </div>
                </div>
                ...
                {/* Row 5: Associated Company */}
                {companies && companies.length > 0 && (
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">
                      الشركة التابع لها الموكل (اختياري)
                    </label>
                    <select
                      value={manualClientCompanyId}
                      onChange={(e) => setManualClientCompanyId(e.target.value)}
                      className="w-full px-3 py-2 bg-white border border-slate-300 focus:border-amber-500 focus:ring-2 focus:ring-amber-400/20 rounded-xl text-xs font-bold text-slate-700 outline-none transition-all cursor-pointer"
                    >
                      <option value="">-- موكل فردي (غير مرتبط بشركة) --</option>
                      {companies.map(co => (
                        <option key={co.id} value={co.id}>
                          {co.name} {co.companyType ? `(${co.companyType})` : ''}
                        </option>
                      ))}
                    </select>
                  </div>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">
                    ملاحظات وتوجيهات خاصة بالموكل (اختياري)
                  </label>
                  <textarea
                    rows={2}
                    placeholder="أي ملاحظات حول الموكل أو ملفه..."
                    value={manualClientNotes}
                    onChange={(e) => setManualClientNotes(e.target.value)}
                    className="w-full px-3 py-2 bg-white border border-slate-300 focus:border-amber-500 focus:ring-2 focus:ring-amber-400/20 rounded-xl text-xs font-medium text-slate-900 outline-none transition-all resize-none"
                  />
                </div>
                ...
                <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-2.5">
                  ...
                  <p className="text-[11px] text-amber-900 font-bold leading-relaxed">
                    سيتم حفظ الموكل مباشرة في قاعدة بيانات الموكلين السحابية، وربطه فورياً بالقضية الحالية، وسيظهر تلقائياً ضمن قائمة الموكلين في كافة أقسام النظام.
                  </p>
                </div>
                ...
                <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-slate-150">
                  ...
                  <button
                    type="submit"
                    disabled={isSavingManualClient}
                    className="px-5 py-2 bg-amber-500 hover:bg-amber-600 active:scale-95 text-slate-950 rounded-xl text-xs font-black shadow-md transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
                  >
                    {isSavingManualClient ? (
                      <>
                        <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                        <span>جاري حفظ الموكل...</span>
                      </>
                    ) : (
                      <>
                        <UserCheck className="w-4 h-4 text-slate-950" />
                        <span>حفظ الموكل</span>
                      </>
                    )}
                  </button>
                </div>
      ...
      {showDocViewer && (
        <SmartCaseFile
          ...
          onClose={() => {
            setShowDocViewer(null);
            if (onClearSelectedCaseId) onClearSelectedCaseId();
            if (onClearExternalSearch) onClearExternalSearch();
          }}
        />
      )}

      {/* Structured Detention Renewals Modal */}
      {detentionModalCase && detentionModalCase.isInvestigationActive && (
        <DetentionRenewalsModal
          isOpen={!!detentionModalCase && !!detentionModalCase.isInvestigationActive}
          onClose={() => setDetentionModalCase(null)}
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden text-right" dir="rtl">
            <div className="p-4 bg-slate-800/80 border-b border-slate-800 flex items-center justify-between">
              <div className="flex items-center gap-2">
                {isExpertSession(decisionSession) ? <Scale className="w-5 h-5 text-amber-400" /> : <Gavel className="w-5 h-5 text-amber-400" />}
                <h3 className="text-sm font-bold text-white">
                  {decisionSession.decision
                    ? isExpertSession(decisionSession)
                      ? 'تعديل إجراء وقرار جلسة الخبير'
                      : 'تعديل قرار الجلسة والنتيجة'
                    : isExpertSession(decisionSession)
                    ? 'تسجيل ورصد إجراء وقرار جلسة الخبير'
                    : 'تسجيل ورصد قرار الجلسة'}
                </h3>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleDecisionSubmit} className="p-5 space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 mb-1.5">
                  {isExpertSession(decisionSession)
                    ...
                    : isDetentionSession(decisionSession, cases.find(c => c.id === decisionSession.caseId))
                ...
                <textarea
                  placeholder={
                    isExpertSession(decisionSession)
                      ...
                      : 'مثال: التأجيل لجلسة القادمة لتقديم المستندات والاطلاع على التقرير...'
                  }
                  value={decisionText}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-400 mb-1.5">
                    {isExpertSession(decisionSession) ? 'موعد جلسة الخبير القادمة (إن وجد)' : 'تاريخ الجلسة القادمة (إن وجد)'}
                  </label>
                  <input
                    type="date"
                ...
                <div>
                  ...
                  <div className="text-[11px] bg-emerald-500/15 text-emerald-400 p-2.5 rounded-xl border border-emerald-500/30 font-bold flex items-center gap-1.5">
                    ...
                    {isExpertSession(decisionSession) ? 'سيتم قيدها كـ "جلسة خبرة منجزة ومسجلة"' : 'سيتم قيدها كـ "تمت ورصدت بالأجندة"'}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-slate-950 p-4 rounded-xl border border-dashed border-slate-800 text-center space-y-3">
                <div>
                  <p className="text-xs font-bold text-slate-300 mb-1">
                    {isExpertSession(decisionSession) ? 'إرفاق صورة محضر جلسة الخبير أو المعاينة' : 'إرفاق صورة رول الجلسة أو مستند منطوق الحكم'}
                  </p>
                ...
                <div className="animate-fadeIn space-y-8">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                ...
                                const found = (clients || []).find(cl => cl && cl.id === cid);
                                if (found) {
                            ...
                            >
                              ...
                              {(clients || []).map(c => (
                                <option key={c?.id || Math.random()} value={c?.id || ''}>{c?.name || 'موكل'}</option>
                              ))}
                            </select>
                          </div>
                        </div>
                              ...
                              {coPartners.map((partner, idx) => (
                                <div key={idx} className="space-y-1">
                                  <div className="flex items-center justify-between text-[11px]">
                                    <span className="font-bold text-slate-700">{partner?.name || 'شريك'}</span>
                        ...
                        <tbody className="divide-y divide-slate-100">
                          {coPartners.length === 0 ? (
                            ...
                            coPartners.map((partner, index) => {
                              ...
                              const isPartnerClient = clients.some(c => c && c.name === partner?.name);
                              return (
                  ...
                  <div className="pt-6 border-t border-slate-200">
                    ...
                    <div className="space-y-3 mt-4">
                      <h5 className="text-xs font-bold text-slate-800">قائمة الموكلين والممثلين الحاليين للشركة ({(clients || []).filter(c => c && c.companyId === (editingCompany?.id || currentCompanyId)).length})</h5>
                      {(clients || []).filter(c => c && c.companyId === (editingCompany?.id || currentCompanyId)).length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          {(clients || []).filter(c => c && c.companyId === (editingCompany?.id || currentCompanyId)).map(client => (
                            <div key={client?.id || Math.random()} className="bg-white border border-slate-150 p-4 rounded-xl flex items-center justify-between hover:shadow-xs transition-all">
                              <div>
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div className="space-y-3 pr-2">
                                    <div className="flex items-start justify-between gap-2">
                                      <div className="truncate flex-1">
                                        <h6 className="text-[11px] font-black text-slate-950 truncate" title={doc?.name || 'مستند'}>
                                          📄 {doc?.name || 'مستند'}
                                        </h6>
                                      </div>
                                      <span className={`text-[8px] font-extrabold px-1.5 py-0.5 rounded border leading-none ${formatConfig.bg}`}>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col" dir="rtl">
            <div className="p-5 border-b border-slate-100 flex items-center justify-between bg-slate-900 text-white rounded-t-2xl shrink-0">
              <h3 className="text-sm font-bold flex items-center gap-2">
                ...
                سجل وثائق وعقود وأوراق - {viewDocsCompany?.name || 'الشركة'}
              </h3>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl w-full max-w-md p-6" dir="rtl">
            <div className="flex items-center gap-2 text-amber-600 mb-3">
              ...
              <h3 className="text-sm font-bold">أرشفة الملف القانوني لشركة - {archiveCoTarget?.name || 'الشركة'}</h3>
            </div>
      ...
      {deleteCompanyTarget && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-red-200 shadow-2xl w-full max-w-md p-6" dir="rtl">
            ...
            <p className="text-xs text-slate-600 mb-4 leading-relaxed">
              تحذير: الحذف نهائي وسيؤدي لإزالة شركة <strong className="text-slate-800">{deleteCompanyTarget?.name || 'الشركة'}</strong> وكل الشركاء والملفات القانونية المرفقة تماماً من قاعدة البيانات. لتأكيد الإجراء، يرجى إدخال <strong className="text-slate-800">كلمة سر الدخول</strong> الخاصة بك.
            </p>
      ...
      {/* Case Detailed Report (Navy & Gold Comprehensive Modal) */}
      {showReportViewer && (
        <ComprehensiveCaseReportModal
          caseData={showReportViewer}
          clients={clients}
          users={users}
          sessions={sessions}
          onClose={() => setShowReportViewer(null)}
          onSendWhatsApp={() => handleSendReportWhatsApp(showReportViewer)}
        />
      )}

      {/* Detention Renewals Modal */}
      {detentionModalCase && detentionModalCase.isInvestigationActive && (
        <DetentionRenewalsModal
          isOpen={!!detentionModalCase && !!detentionModalCase.isInvestigationActive}
          onClose={() => setDetentionModalCase(null)}
          ...
          onUpdateCase={(updatedCase) => {
            onUpdateCase(updatedCase);
            setDetentionModalCase(updatedCase);
          }}
          currentUser={currentUser}
        />
      )}

      {/* Case Documents Manager & Attachment Modal */}
      {documentsModalCase && (
        <CaseDocumentsModal
          caseData={documentsModalCase}
          currentUser={currentUser}
          onClose={() => setDocumentsModalCase(null)}
          onUpdateCase={async (updatedCase) => {
            await onUpdateCase(updatedCase);
            setDocumentsModalCase(updatedCase);
          }}
          logAction={async (act, det) => {
            if (onAddAuditLog) {
              await onAddAuditLog(currentUser, act as any, det);
            }
          }}
        />
      )}

      {/* Standalone Case Note Modal */}
      {noteModalCase && (
        <CaseNoteModal
          isOpen={!!noteModalCase}
          caseData={noteModalCase}
          currentUser={currentUser}
          onClose={() => setNoteModalCase(null)}
          onSaveNote={async (note, updatedCase) => {
            await onUpdateCase(updatedCase);
            setNoteModalCase(updatedCase);
            if (onAddAuditLog && note) {
              await onAddAuditLog(currentUser, 'edit' as any, `إضافة ملاحظة قانونية لملف القضية (${updatedCase.caseNumberFirstInstance}/${updatedCase.caseYearFirstInstance}): ${note.text.substring(0, 40)}...`);
            }
          }}
          onDeleteNote={async (noteId, updatedCase) => {
            await onUpdateCase(updatedCase);
            setNoteModalCase(updatedCase);
            if (onAddAuditLog) {
              await onAddAuditLog(currentUser, 'edit' as any, `حذف ملاحظة من ملف القضية (${updatedCase.caseNumberFirstInstance}/${updatedCase.caseYearFirstInstance})`);
            }
          }}
        />
      )}
      ...
      {viewingCaseFile && (
        <DocumentViewerModal
          file={viewingCaseFile.file}
          caseData={viewingCaseFile.caseData}
          onClose={() => setViewingCaseFile(null)}
        />
      )}

      {/* Real-time Confirmation Toast Notification */}
      {toast.show && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[9999] transition-all duration-300 animate-in fade-in slide-in-from-bottom-5">
          <div className={`px-5 py-3 rounded-2xl shadow-2xl flex items-center gap-3 border text-sm font-black ${
            toast.type === 'success' 
              ? 'bg-slate-900 text-emerald-400 border-emerald-500/40 shadow-emerald-950/20' 
              : toast.type === 'error'
              ? 'bg-red-950 text-red-200 border-red-500/50 shadow-red-950/30'
            ...
            <button
              ...
              className="p-1 hover:bg-white/10 rounded-lg text-slate-400 hover:text-white transition-colors mr-2 cursor-pointer"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
