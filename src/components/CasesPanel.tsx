...
export default function CasesPanel({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const caseData: Case = {
      ...
      isInvestigationActive: isInvestigationActive || isReferredToCourt || !!editingCase?.isInvestigationActive || !!editingCase?.investigationNumber,
      investigationNumber: finalInvestigationNumber,
      investigationYear: finalInvestigationYear,
      investigationAuthority: investigationAuthority || undefined,
      investigationStartDate: investigationStartDate || undefined,
      investigationDefendantStatus: investigationDefendantStatus || undefined,
      detentionRenewals: detentionRenewals || [],
      investigationNotes: investigationNotes || undefined,
      investigationProcedures: editingCase ? editingCase.investigationProcedures : []
    };
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                        <div className="flex flex-wrap items-center gap-1.5">
                          ...
                          {(c.isInvestigationActive || c.status === 'قيد التحقيق') && (
                    ...
                    <div className="border-t border-slate-150 pt-4 flex flex-col gap-2.5">
                      {(c.isInvestigationActive || c.status === 'قيد التحقيق') && (
                        <button
                          onClick={() => {
                            setShowDocViewer(c);
                            setDocViewerTab('sessions');
                          }}
                          className="w-full py-2 px-3 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 text-amber-950 text-xs font-black rounded-xl transition-all duration-150 flex items-center justify-center gap-2 cursor-pointer shadow-3xs"
                        >
                          <span className="w-5 h-5 rounded-full bg-amber-500/20 border border-amber-500/35 flex items-center justify-center shrink-0 text-amber-800">
                            <ShieldAlert className="w-2.5 h-2.5 text-amber-700" />
                          </span>
                          <span>متابعة جلسات التحقيق وإجراءات الحبس الاحتياطي</span>
                        </button>
                      )}
                      ...
                      <div className="flex items-center justify-end gap-3 text-xs border-t border-slate-100/60 pt-3">
      ...
      >
        ...
        <div className="bg-slate-50 border border-slate-150 rounded-2xl px-6 py-3 flex items-center justify-between gap-2 shrink-0 overflow-x-auto select-none mb-6">
          {[
            ...
            const stepsOrder = ['investigation', 'judicial', 'litigants', 'financials', 'attachments'];
            const isCompleted = stepsOrder.indexOf(activeFormTab) > idx;
            return (
              <button
                key={step.id}
                type="button"
            ...
            <div className="space-y-6">
              {isInvestigationActive ? (
                <>
                  {(investigationDefendantStatus === 'تمت الإحالة للمحاكمة' || status === 'محالة للمحاكمة الجنائية') ? (
                    ...
                    <div className="p-4 bg-amber-50 border-2 border-amber-300 rounded-2xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-sm">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] font-bold text-amber-900/80 mt-0.5">
                            تم تجميد إدخال بيانات "درجة التقاضي والمحكمة" أوتوماتيكياً لحين صدور قرار الإحالة للمحكمة الجنائية.
                          </p>
                        </div>
                      </div>
                      ...
                      <button
                        type="button"
                        onClick={() => setShowCancelInvestigationConfirm(true)}
                      ...
                      >
                        ...
                        <span>إلغاء مرحلة التحقيق</span>
                      </button>
                    </div>
                  )}

                  <FormCard title="بيانات مرحلة التحقيق والتجديد" icon={ShieldAlert}>
                  <div className="space-y-4">
                    <FormGrid cols={4}>
                      <FormField label="رقم القضية بالتحقيقات" isMono>
                        <input
                          type="text"
                      ...
                      <FormField label="جهة التحقيق">
                        ...
                        >
                          ...
                          {investigationAuthority && !["النيابة العامة", "نيابة الأموال العامة", "نيابة الشؤون المالية والتجارية", "نيابة أمن الدولة العليا", "نيابة الطفل"].includes(investigationAuthority) && (
                    ...
                    <div className="border-t border-slate-100 pt-4 mt-2">
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                          ...
                          <span>سجل تجديدات الحبس الاحتياطي</span>
                        </h4>
                        <button
                          type="button"
                          onClick={() => {
                          ...
                          className="px-3 py-1 bg-amber-500/10 hover:bg-amber-500/20 text-amber-700 border border-amber-500/20 rounded-xl text-xs font-bold transition-all flex items-center gap-1 cursor-pointer"
                        >
                          <Plus className="w-3.5 h-3.5" />
                          <span>إضافة تجديد</span>
                        </button>
                      </div>

                      {detentionRenewals.length === 0 ? (
                        <div className="p-3 bg-slate-50 border border-slate-200/60 rounded-xl text-center text-xs text-slate-500">
                          لا توجد تجديدات مسجلة بعد. اضغط على "+ إضافة تجديد" لإضافة تجديد حبس جديد.
                        </div>
                      ) : (
                        <div className="space-y-2">
                            ...
                            <div key={ren.id || idx} className="grid grid-cols-12 gap-2 items-center bg-slate-50 border border-slate-200 p-2.5 rounded-xl text-xs">
                              ...
                              <div className="col-span-12 sm:col-span-2">
                                ...
                                <input
                                  ...
                                  className="w-full px-2 py-1 bg-amber-50/50 border border-amber-200 rounded-lg text-xs font-mono"
                                />
                              </div>
                              ...
                              <div className="col-span-12 sm:col-span-2">
                                ...
                                <select
                                  ...
                                  className="w-full px-2 py-1 bg-amber-50/50 border border-amber-200 rounded-lg text-xs font-bold text-amber-950 focus:ring-amber-500 focus:border-amber-500"
                                >
                                  <option value="">اختر الجهة...</option>
                                  <option value="النيابة العامة">النيابة العامة</option>
                                  <option value="تجديد جزئي">تجديد جزئي</option>
                              ...
                              <div className="col-span-10 sm:col-span-2">
                                <label className="block text-[10px] font-bold text-slate-600 mb-0.5">ملاحظات</label>
                                <input
                                  type="text"
                                  placeholder="ملاحظات القرارات..."
                                  value={ren.notes || ''}
                                  onChange={(e) => {
                                    const updated = [...detentionRenewals];
                    ...
                    <FormField label="ملاحظات مرحلة التحقيق">
                      <textarea
                        rows={2}
                        placeholder="أي ملاحظات إضافية عن مسار التحقيق وقرارات النيابة..."
                ...
                <div className="p-8 bg-slate-50 border border-slate-200 rounded-2xl text-center space-y-3">
                  ...
                  <p className="text-xs font-bold text-slate-600">مرحلة التحقيق والتجديد لدى النيابة العامة غير مفعلة لهذا الملف.</p>
                  <button
                    type="button"
                    onClick={() => {
                  ...
                  >
                    تفعيل مرحلة التحقيق والتجديد الآن ⚡
                  </button>
                </div>
              )}
            </div>
