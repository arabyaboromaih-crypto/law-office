...
export const ComprehensiveCaseReportModal: React.FC<ComprehensiveCaseReportModalProps> = ({
  ...
  const matchedClient = (clients || []).find(cl => cl && (cl.id === caseData.clientId || cl.name === caseData.clientName));
  ...
  const caseSessions = sessions.filter(s => s.caseId === caseData.id).filter(s => {
    const key = s.id || `${s.date}_${s.time || ''}_${s.subject || ''}`;
    if (seenSess.has(key)) return false;
    seenSess.add(key);
    return true;
  ...
  const filesList = (caseData.files || []).filter(f => {
    if (!f) return false;
    const key = f.id || `${f.name || 'file'}_${f.uploadDate || ''}`;
          ...
          <div className="max-w-4xl mx-auto bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 shadow-sm space-y-6 text-xs relative" style={{ fontFamily: "'Cairo', sans-serif" }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-3.5 space-y-3 shadow-xs">
                  ...
                  {caseData.clientsList && caseData.clientsList.length > 0 ? caseData.clientsList.map((cl, idx) => (
                    <div key={idx} className="bg-emerald-50/70 border border-emerald-200 rounded-lg p-2.5 text-emerald-950">
                      <p className="font-extrabold text-emerald-900">الموكل ({idx + 1}): {cl?.name || 'غير مدون'}</p>
                  ...
                  {caseData.opponentsList && caseData.opponentsList.length > 0 ? caseData.opponentsList.map((opp, idx) => (
                    <div key={idx} className="bg-rose-50/70 border border-rose-200 rounded-lg p-2.5 text-rose-950">
                      <p className="font-extrabold text-rose-900">الخصم ({idx + 1}): {opp?.name || 'غير مدون'}</p>
                      <p className="text-[10px] text-rose-700 mt-0.5">الصفة: {opp?.role || 'خصم'} | محاميه: {opp?.lawyer || 'لا يوجد'}</p>
                    </div>
                  )) : (
                    <div className="bg-rose-50/70 border border-rose-200 rounded-lg p-2.5 text-rose-950">
                      <p className="font-extrabold text-rose-900">الخصم: {caseData.opponent?.name || 'غير مدون'}</p>
                      <p className="text-[10px] text-rose-700 mt-0.5">الصفة: {caseData.opponent?.role || 'خصم'}</p>
              ...
              <div className="space-y-5 md:col-span-2">
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-xs space-y-2">
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 text-slate-800 leading-relaxed text-xs space-y-2">
                    {caseData.subject && <p className="font-bold text-amber-800 mb-1">📌 الموضوع والطلبات: {caseData.subject}</p>}
                    {caseData.notes && <p className="text-slate-700">{caseData.notes}</p>}
                    {!caseData.notes && !caseData.caseNotes?.length && (
                      <p className="text-slate-600">{`دعوى مقيدة برقم (${caseData.caseNumberFirstInstance} لسنة ${caseData.caseYearFirstInstance}) أمام محكمة ${caseData.court} الدائرة ${caseData.circuit}، دفاعاً عن الموكل (${caseData.clientName}) في مواجهة الخصم (${caseData.opponent?.name || 'الطرف الآخر'}).`}</p>
                    )}
                    {caseData.caseNotes && caseData.caseNotes.length > 0 && (
                      <div className="pt-2 border-t border-slate-200 space-y-2">
                        <span className="font-bold text-slate-900 block text-[11px]">💡 ملاحظات ملف القضية الموثقة:</span>
                        {caseData.caseNotes.map((cn, idx) => (
                          <div key={cn.id || idx} className="bg-white p-2.5 rounded-lg border border-slate-200 space-y-1">
                            <div className="flex items-center justify-between text-[10px] text-slate-500 border-b border-slate-100 pb-1">
                              <span className="font-bold text-amber-800">👤 {cn.createdBy}</span>
                              <span className="font-mono">{cn.createdAtFormatted || cn.createdAt}</span>
                            </div>
                            <p className="text-slate-800 whitespace-pre-line text-xs">{cn.text}</p>
                          </div>
                        ))}
                      </div>
                    )}
            ...
            <div className="pt-6 border-t-2 border-slate-200 space-y-4">
              ...
              <div className="bg-[#0b1b2b] text-slate-300 p-2.5 rounded-xl flex flex-col sm:flex-row justify-between items-center text-[10px] gap-2 border-t-2 border-amber-500">
                <div>📞 الهاتف الرئيسي: <strong className="text-amber-400">01143472682</strong></div>
