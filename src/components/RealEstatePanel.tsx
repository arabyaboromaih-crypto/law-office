...
export default function RealEstatePanel({ currentUser }: RealEstatePanelProps) {
  ...
  const [isOwnerStatementsCleared, setIsOwnerStatementsCleared] = useState<boolean>(false);

  // Subscriptions to Firestore (real-time synced)
  const [deletedDueIds, setDeletedDueIds] = useState<string[]>([]);
  const [deletedEntityIds, setDeletedEntityIds] = useState<Set<string>>(new Set<string>());
  ...
  useEffect(() => {
    ...
    validContractTenants.forEach(tenant => {
      ...
      while (current <= last && count < 120) {
        ...
        const isSuspended = isTenantMonthSuspended(mY, tenant);

        if (!exists && !isDeleted && !isSuspended) {
          ...
          // Check for existing saved rent adjustment for this tenant & month (applied from effective month onwards)
    ...
    try {
      ...
      for (const [, items] of groups.entries()) {
        if (items.length > 1) {
          ...
          items.sort((a, b) => {
            const hasAdjA = a.isAdjusted || !!getApplicableRentAdjustment(rentAdjustments, a.tenantId, a.tenantName, a.forMonthYear);
            const hasAdjB = b.isAdjusted || !!getApplicableRentAdjustment(rentAdjustments, b.tenantId, b.tenantName, b.forMonthYear);
  ...
  const generateDuesForSingleTenant = async (tenant: ReTenant) => {
    ...
    while (current <= last && count < 120) {
      ...
      // Check for existing saved rent adjustment for this tenant & month (applied forward)
      const matchedAdj = getApplicableRentAdjustment(rentAdjustments, tenant.id, tenant.fullName, mY);
    ...
    <div className="w-full h-full bg-gradient-to-br from-[#08111F] via-[#0D1B2A] to-[#132238] rounded-2xl border border-[#D4A84F]/20 overflow-hidden shadow-[0_15px_40px_rgba(0,0,0,0.5)] flex flex-col lg:flex-row relative text-[#F8F9FB]" dir="rtl">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-5">
                  {filteredOwners.map((owner, idx) => {
                    ...
                    const getArabicInitial = (name: string) => {
                      ...
                      return cleaned ? cleaned.charAt(0) : (name ? name.charAt(0) : 'م');
                    };
      ...
      {isModalOpen && modalType === 'collect_rent' && (
        <AddCollectionReceiptModal
          ...
          onSaveRentAdjustment={async (adj, updatedDue) => {
            setRentAdjustments(prev => {
              const filtered = prev.filter(a => a.id !== adj.id);
              return [...filtered, adj];
            });
            setDues(prev => {
              return prev.map(d => {
                const matchTenant = (d.tenantId && d.tenantId === adj.tenantId) || 
                                    (d.tenantName && d.tenantName === adj.tenantName);
                if (matchTenant && d.forMonthYear && d.forMonthYear >= adj.forMonthYear && d.status !== 'collected') {
                  const isCurrent = (d.id && d.id === updatedDue.id) || d.forMonthYear === adj.forMonthYear;
                  if (isCurrent) return { ...d, ...updatedDue };
                  return {
                    ...d,
                    rentAmount: adj.adjustedRentAmount,
                    isAdjusted: true,
                    adjustedRentAmount: adj.adjustedRentAmount,
                    commissionType: adj.commissionType,
                    commissionValue: adj.commissionValue,
                    commissionAmount: adj.commissionAmount,
                    netOwnerAmount: adj.netOwnerAmount
                  };
                }
                return d;
              });
            });
            setTenants(prev => prev.map(t => (t.id === adj.tenantId || t.fullName === adj.tenantName) ? { ...t, rentAmount: adj.adjustedRentAmount } : t));
          }}
          onDeleteRentAdjustment={async (adjId, resetDue) => {
            setRentAdjustments(prev => prev.filter(a => a.id !== adjId));
            setDues(prev => prev.map(d => {
              const matchTenant = (d.tenantId && d.tenantId === resetDue.tenantId) || 
                                  (d.tenantName && d.tenantName === resetDue.tenantName);
              if (matchTenant && d.forMonthYear && d.forMonthYear >= resetDue.forMonthYear && d.status !== 'collected') {
                const isCurrent = (d.id && d.id === resetDue.id) || d.forMonthYear === resetDue.forMonthYear;
                if (isCurrent) return resetDue;
                const orig = resetDue.originalRentAmount || d.originalRentAmount || d.rentAmount;
                return {
                  ...d,
                  rentAmount: orig,
                  isAdjusted: false,
            ...
            if (resetDue.originalRentAmount) {
              setTenants(prev => prev.map(t => (t.id === resetDue.tenantId || t.fullName === resetDue.tenantName) ? { ...t, rentAmount: resetDue.originalRentAmount! } : t));
            }
          }}
        />
      )}
                ...
                <div id="printable-receipt-card" className="p-8 space-y-6 bg-white border border-slate-100 rounded-2xl m-4 md:m-6 shadow-inner relative">
                  ...
                  <div className="flex justify-between items-start border-b-2 border-slate-900 pb-4">
                    <div className="space-y-1">
                      ...
                      <p className="text-[9px] text-slate-400 font-bold">مصر - التجمع الخامس والرحاب - هاتف: 01143472682</p>
                    </div>
