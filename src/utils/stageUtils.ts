...
export function computeCaseDegree(
  c: Partial<Case>,
  showAppealSection?: boolean,
  showCassationSection?: boolean
): LitigationDegree {
  // 1. Cassation (النقض): if cassation section is active or cassation data is recorded
  if (
  ...
  // 2. Appeal (الاستئناف): if appeal section is active or appeal data is recorded
  if (
    showAppealSection ||
    !!c.caseNumberSecondInstance ||
    !!c.courtSecondInstance
  ) {
    return 'استئناف';
  }
  // 3. Investigation (تحقيق): if investigation active
  if (
...
export function getEffectiveStageInfo(c: Case | undefined, session?: HearingSession): EffectiveStageInfo {
  if (!c) {
    if (session && isExpertSession(session)) {
      return {
        degreeLabel: 'خبراء',
        caseNumber: session.caseNumber || '',
        caseYear: session.caseYear || '',
        court: session.court || 'مكتب خبراء وزارة العدل',
        circuit: session.circuit || 'جلسة خبراء',
        isAppeal: false,
        isCassation: false,
        isInvestigation: false,
        isFirstInstance: false,
        stageNameAr: 'مكتب خبراء وزارة العدل',
        badgeText: 'جلسة خبراء',
  ...
  // 0. Check Expert Session (جلسة خبراء)
  if (session && isExpertSession(session)) {
    return {
      degreeLabel: 'خبراء',
      caseNumber: c.caseNumberFirstInstance || session.caseNumber || '',
      caseYear: c.caseYearFirstInstance || session.caseYear || '',
      court: session.court || c.expertReferral?.expertOffice || 'مكتب خبراء وزارة العدل',
      circuit: session.circuit || c.expertReferral?.courtOrCircuit || 'جلسة خبراء',
      isAppeal: false,
      isCassation: false,
      isInvestigation: false,
      isFirstInstance: false,
  ...
  // 3. Check Investigation / Detention Renewal (التحقيق والتجديد - مشروط حصرياً بتفعيل مرحلة التحقيق)
  const isInvestigation = !!c.isInvestigationActive;
  const isDetentionSession = !!(isInvestigation && session && (
    session.isDetentionRenewal || 
    session.detentionRenewalId ||
    session.detentionRenewalNumber ||
    session.detentionAuthority ||
    (session.id && (session.id.includes('detention') || session.id.includes('session-detention'))) ||
