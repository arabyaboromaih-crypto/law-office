...
export function generateAllCasesComprehensiveReportHTML(
  ...
  uniqueCases.forEach((c) => {
    const matchedClient = clients.find(
      (cl) => cl && (cl.id === c.clientId || (cl.name && cl.name.trim() === c.clientName?.trim()))
    );
    ...
    group.cases.forEach((c) => {
      ...
      casesHtml += `
        <div class="case-card keep-together ${themeClass}">
          ...
          <!-- Notes (If any) -->
          ${c.notes || (c.caseNotes && c.caseNotes.length > 0) ? `
            <div class="notes-box" style="padding:8px 12px; font-size:11px;">
              ${c.notes ? `<div><strong>📝 ملاحظات وتوجيهات المكتب:</strong> ${c.notes}</div>` : ''}
              ${c.caseNotes && c.caseNotes.length > 0 ? `
                <div style="margin-top: ${c.notes ? '6px' : '0'};">
                  <strong>💡 ملاحظات ملف القضية:</strong>
                  <ul style="margin: 4px 0 0 0; padding-right: 18px; list-style-type: square;">
                    ${c.caseNotes.map(cn => `
                      <li style="margin-bottom: 3px;">
                        <span style="font-weight: bold; color: #1e293b;">${cn.text}</span>
                        <span style="color: #64748b; font-size: 10px;">(${cn.createdBy} - ${cn.createdAtFormatted || cn.createdAt})</span>
                      </li>
                    `).join('')}
                  </ul>
                </div>
...
export function generateAllCompaniesComprehensiveReportHTML(
  ...
  const uniqueCompanies = (companies || []).filter(co => {
    if (!co) return false;
    const key = co.id || (co.name ? co.name.trim() : Math.random().toString());
            ...
            <tbody>
              ${partnersList.map((p, pIdx) => `
                <tr>
                  ...
                  <td style="font-weight:700;">${p?.name || 'شريك'}</td>
                  <td>${p?.nationalId ? toArabicNum(p.nationalId) : '-'}</td>
                  <td>${p?.phone ? toArabicNum(p.phone) : '-'}</td>
                  <td style="font-weight:800; color:#b45309; text-align:center;">${toArabicNum(p?.participationPercentage || 0)}%</td>
            ...
            <tbody>
              ${docsList.map((d, dIdx) => `
                <tr>
                  ...
                  <td style="font-weight:700;">${d?.name || 'مستند'}</td>
