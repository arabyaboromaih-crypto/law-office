...
import { getEffectiveStageInfo } from './stageUtils';
...
export function buildLitigationTimeline(
  ...
  // 2. Stage 2: Investigation / Detention (التحقيقات وتجديد الحبس - تظهر حصرياً عند تفعيل مرحلة التحقيق)
  if (c.isInvestigationActive) {
    ...
    if (c.detentionRenewals && c.detentionRenewals.length > 0) {
      const seenRen = new Set<string>();
      const uniqueRenewals = c.detentionRenewals.filter(ren => {
        const rKey = ren.id || `${ren.date || ren.renewalDate || ''}_${ren.authority || ''}_${ren.decision || ''}`;
        if (seenRen.has(rKey)) return false;
        seenRen.add(rKey);
        return true;
      });
      uniqueRenewals.forEach((ren) => {
        stages.push({
          stepNumber: stepCounter++,
  ...
  const uniqueSessions = caseSessions.filter(s => {
    const sKey = s.id || `${s.date}_${s.time || ''}_${s.subject || ''}_${s.decision || ''}`;
    if (seenSessions.has(sKey)) return false;
    seenSessions.add(sKey);
    return true;
  });
  const sortedSessions = [...uniqueSessions].sort((a, b) => a.date.localeCompare(b.date));
  ...
  const hasFutureHearingInSessions = sortedSessions.some(s => s.date === c.nextHearingDate);
  if (c.nextHearingDate && !hasFutureHearingInSessions) {
    stages.push({
      stepNumber: stepCounter++,
...
export function generateCaseReportHTML(
  ...
  const effStage = getEffectiveStageInfo(c);
  const matchedClient = (clients || []).find(cl => cl && (cl.id === c.clientId || cl.name === c.clientName));
  const assignedLawyer = (users || []).find(u => u && u.id === c.assignedLawyerId);
  ...
  const sortedSessions = [...uniqueSessions].sort((a, b) => (a?.date || '').localeCompare(b?.date || ''));
  const timelineStages = buildLitigationTimeline(c, uniqueSessions);

  const seenFiles = new Set<string>();
  const filesList = (c.files || []).filter(f => {
    if (!f) return false;
    const fKey = f.id || `${f.name || ''}_${f.uploadDate || ''}`;
    if (seenFiles.has(fKey)) return false;
    seenFiles.add(fKey);
    return true;
  });
  ...
  return `
    ...
    <html lang="ar" dir="rtl">
      <head>
        <meta charset="utf-8" />
        <title>تقرير قضائي شامل - قضية رقم ${effStage.caseNumber || c.caseNumberFirstInstance} لسنة ${effStage.caseYear || c.caseYearFirstInstance} - ${effStage.degreeLabel}</title>
        <style>
        ...
        <div class="report-container">
          ...
          <div class="metrics-ribbon">
            ...
            <div class="metrics-grid">
              <div class="metric-card">
                ...
                <div class="metric-value">${effStage.court} - د/ ${effStage.circuit}</div>
              </div>
              <div class="metric-card">
              ...
              <div class="metric-card">
                ...
                <div class="metric-value gold">${c.type} (${effStage.degreeLabel})</div>
              </div>
              <div class="metric-card">
              ...
              <div class="side-card">
                ...
                ${c.clientsList && c.clientsList.length > 0 ? c.clientsList.map((cl, idx) => `
                  <div class="party-box client">
                    <div class="party-title">الموكل (${idx + 1}): ${cl?.name || 'موكل'}</div>
                ...
                ${c.opponentsList && c.opponentsList.length > 0 ? c.opponentsList.map((opp, idx) => `
                  <div class="party-box opponent">
                    <div class="party-title">الخصم (${idx + 1}): ${opp?.name || 'غير مدون'}</div>
                    <div class="party-detail">الصفة: ${opp?.role || 'خصم'} | محاميه: ${opp?.lawyer || 'لا يوجد'}</div>
                  </div>
                `).join('') : `
                  <div class="party-box opponent">
                    <div class="party-title">الخصم: ${c.opponent?.name || 'غير مدون'}</div>
                    <div class="party-detail">
                      الصفة: ${c.opponent?.role || 'خصم'}<br/>
            ...
            <div class="left-main">
              ...
              <div class="main-section">
                ...
                <div style="font-size: 10px; color: #1e293b; line-height: 1.6; text-align: justify; background: #f8fafc; padding: 8px 10px; border-radius: 6px; border: 1px solid #e2e8f0;">
                  ...
                  ${c.notes ? `<p style="margin-bottom: 4px;">${c.notes}</p>` : ''}
                  ${!c.notes && !c.caseNotes?.length ? `<p>دعوى مقيدة برقم (${c.caseNumberFirstInstance} لسنة ${c.caseYearFirstInstance}) أمام محكمة ${c.court} الدائرة ${c.circuit}، دفاعاً عن الموكل (${c.clientName}) في مواجهة الخصم (${c.opponent?.name || 'الطرف الآخر'}).</p>` : ''}
                  ${c.caseNotes && c.caseNotes.length > 0 ? `
                    <div style="margin-top: 6px; padding-top: 6px; border-top: 1px dashed #cbd5e1;">
                      <strong style="font-size: 9.5px; color: #334155; display: block; margin-bottom: 3px;">💡 ملاحظات ملف القضية:</strong>
                      ${c.caseNotes.map(cn => `
                        <div style="background: #ffffff; padding: 4px 6px; border-radius: 4px; border: 1px solid #e2e8f0; margin-bottom: 3px; font-size: 9px;">
                          <div style="display: flex; justify-content: space-between; color: #64748b; font-size: 8.5px; margin-bottom: 2px;">
                            <span>بواسطة: <b>${cn.createdBy}</b></span>
                            <span>${cn.createdAtFormatted || cn.createdAt}</span>
                          </div>
                          <div style="color: #1e293b; white-space: pre-line;">${cn.text}</div>
                        </div>
                      `).join('')}
                    </div>
                  ` : ''}
                ...
                <div class="main-section">
                  ...
                  <table class="custom-table">
                    ...
                    <tbody>
                      ${filesList.map(f => `
                        <tr>
                          <td style="font-weight: 700;">📄 ${f?.name || 'مستند'}</td>
          ...
          <div class="footer-section">
            ...
            <div class="bottom-bar">
              <div>📞 الهاتف الرئيسي: <strong>01143472682</strong></div>
