...
export function handleFirestoreError(error: unknown, operationType: OperationType, path: string | null) {
  ...
  // Handle transient offline / reconnecting states gracefully without crashing the app
  ...
  if (isOfflineOrUnavailable) {
    console.warn(`[Firestore ${operationType} - ${path}] Network is currently unavailable or client is operating in offline mode. Continuing with local cache.`);
    return;
  }
...
// Deep sanitizer to strip undefined values so Firestore never throws Unsupported field value: undefined
...
export async function addFirestoreDoc(collectionName: string, data: any, customId?: string) {
  ...
  try {
    ...
    if (customId) {
      ...
      const dataToSave = { ...sanitized, id: customId };
      await setDoc(docRef, dataToSave, { merge: true });
      return { id: customId, ...dataToSave };
    } else {
      const colRef = collection(db, collectionName);
      const docRef = await addDoc(colRef, sanitized);
      await setDoc(docRef, { ...sanitized, id: docRef.id }, { merge: true });
...
// 6. Save commission status using robust sanitized setDoc with merge
export async function saveCommissionStatusDoc(statusRecord: any) {
  ...
  // Sanitize data to prevent any undefined field error in Firestore
  const sanitizedData: Record<string, any> = {
    id: cleanDocId,
    propertyId: statusRecord.propertyId || '',
    propertyName: statusRecord.propertyName || '',
    ownerId: statusRecord.ownerId || '',
    ownerName: statusRecord.ownerName || '',
    forMonthYear: statusRecord.forMonthYear || '',
    status: statusRecord.status || 'collected',
    isCollectedFromOwner: Boolean(statusRecord.isCollectedFromOwner ?? true),
    amountCollectedFromOwner: Number(statusRecord.amountCollectedFromOwner) || 0,
    collectionDate: statusRecord.collectionDate || new Date().toISOString().slice(0, 10),
    paymentMethod: statusRecord.paymentMethod || 'نقدي',
    referenceNumber: statusRecord.referenceNumber ? String(statusRecord.referenceNumber).trim() : '',
    notes: statusRecord.notes ? String(statusRecord.notes).trim() : '',
    updatedAt: statusRecord.updatedAt || new Date().toISOString().slice(0, 10),
    updatedBy: statusRecord.updatedBy || 'المسؤول',
    receipts: Array.isArray(statusRecord.receipts)
      ? statusRecord.receipts.map((r: any) => ({
          id: r.id || `rec_${Date.now()}`,
          receiptNumber: r.receiptNumber || statusRecord.referenceNumber || '',
          propertyId: r.propertyId || statusRecord.propertyId || '',
          propertyName: r.propertyName || statusRecord.propertyName || '',
          ownerId: r.ownerId || statusRecord.ownerId || '',
          ownerName: r.ownerName || statusRecord.ownerName || '',
          forMonthYear: r.forMonthYear || statusRecord.forMonthYear || '',
          amount: Number(r.amount) || 0,
          date: r.date || statusRecord.collectionDate || '',
          paymentMethod: r.paymentMethod || statusRecord.paymentMethod || 'نقدي',
          referenceNumber: r.referenceNumber || statusRecord.referenceNumber || '',
          notes: r.notes || '',
          collectedBy: r.collectedBy || statusRecord.updatedBy || 'المسؤول',
          createdAt: r.createdAt || statusRecord.updatedAt || ''
        }))
      : []
  };

  // Copy any additional custom properties while omitting undefined values
  Object.keys(statusRecord).forEach(key => {
    if (statusRecord[key] !== undefined && sanitizedData[key] === undefined) {
      sanitizedData[key] = statusRecord[key];
    }
  });
  ...
  try {
    ...
    console.error(`Error saving commission status doc ${cleanDocId}:`, error?.message || error, error?.stack || "");
