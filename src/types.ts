...
export interface CaseNote {
  ...
  createdAtFormatted: string; // Formatted date and time
  createdBy: string; // اسم المستخدم الذي أضاف الملاحظة
  userId?: string;
  userRole?: string;
}
...
export interface Case {
  ...
  noUpcomingHearing?: boolean; // لا توجد جلسة محددة للقضية
  ...
  notes?: string;
  caseNotes?: CaseNote[]; // ملاحظات القضية المسجلة
...
export interface ReCommissionReceipt {
  id: string;
  receiptNumber: string;
  propertyId: string;
  propertyName?: string;
  ownerId?: string;
  ownerName?: string;
  forMonthYear: string; // e.g. "2026-07" or "حساب إجمالي شامل"
  amount: number;
  date: string;
  paymentMethod: string;
